...
- **`git add` and `git push` are blocked in Cowork.** Cowork's role per CLAUDE.md is design/docs/Linear only — it cannot commit or push. **Workaround:** stage handoff via Linear ("Ready for Dev") and the `session-handoff` skill; Claude Code performs the actual git operations. Repeated occurrences: impediments #23, #24.
