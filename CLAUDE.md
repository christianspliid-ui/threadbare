...
- [ ] **Update docs** — write your Current-Focus narrative to a **new** `Docs/status/YYYY-MM-DD-thr-XXXX.md` and run `npm run generate-project-status`; add a one-line `✅` entry to `project-history.md`; append rows to `changelog.md`. Add a completion comment to the Linear issue (the `Fixes THR-XX` keyword in the commit auto-closes it, but a human-readable comment is still expected). **Never hand-edit `Docs/project-status.md`, and never trim another entry to make room** — it is generated, and the generator holds the ≤60-line cap by rendering only the newest fragments that fit (THR-1016). Everything older stays in `Docs/status/`, uncapped.
  ...
  - **Merging.** `changelog.md`, `project-history.md` and `impediments.md` are `merge=union` (THR-691) and `project-status.md` is `merge=ours` (THR-1016), so a stale branch resolves **locally** without judgment — GitHub's server-side merge ignores `.gitattributes` (measured), so a PR that idles into `CONFLICTING` is fixed with `git merge origin/main && npm run generate-project-status && git commit -a && git push`, not the web resolver. The regeneration is not optional: `ours` keeps *your* page, and `check:generated-freshness` is blocking and will reject it if you skip. See `.claude/skills/pull-work/SKILL.md` § "Closeout — resolving a conflicted closeout-docs PR".
