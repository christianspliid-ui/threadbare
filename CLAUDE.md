...
| `?spawn=<templateId>` | **Direct-URL encounter spawn (THR-883).** Stages the named encounter template on `@hero` at the attended tier and opens it as soon as the world can resolve an agent — a shareable one-click review link for a specific encounter. Works on the deployed build (URL flag, the `?forceencounters` pattern). Fail-soft: an unknown id retries briefly, warns once, and the game proceeds. Example: `?view=game&seeded&size=medium&spawn=encounter.slice.unsafe_bridge` |
