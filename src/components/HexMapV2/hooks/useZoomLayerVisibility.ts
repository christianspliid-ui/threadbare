...
export interface UseZoomLayerVisibilityParams {
  ...
  groups: {
    ...
    borderRealm: React.MutableRefObject<THREE.Mesh | null>;
    ...
    /** Battle crossed-swords indicators — controlled by 'battle_indicator' matrix entry (Plan 13-04) */
    battleIndicator?: React.MutableRefObject<THREE.Group | null>;
    /** Siege ring indicators — controlled by 'siege_ring' matrix entry (Plan 13-04) */
    siegeRing?: React.MutableRefObject<THREE.Group | null>;
  };
...
export function useZoomLayerVisibility({
  ...
  useEffect(() => {
    ...
    if (groups.borderRealm.current) groups.borderRealm.current.visible = ZOOM_VISIBILITY_MATRIX.borders_realm[tier];
    ...
    if (groups.battleIndicator?.current) groups.battleIndicator.current.visible = ZOOM_VISIBILITY_MATRIX.battle_indicator[tier];
    if (groups.siegeRing?.current) groups.siegeRing.current.visible = ZOOM_VISIBILITY_MATRIX.siege_ring[tier];
