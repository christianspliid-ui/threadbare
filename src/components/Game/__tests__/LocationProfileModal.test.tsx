...
import {
  ...
  CONDITION_TENDED_SHRINE_DURATION,
...
describe('LocationProfileModal — active conditions (THR-1143)', () => {
  ...
  it('THR-1175: a positive place-state reaches the player on the place that carries it', () => {
    // The acted-on half of THR-1175, re-pointed by THR-1483. The Grateful Kin used
    // to express "there is a roof in this town that opens for them now" as an
    // `owes_favor` edge with the *town* as debtor — real, well-formed, and inert,
    // because every favour consumer is person-shaped, so the only code that would
    // ever touch it again was the expiry sweep. A place-carried condition replaces
    // it, and this is the assertion that it is not the same defect in a new coat:
    // the state a player was promised is on the surface a player can open, named in
    // words, on the place the chip anchors to.
    //
    // The noun moved twice and the claim did not. THR-1206 retired
    // `standing_welcome` in favour of reputation, and THR-1483 deleted the
    // definition once its read-tolerance window lapsed; `tended_shrine` is the
    // live positive location condition now, and it makes exactly the same point.
    const graph = graphWithKeep();
    seedEncounterTraitDefinitions(graph);
    graph.addEdge({
      id: 'has_trait_loc_0_tended_shrine',
      source: 'loc_0',
      target: 'trait.condition.location.tended_shrine',
      type: 'has_trait',
      properties: {
        appliedAt: 10,
        durationTicks: CONDITION_TENDED_SHRINE_DURATION,
        ticksRemaining: CONDITION_TENDED_SHRINE_DURATION,
        intensity: 0.55,
      },
    } as never);
    ...
    const section = screen.getByTestId('location-profile-conditions');
    expect(section.textContent).toContain('A Tended Shrine');
    expect(section.textContent).toContain('Conditions (1)');
