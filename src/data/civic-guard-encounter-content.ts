...
export const CIVIC_GUARD_ENCOUNTER_TEMPLATES: EncounterTemplate[] = [
...

  {
    ...
    name: 'Gate Duty',
    locationTypes: ['town', 'city', 'capital'],
    sublocationTypes: ['sublocation-type.gatehouse'],
    steps: [
      {
        id: 'cg.quest.gate_duty.1',
        name: 'Read the Queue',
        narrative: 'The gate is already under judgment. Read the queue before waiting turns into watching and a stoppage becomes a story.',
        reach: 'eye',
        difficulty: CG_DIFFICULTY_BASE,
        duration: 2,
        onSuccess: {
          narrative: 'You feel the wrongness in the papers before the whole line can name it.',
          reputationDelta: 0.01,
        },
        onFailure: {
          narrative: 'Your eye skims the line without mastering it; the crowd begins to write the scene for itself.',
          reputationDelta: -0.005,
        },
      },
      {
        id: 'cg.quest.gate_duty.2',
        name: 'Seize the Courier',
        narrative: 'The seizure begins. Stop the courier cleanly enough that the gate still looks like shelter rather than appetite.',
        reach: 'iron',
        difficulty: CG_DIFFICULTY_BASE + CG_DIFFICULTY_STEP,
        duration: 2,
        onSuccess: {
          narrative: 'The courier is stopped, and the line sees discipline before it sees spectacle.',
          reputationDelta: 0.015,
        },
        onFailure: {
          narrative: 'The stop turns ugly enough that the crowd remembers force before proof.',
          reputationDelta: -0.015,
        },
      },
      {
        id: 'cg.quest.gate_duty.3',
        name: 'Hold the Line',
        narrative: 'The cart is stopped, but the story is still moving. Decide what kind of authority the checkpoint becomes after force enters it.',
        reach: 'heart',
        difficulty: CG_DIFFICULTY_BASE + CG_DIFFICULTY_STEP + 5,
        duration: 1,
        onSuccess: {
          narrative: 'The checkpoint settles, but its memory now belongs partly to you.',
        ...
        onFailure: {
          narrative: 'The gatehouse keeps standing, but the district leaves with a harsher story than the watch can contain.',
          reputationDelta: -0.02,
          rewardPool: {
