...
export const HIDDEN_MOTIVE_PROSE: Record<string, string[]> = {
  'tit-for-tat': [
    ...
    'The deeper logic beneath {name}\'s {strategy_description} is a theory of systems: that groups which cooperate outperform groups that don\'t, and that {name}\'s individual behavior is a small input into which version of that future arrives.',
    '{name}\'s {strategy_description} contains a patience that others mistake for calculation. It is calculation, but of a kind that functions on longer timescales than most — the patience of someone who treats their own behavior as investment rather than transaction.',
  ],
  grudger: [
    ...
    'The record that {name} keeps is not spite but information. {strategy_description} because certain data points are load-bearing: who, when tested, has proven reliable, and who has not. The memory serves a function; the function is not vindictiveness but risk management.',
    'What {name}\'s {strategy_description} is actually about is not the past but the future: the specific management of exposure that comes from knowing who to extend trust to and under what conditions. The holding of grudges is, from their position, accurate bookkeeping.',
  ],
  pavlov: [
    ...
    'The pattern beneath {name}\'s {strategy_description} is adaptive rather than deliberate — a response to a world that has consistently rewarded certain behaviors. What looks like calculation from the outside is, from the inside, following a path that has proven itself.',
    'What {name}\'s {strategy_description} reveals is not their strategy but their history. The approach that results in cooperation or defection is a map of what has worked before, written in behavior rather than language.',
  ],
  'always-cooperate': [
    ...
    'The motive beneath {name}\'s {strategy_description} is not innocence but a specific conviction about causation: that cooperative behavior, modeled consistently, alters the environment in which it operates. {name} is not naive. They are investing in a world that does not yet exist.',
    '{name}\'s {strategy_description} contains a premise worth examining: that the cost of occasional exploitation is lower than the cost of the defensive behavior that, maintained long enough, becomes indistinguishable from the behavior it was designed to prevent.',
  ],
  'always-defect': [
    ...
    'The logic beneath {name}\'s {strategy_description} is not malice but hypothesis: that the world, observed carefully enough, reveals a structure in which the cooperative consistently arrive second. {name} has drawn a conclusion from that structure and applied it consistently.',
    'What {name}\'s {strategy_description} is actually serving is a specific belief about human nature — not cynicism as aesthetic but cynicism as empiricism. They have gathered the data. This is the interpretation. They are, in theory, open to evidence that would revise it.',
  ],
};
