...
/**
 * The entry shape and the three tag readers moved to `contentEntryTags.ts` (THR-1487) so
 * that reading an entry's tags no longer requires loading every catalog in the game —
 * the import weight closed a cycle through `undertaking-cells`. Re-exported here because
 * this is where callers have always found them, and the move is not theirs to care about.
 */
export type { ContentCatalogEntry } from './contentEntryTags';
export { authoredTags, projectedTags, effectiveTags } from './contentEntryTags';
import type { ContentCatalogEntry } from './contentEntryTags';
