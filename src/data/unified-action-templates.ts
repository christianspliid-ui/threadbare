...
import { PAGE_BENEATH_THE_SAINT_TEMPLATE } from './encounters/the-page-beneath-the-saint';
import { VERDICT_THAT_BURNS_TEMPLATE } from './encounters/the-verdict-that-burns';
import { GRANARIES_IN_THE_FAMINE_YEAR_TEMPLATE } from './encounters/the-granaries-in-the-famine-year';
...
const DIVINE_ACTION_TEMPLATES: UnifiedActionTemplate[] = [
  ...
  // ─── Self-Targeting Divine Actions (THR-399) ────────────────────────────
  {
    id: 'divine.self.stillness',
    name: 'Stillness',
    spellName: 'The Long Stillness',
    rarityTier: 1,
    intrinsicTier: 'background',
    description: 'The god draws inward, releasing the constant attention that divine will requires and allowing essence to pool in the space of that letting-go. A moment of deliberate stillness restores what constant intervention depletes. The mortal world moves on without celestial interference — and the god is the richer for it.',
    reach: 'heart',
    crudType: 'update',
    scale: 'personal',
    steps: [{
    ...
    requiredNodeProperties: { actorType: 'ascendant' },
    trayTier: 'self',
    motivations: [],
    narrativeTemplates: {
      initiation: 'withdraws divine attention, letting the world proceed without celestial touch',
      success: 'the stillness completes — essence pools in the quiet',
      failure: 'the stillness cannot hold; the god\'s attention is drawn back before it can settle',
    },
  },
  {
    id: 'divine.self.recede',
    name: 'Recede',
    spellName: 'The Tide Pulls Back',
    rarityTier: 1,
    intrinsicTier: 'background',
    description: 'The god gathers its power inward rather than expending it outward, preparing the next divine expenditure to cost less than it otherwise would. Like a tide that retreats only to strike harder on its return, the god\'s next intervention is made from a position of accumulated readiness.',
    reach: 'veil',
    crudType: 'update',
    scale: 'personal',
    steps: [{
      ...
      onSuccess: [{
        op: 'update_node',
        nodeId: '$target',
        changes: { nextActionDiscount: RECEDE_DISCOUNT_FRACTION },
      }],
      onFailure: [],
      failBehavior: 'fail_action',
    }],
    ...
    motivations: [],
    narrativeTemplates: {
      initiation: 'pulls back like a tide gathering for the next wave',
      success: 'readiness accumulates — the next divine act will cost less than it should',
      failure: 'the gathering dissipates before it can be held',
    },
  },
  {
    id: 'divine.self.focus',
    name: 'Focus',
    spellName: 'The Single Eye',
    rarityTier: 2,
    intrinsicTier: 'background',
    description: 'The god narrows its perception to a single point of intent, excluding all distraction until a crystalline clarity of purpose is achieved. The next divine action is performed from a state of absolute concentration — more likely to succeed, more likely to succeed well. The focus cannot be held for long; it must be used quickly.',
    reach: 'star',
    crudType: 'update',
    scale: 'personal',
    steps: [{
      ...
      onSuccess: [{
        op: 'update_node',
        nodeId: '$target',
        changes: { nextActionTierBoost: FOCUS_TIER_BOOST },
      }],
      onFailure: [],
      failBehavior: 'fail_action',
    }],
    ...
    motivations: [],
    narrativeTemplates: {
      initiation: 'narrows all divine attention to a single luminous point of intent',
      success: 'clarity achieved — the next act will be performed from perfect concentration',
      failure: 'the focus scatters before it can crystallise',
    },
  },
  {
    id: 'divine.self.reveal',
    name: 'Reveal',
    spellName: 'The True Face',
    rarityTier: 3,
    intrinsicTier: 'shaping',
    description: 'The god briefly strips away the veils of immanence and allows its true nature to shine through — not through a mortal vessel but as raw divine presence. Mortals within range cannot look away and cannot pretend they did not see. The moment passes, but those who witnessed it carry the impression forward: reverence or terror, or both at once.',
    reach: 'star',
    crudType: 'create',
    scale: 'personal',
    steps: [{
...
const LOCATION_ACTION_TEMPLATES: UnifiedActionTemplate[] = [
  ...
  {
    id: 'loc.reveal_vein',
    name: 'Reveal the Vein',
    spellName: 'The Buried Gleam',
    rarityTier: 2,
    intrinsicTier: 'background',
    description: 'Draws divine sight down through soil and stone until something worth digging answers back. A seam of ore, a bed of clay, a stand of timber the surveyors walked past for years — the land had it all along; it merely needed to be looked at properly.',
    reach: 'eye',
    crudType: 'update',
    scale: 'local',
    steps: [{
  ...
  {
    id: 'loc.guide_caravan',
    name: 'Guide the Caravan',
    spellName: 'The Kind Mile',
    rarityTier: 2,
    intrinsicTier: 'background',
    description: 'Walks unseen alongside the wagons of a settlement. Fords run shallow, axles hold, and the watchers in the rocks find reasons to be elsewhere. Every road that feeds this place runs fuller and safer for a season — and no drover will ever know why the miles were kind.',
    reach: 'eye',
    crudType: 'update',
    scale: 'local',
    steps: [{
      ...
      difficulty: 0.3,
      onSuccess: [
        { op: 'guide_caravan', nodeId: '$target' },
      ],
      onFailure: [],
      failBehavior: 'fail_action',
    }],
    ...
    narrativeTemplates: {
      initiation: 'lays divine attention along the roads like a hand smoothing a wrinkled map',
      success: 'every wagon comes in on time and nothing was lost to the road — the carters toast their luck, which has a name they do not know',
      failure: 'the roads shrug off the blessing; the miles stay exactly as long and as hungry as they were',
    },
  },

  {
    id: 'loc.sour_mine',
    name: 'Sour the Mine',
    spellName: 'The Pinched Seam',
    rarityTier: 2,
    intrinsicTier: 'background',
    description: 'Reaches into the diggings beneath a settlement and closes the hand of the land. Veins pinch to rubble, timber stands rot at the heart, the clay runs thin — the wealth beneath simply stops answering. Mortals will blame the surveyors, then the miners, then each other. The divine name will not come up.',
    reach: 'shadow',
    crudType: 'update',
    scale: 'local',
    steps: [{
    ...
    narrativeTemplates: {
      initiation: 'presses a thought into the stone beneath the workings, and the stone forgets what it held',
      success: 'the foreman calls the vein played out — twenty years early, and nobody can say why the mountain went barren',
      failure: 'the seam resists; the picks keep striking pay and the wagons keep filling',
    },
  },
...
const ATTACHMENT_ACTION_TEMPLATES: UnifiedActionTemplate[] = [
  ...
  },
  {
    ...
    technicalEffect: 'Appends the ascendant\'s primary-sphere positive passive (the canonical SPHERE_EFFECT_TABLE effect — RNG-free) to the artifact\'s effects[] and stamps attunedSphere. Persistent; consumed by the effect walker, so whoever holds the artifact gains the sphere-aligned reach bonus.',
    reach: 'heart',
    crudType: 'update',
    scale: 'local',
    steps: [{
  ...
  },
  {
    ...
    technicalEffect: 'Clears the artifact\'s effects[] array (every imbued/attuned/cursed power) and resets the attunedSphere / cursed / curseConcealed flags to inert. The inverse of imbue/attune/curse; the bearer loses all applied bonuses and curses via the effect walker. Fail-soft on an already-inert artifact.',
    reach: 'veil',
    crudType: 'delete',
    scale: 'local',
    steps: [{
  ...
  },
  {
    ...
    technicalEffect: 'Appends a concealed per-tick quintessence drain (CURSE_QUINTESSENCE_DRAIN, the inverse of the bestow regen boon) to the artifact\'s effects[] and sets cursed / curseConcealed. Consumed each tick by the effect walker for whoever holds it; the bearer is not told. Clamped at 0 quintessence, never negative.',
    reach: 'shadow',
    crudType: 'update',
    scale: 'local',
    steps: [{
...
const HEX_ACTION_TEMPLATES: UnifiedActionTemplate[] = [
  ...
  },
  {
    ...
    narrativeTemplates: {
      ...
      success: 'the land reveals its secrets — terrain, resources, faction presence, and the mood of the people become clear',
...
const RAW_UNIFIED_ACTION_TEMPLATES: UnifiedActionTemplate[] = [
  ...
  PAGE_BENEATH_THE_SAINT_TEMPLATE,
  VERDICT_THAT_BURNS_TEMPLATE,
 ...
 *
 * THR-74: `withGroupAffinity` then sweeps the ruins/delve, borderland-threat and
 * monster-combat families, adding `'group'` to `actorAffinities` on every
 * physical-challenge-shaped template. The spec is the predicate in
