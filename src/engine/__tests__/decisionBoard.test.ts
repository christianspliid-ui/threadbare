...
// ─── The anti-vacuity guard on the ambition *boost* ─────────────

describe('computeAmbitionCentralityBoost (guard: the term must discriminate on real data)', () => {
  /**
   * The same shipped-data walk the `ambitionPrefersVerb` guard above uses, carrying
   * the primary reach the board would actually score each pair on.
   *
   * A fixture is specifically disallowed here. The defect this term replaces
   * (THR-1302) was a predicate whose population is generated by the very thing it
   * tests, and a hand-built ambition is free to declare an affinity the shipped
   * corpus does not contain — which would verify a fiction on both sides.
   */
  const realPairs: ReadonlyArray<{ ambitionId: string; template: StrategicActionTemplate; reach: ReachDomain }> = (() => {
    const pairs: { ambitionId: string; template: StrategicActionTemplate; reach: ReachDomain }[] = [];
    for (const ambition of AMBITION_TEMPLATES) {
      const profile = ambition.strategicProfile;
      if (!profile) continue;
      for (const templateId of profile.templateIds) {
        const template = getAllStrategicTemplates().find(t => t.id === templateId);
        if (template) pairs.push({ ambitionId: ambition.id, template, reach: pickPrimaryReach(template) });
      }
    }
    return pairs;
  })();

  const boostOf = (p: { ambitionId: string; reach: ReachDomain }) =>
    computeAmbitionCentralityBoost(p.ambitionId, p.reach);

  it('has real pairs to judge (otherwise every assertion below is vacuous)', () => {
    expect(realPairs.length).toBeGreaterThan(10);
  });

  it('is not a constant across the shipped corpus — THE test this term owes', () => {
    // The term it replaced returned a flat `AMBITION_REACH_BOOST` on *every*
    // strategic candidate, because the board asked "does the agent pursue any
    // ambition with affinity for this reach?" of a population the ambition itself
    // generated. Measured: p25 = p50 = p75 on both seeds. Every other assertion in
    // this file passed against that constant. This one does not.
    const distinct = new Set(realPairs.map(p => boostOf(p).toFixed(6)));
    expect(distinct.size).toBeGreaterThan(2);
  });

  it('pays zero when the proposing ambition has no affinity for the reach', () => {
    // The overpaid case, and the reason the old term could not discriminate: an
    // ambition may *permit* a template whose primary reach it is not about. These
    // were all paid the full flat boost before.
    const zeros = realPairs.filter(p => boostOf(p) === 0);
    expect(zeros.length).toBeGreaterThan(0);
    expect(zeros.length).toBeLessThan(realPairs.length);
    for (const p of zeros) {
      expect(findAmbitionTemplate(p.ambitionId)?.reachAffinity?.[p.reach] ?? 0).toBe(0);
    }
  });

  it('pays exactly the ceiling on an ambition’s most-preferred reach', () => {
    // Normalising by the ambition's own maximum is what keeps the old flat value
    // reachable: a maximally-central undertaking is scored exactly as before, and
    // only peripheral ones are scored down. Raw affinities would cap at 0.4,
    // since no ambition authors a 1.0 — a level cut, not a spread.
    const ceilings = realPairs.filter(p => boostOf(p) === UNDERTAKING_AMBITION_CENTRALITY_BOOST);
    expect(ceilings.length).toBeGreaterThan(0);
    for (const p of realPairs) {
      expect(boostOf(p)).toBeLessThanOrEqual(UNDERTAKING_AMBITION_CENTRALITY_BOOST);
      expect(boostOf(p)).toBeGreaterThanOrEqual(0);
    }
  });

  it('does not double-count what computeTemperamentWeight already reads', () => {
    // This issue's second Done-when, as a falsifiable assertion rather than a
    // docblock claim. `computeTemperamentWeight` reads `preferredVerbs` membership
    // and the *template's* `reachProfile`; this reads the *ambition's*
    // `reachAffinity`. If those were the same signal wearing two names, the boost
    // could not move while the temperament weight stood still — so find real pairs
    // where it does exactly that.
    const byTemperament = new Map<string, Set<string>>();
    for (const p of realPairs) {
      const w = computeTemperamentWeight(p.template, p.reach, ambitionPrefersVerb(p.ambitionId, p.template))
        .toFixed(6);
      const seen = byTemperament.get(w) ?? new Set<string>();
      seen.add(boostOf(p).toFixed(6));
      byTemperament.set(w, seen);
    }
    const independent = [...byTemperament.values()].filter(boosts => boosts.size > 1);
    expect(independent.length).toBeGreaterThan(0);
  });

  it('is zero — never thrown — for an unknown ambition or an unaffiliated reach', () => {
    expect(computeAmbitionCentralityBoost('no_such_ambition', 'iron')).toBe(0);
    const withProfile = AMBITION_TEMPLATES.find(a => a.strategicProfile)!;
    const unloved = REACH_DOMAINS.find(d => (withProfile.reachAffinity?.[d] ?? 0) === 0);
    expect(unloved).toBeDefined();
    expect(computeAmbitionCentralityBoost(withProfile.id, unloved!)).toBe(0);
  });
});
