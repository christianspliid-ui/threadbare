...
describe('applyResourceDelta', () => {
  it('adds essence and clamps to non-negative', () => {
    ...
    const result = applyResourceDelta(effect, { essence: 5 }, 'agent-1', 'reward');
  ...
  it('subtracts essence and clamps to 0', () => {
    ...
    const result = applyResourceDelta(effect, { essence: 30 }, 'agent-1', 'encounter');
  ...
  it('adds quintessence and clamps to max', () => {
    ...
    const result = applyResourceDelta(effect, { quintessence: 90, quintessenceMax: 100 }, 'agent-1', 'reward');
  ...
  it('clamps amount to per-resource cap', () => {
    ...
    const result = applyResourceDelta(effect, { essence: 0 }, 'agent-1', 'reward');
  ...
  it('adds doom and clamps to doomThreshold', () => {
    ...
    const result = applyResourceDelta(effect, { doom: 80, doomThreshold: 100 }, 'agent-1', 'encounter');
  ...
  it('clamps doom delta to RESOURCE_DELTA_DOOM_CAP', () => {
    ...
    const result = applyResourceDelta(effect, { doom: 0, doomThreshold: 100 }, 'agent-1', 'encounter');
  ...
  it('records source in trace', () => {
    ...
    const result = applyResourceDelta(effect, { essence: 0 }, 'agent-1', 'action_trigger', 'attachment_123');
    expect(result.trace.source).toBe('action_trigger');
    expect(result.trace.sourceAttachmentId).toBe('attachment_123');
  });

  it('populates actorId in trace from agentId parameter', () => {
    const effect: ResourceDeltaEffect = { type: 'resource_delta', resource: 'essence', amount: 5 };
    const result = applyResourceDelta(effect, { essence: 0 }, 'my-agent-42', 'reward');
    expect(result.trace.actorId).toBe('my-agent-42');
  });
});
