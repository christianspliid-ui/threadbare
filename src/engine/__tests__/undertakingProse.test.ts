...
function world(): WorldGraph {
  ...
  g.addEdge({ id: 'mark', source: 'maerin', target: 'kael', type: 'knows_secret_of', properties: { magnitude: 0.5, revealed: false, secretType: 'affair', discoveredTick: 0, source: 'observed' } });
  return g;
}
...
describe('resolveUndertakingProse', () => {
  it('fills object, owner, actor and place from the world and names each as a concept', () => {
    ...
    const r = resolveUndertakingProse('{Actor} is unmaking {object} at {place}; {owner} will not thank them.', {
      graph: g, actorId: 'kael', objectTypeId: 'room', handle: { kind: 'node', nodeId: 'mill' },
    });
  ...
  it('describes an edge object by its ends and places it at its subject', () => {
    const g = world();
    expect(objectDisplayName(g, 'mark', { kind: 'edge', edgeId: 'mark' })).toBe('the hold on Kael Thornweaver');
    const r = resolveUndertakingProse('{Owner} holds {object} at {place}.', {
      graph: g, actorId: 'kael', objectTypeId: 'mark', handle: { kind: 'edge', edgeId: 'mark' },
    });
  ...
  it('picks lines deterministically per key, and the cell helpers resolve the set for a project', () => {
    const lines = UNDERTAKING_VERB_PROSE.undo.activity;
    ...
    const g = world();
    const project = { projectId: 'proj_a', actorId: 'kael', objectTypeId: 'room' as const, objectHandle: { kind: 'node' as const, nodeId: 'mill' }, targetNodeId: 'mill', victimAgentId: 'maerin' };
    const activity = cellActivityProse(g, project, 'undo');
    ...
    const completion = cellCompletionProse(g, project, 'undo');
