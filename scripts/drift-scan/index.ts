...
/**
 * Leading literal of every drift-scan issue title. Shared by the formatter and
 * the matcher below so the two halves of the title contract cannot drift apart.
 */
const DRIFT_ISSUE_TITLE_PREFIX = "Drift scan";

export function formatIssueTitle(runDate: string, signalName: string, summary: string): string {
  return `${DRIFT_ISSUE_TITLE_PREFIX} [${runDate}]: ${signalName} — ${summary}`;
}

/**
 * Recovers the signal name from a drift-scan issue title.
 *
 * The title embeds the run date, so exact-title matching can never recognise a
 * prior week's issue for the same signal — which is why every weekly run filed a
 * fresh duplicate (THR-756). The signal name is the stable part, and it is
 * delimited by the em dash that separates it from the run's summary. Signal
 * names use "→" rather than "—", so the first em dash is unambiguously the
 * name/summary boundary.
 *
 * Returns null for any title that is not in the generated format.
 */
const DRIFT_ISSUE_TITLE_PATTERN = new RegExp(
  // DRIFT_ISSUE_TITLE_PREFIX is a plain literal — no regex metacharacters to escape.
  `^${DRIFT_ISSUE_TITLE_PREFIX} \\[[^\\]]*\\]:\\s*(.+?)\\s+—(?:\\s|$)`,
);

export function driftIssueSignalName(title: string): string | null {
  const match = DRIFT_ISSUE_TITLE_PATTERN.exec(title);
  return match?.[1] ?? null;
}

/** True when `title` is this signal's issue, whatever run date it carries. */
export function isDriftIssueForSignal(title: string, signalName: string): boolean {
  const parsed = driftIssueSignalName(title);
  return parsed !== null && parsed.toLowerCase() === signalName.toLowerCase();
}
...
async function main(): Promise<void> {
  ...
  // One open-issue fetch per run, reused across every red signal below.
  //
  // Fail-soft (NFP #4): if the lookup fails we fall back to the pre-THR-756
  // exact-title check per signal. That can re-file a duplicate, which is the
  // recoverable failure; dropping the signal entirely is not.
  let openDriftIssues: OpenDriftIssue[] | null = null;
  try {
    openDriftIssues = await listOpenDriftIssues();
    console.log(`open drift-scan issues: ${openDriftIssues.length}`);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`open drift-issue lookup failed (${message}); falling back to exact-title dedup`);
  }
  ...
  for (const outcome of outcomes) {
    ...
    if (openDriftIssues) {
      const existing = openDriftIssues.find((issue) =>
        isDriftIssueForSignal(issue.title, outcome.name),
      );

      if (existing) {
        if (existing.title === title) {
          console.log(`${existing.identifier} already current for ${outcome.id}; no update needed`);
          continue;
        }

        await updateDriftIssue({ id: existing.id, title, body });
        await commentOnDriftIssue({
          id: existing.id,
          body: [
            `**Refreshed by the ${runDate} drift scan.**`,
            "",
            `Previous title: \`${existing.title}\``,
            `Current title: \`${title}\``,
            "",
            "The description above now carries the current run's data. This signal was already open, so the scan refreshed it in place rather than filing a second issue (THR-756).",
          ].join("\n"),
        });
        console.log(`refreshed ${existing.identifier} for ${outcome.id}: ${title}`);
        continue;
      }
    } else {
      const existingIdentifier = await findIssueByExactTitle(title);
      if (existingIdentifier) {
        console.log(
          `existing issue found (${existingIdentifier}) for ${outcome.id}; skipping duplicate creation`,
        );
        continue;
      }
    }
