...
export const LIVE_CELL_NOTES: Readonly<Partial<Record<WorldObjectKindId, Partial<Record<UndertakingVerbVariant, LiveCellNote>>>>> = {
  ...
  army: {
    ...
    'control:claim': { op: 'take_command (setCommander)', note: 'Taking command of an army nobody leads (THR-1438) — the same op as the company, with the faction added: the claimant must be a member of the army\'s own faction, and may not already command another host. The warlord\'s tier-one work.', readBy: 'The war layer: army movement, battle resolution, supply, and the `war` debug readout — all of which resolve an army\'s commander through `commanded_by`.' },
    'control:seize': { op: 'take_command (setCommander) + writeGrudge + applyReputationWithDelta', note: 'A coup (THR-1438) — the usurping fork one rank down, **resolved by the faction and the band, never by the blade**. From inside the army\'s faction only; from outside it would be a battle, and the war layer already has one. A winning band moves the command; any other band (including an absent one — a review lever with no pin takes the failure arm) writes the commander a `usurpation_failed` grudge and docks the claimant standing with the faction, doubled on a `critical_failure`. Nobody dies and nothing dissolves.', readBy: 'The same army `commanded_by` readers as the claim fork; the faction\'s `reputation_with` ladder, which the failure arm moves; and the motive gate, which reads the grudge the commander is left holding.' },
    observe: { op: 'record_intelligence + seedKnowsOf', note: 'Scouting an army (THR-1438). **Writes familiarity with where the army stands, not with the army** — which narrows the line recorded on THR-1397 and says so deliberately: `knows_of` is a knower → Location edge (`types/graph.ts`) and cannot point at an actor, so "familiarity with the army and its commander" was not a thing the schema could hold. Where a host stands is also the more useful fact — it is what a march or an encounter needs to know. The intelligence record names the army itself, typed `ARMY_SCOUT_INTELLIGENCE_TYPE`.', readBy: 'The familiarity half feeds the same readers every survey does (THR-1428 R1): encounter awareness and the sheet\'s *Knows the way to* row. The intelligence half is read back by the war readout, which lists each army\'s `scoutedBy` — the mortals who have had eyes on it.' },
  },
  network: {
  ...
  agreement: {
    create: { op: 'mint_leverage_mark', note: 'Digging up a secret: a mark on the mortal the work was done about. The mark class only — a favour is minted by use × Standing, never dug up. **And a meeting (THR-1519):** the cell carries an `appointmentPayoff` (`UNDERTAKING_CELL_APPOINTMENTS`), so on completion the digger also plants an *appointment* with the subject — place = where the subject stands when the work finishes, due `UNDERTAKING_APPOINTMENT_DELAY_TICKS` later, kept branch `#thieves_errand`, missed branch `#court_errand` — through slice 1\'s one planter, with the promise written as an `owes_favor` edge of the appointment shape. The first cell whose product is a promise to be somewhere.', readBy: 'The appointment layer (THR-1479): the decision phase reads the seed for slack and the lean; `evaluateEncounterSeeds` keeps or misses it at the due tick and writes the `appointment_kept` / `appointment_missed` Event; the `agent_kept_appointment` milestone condition (`graphConditions.ts`) counts the kept ones for `ambition_uncover_secrets`; the sheet lists the promise on its Agreements rows. (THR-1519.)' },
...
export const CELL_DISPOSITIONS: Readonly<Partial<Record<WorldObjectKindId, Partial<Record<UndertakingVerbVariant, CellDisposition>>>>> = {
  ...
  faction: {
    ...
    // control:claim and control:seize shipped with THR-1438 — see LIVE_CELL_NOTES.
  },
  company: {
  ...
  army: {
    ...
    // control:claim, control:seize and observe shipped with THR-1438 — see LIVE_CELL_NOTES.
  },
  network: {
