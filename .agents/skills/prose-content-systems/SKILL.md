...
**Wounds and conditions in `UnifiedActionTemplate` aftermath (THR-117):** Use `{ kind: 'condition_attachment', templateId: 'trait.condition.wounded' }` to apply a wound (or any condition — disease, curse, blessing, bestowed) from an aftermath reaction. Do NOT use `content_grant` (not an aftermath effect kind) or `appliesWound` (`EncounterTemplate`-only, legacy). The `condition_attachment` kind auto-looks up default duration, triggers mid-encounter tier promotion when the wound condition is applied to the actor (making combat failures visible in the chronicle), and feeds into the overflow/incapacitation pipeline automatically. See the "Conditions and wounds" subsection of the wiring guide for constants and subcategory list.
