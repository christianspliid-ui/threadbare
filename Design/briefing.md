# Briefing
**Generated:** 2026-09-26 15:58 local (13:58 UTC) · keep-work-flowing-cc
...
*Correction:* earlier briefs also listed [how much history a new world starts with](https://linear.app/threadbare/issue/THR-1591) as yours. You already answered it on 25 September ("a is fine"), so it is off your list.
...
**Healthy: 11 jobs are ready, and 1 is being built.**

- **The dice change is being built** ([THR-1581](https://linear.app/threadbare/issue/THR-1581), High). A builder picked it up again at 15:12 local, working from [its branch](https://github.com/christianspliid-ui/threadbare/tree/thr-1581-dice-refit).
...
- **The long simulation tests have failed on the last three runs of `main`** ([latest failing runs](https://github.com/christianspliid-ui/threadbare/actions/workflows/heavy-tests.yml)). This is still the follow-up owed after the trade-route fix ([#2069](https://github.com/christianspliid-ui/threadbare/pull/2069)): two checks in `yieldBandCells.test.ts` fail. No builder has claimed it yet. The live game is unaffected, and the required checks are green.
...
- **Worktree reaper:** last ran 15:40 local. 5 worktrees are waiting to be sorted, which is routine.
- **Everything else is green:**
  - The live site is up to date (the last commits were docs only).
  - Game speed is 61 ms per tick, 17% under the weekly median.
