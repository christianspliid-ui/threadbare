# Briefing
**Generated:** 2026-08-29 08:56 local (06:56 UTC) · keep-work-flowing-cc
...
Unchanged since last hour except where noted. Detail in [user-actions.md](https://github.com/christianspliid-ui/threadbare/blob/ops/Design/user-actions.md).

- **More design hours — still the constraint.** [Card-grammar unification](https://linear.app/threadbare/issue/THR-1002/unify-the-card-grammar-action-cards-adopt-the-encounter-card) (10 days) and [traits wave 2](https://linear.app/threadbare/issue/THR-790/traits-wave-2-locations-artifacts-and-draw-by-trait-pools) (14 days) hold the one design slot. **Sit one, or say "park it."** *New in the design queue this morning: [can background characters build things off-screen?](https://linear.app/threadbare/issue/THR-1348/ambitions-held-below-the-spotlight-tier-have-no-agency-path-10-of-the)* — ten strategic pursuits, the whole merchant-expansion family among them, are held only by characters below the attention threshold, who have no way to act on them. Filed as a design call rather than answered by an agent. Nothing is blocked on it.
...
- **The shelf is the same seven as an hour ago**, headed by [the board-cutover blocker](https://linear.app/threadbare/issue/THR-1349/the-decision-board-has-no-variety-term-a-live-board-writes-zero-trade) filed at breakfast, plus [the prose doctrine sweep](https://linear.app/threadbare/issue/THR-1324/prose-doctrine-v2-remediation-sweep-10-operative-surfaces-still-teach) and five small engine and content clean-ups found by builders while building. Still no encounter writing on it — which is what the ask above changes. Nothing is stale; the oldest item was filed two days ago.
- **Two new jobs started themselves at 08:17** — round 3 of the agent-handbook cleanup, this time [the UI layer](https://linear.app/threadbare/issue/THR-1350/context-cleanup-round-3-the-uiux-layer-audit-sweep-per-the-rounds) and [deleting a dead component](https://linear.app/threadbare/issue/THR-1351/delete-dead-agentwheeltsx-component-and-its-orphan-test-rejected) left behind by a design you replaced long ago. Neither changes the game; they change what agents read before they build.
- **In Dev: 7 — four live, three parked.** The four live are the two round-3 jobs above plus [the world-generator ordering fix](https://linear.app/threadbare/issue/THR-1344/the-genomes-reach-pass-is-dead-at-worldgen-worldseed-runs-the-genome) and [the run-founded faction bug](https://linear.app/threadbare/issue/THR-1322/a-run-founded-faction-renders-as-a-fallback-everywhere-in-the-ui), both written and sitting in review (named under Health). Four at once is above the one-at-a-time rule; two of them are finished work waiting on a check, so it reads as a queue at the door rather than four half-built things. Executor's business, not yours. The three parked are your standing asks ([THR-1130](https://linear.app/threadbare/issue/THR-1130/encounter-factory-pilot-volume-retrofit-the-15-nudge-era-encounters-to), [THR-1133](https://linear.app/threadbare/issue/THR-1133/attended-pixel-pass-sweep-five-owed-19201080-captures-one-dev-server), [THR-1168](https://linear.app/threadbare/issue/THR-1168/two-authored-encounter-audio-moments-have-no-live-caller-wire-or)); park shape verified intact this run.

## Health

**Green, with the same two PRs needing a session. Nothing here needs you.**

- **The conflicts on both PRs are gone — the tests are not.** [#1714](https://github.com/christianspliid-ui/threadbare/pull/1714) (the run-founded faction fix, ~8.5h) and [#1717](https://github.com/christianspliid-ui/threadbare/pull/1717) (the world-generator ordering fix, ~5.5h) were both stuck on merge conflicts an hour ago; those are resolved, and each now fails the one required check instead. Auto-merge stays armed and correctly refuses to fire. A session must read the failure and push a fix — that is the executor's job, and I am telling you only because you were told about these two an hour ago in a different shape.
- Everything else green: the live site serves the latest game build ([3be439a6](https://github.com/christianspliid-ui/threadbare/commit/3be439a6eaf0a82a902f2032a368c52a9499eec1) — the two commits since it touched only notes, so nothing needed rebuilding), background jobs healthy, all nine scheduled lanes on schedule, the branch reaper ran at 08:40, and the home copy of the repo is clean and current.
