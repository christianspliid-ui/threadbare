# Briefing
**Generated:** 2026-09-09 00:54 local (22:54 UTC) · keep-work-flowing-cc
...
**The shelf is now empty — not thin, empty.** The last item on it, [a gate flip](https://linear.app/threadbare/issue/THR-1256), was picked up and finished at 00:28 tonight. Nothing is being built, and nothing is waiting to be picked up. This is the first hour the board has had zero on both counts.

Six days of waiting sit behind this one sentence, and it is the smallest ask on the whole list. Approving it also releases [the card-name tightening](https://linear.app/threadbare/issue/THR-1255), a fully-specified job that has been queued invisibly behind it for two weeks.

Two yes/no questions inside it, if you want to answer them at the same time — the six camp encounters were written in July under the old prose doctrine, so: **repair them in place, or re-roll them from fresh premises?** (repair is the plan; two lanes recommend it independently). And the 2-of-6 you sample: `ward_the_camp` and `tend_to_wounds`.

## Also waiting (12)

- [**Rule on the backlog**](https://linear.app/threadbare/issue/THR-1195) — the larger version of the above, and the reason the queue emptied. **Forty-four backlog tickets, none blocked on effort** — two independent censuses tonight (the `Todo` column, then the `Idea` column) both found the same thing. Roughly thirteen are one- or two-sentence rulings that each release buildable work: what [a Divine Herald](https://linear.app/threadbare/issue/THR-1195) is, which Spheres [shadow and void](https://linear.app/threadbare/issue/THR-1114) belong to, whether [a toll](https://linear.app/threadbare/issue/THR-1189) moves wealth or gets deleted, whether [the pressure system](https://linear.app/threadbare/issue/THR-1318) is switched on or retired. Say **"rule on the backlog"** and they come to you in one sitting, in game terms, smallest first.
...
- [**Image credits — gated on you at all?**](https://linear.app/threadbare/issue/THR-876/regenerate-the-5-quarantined-meet-the-first-scene-assets-doctrine) — five scene images break the art rule; substitutes cover the slots. The real question is whether image spends need your yes, or get decided by the lane and reported after.
...
**Zero in flight, zero on the shelf.** First hour with nothing on either count.

- [THR-1256](https://linear.app/threadbare/issue/THR-1256) was the last item on the shelf. A session picked it up at 00:01 and closed it at 00:28 — [PR #1861](https://github.com/christianspliid-ui/threadbare/pull/1861); the verdict was to retire the guidance gate rather than flip it, because the trigger was wrong, not the lists. Nothing replaced it.
- Both In-Dev items are parked with nobody on them. [THR-1130](https://linear.app/threadbare/issue/THR-1130) is the camp-six ask above (parked 6 days). [THR-1392](https://linear.app/threadbare/issue/THR-1392) is shipped and awaiting a close — a lane's job, not yours; it closes as superseded unless you object.
- No agent can refill the shelf on its own, which is why this brief is one sentence long at the top.

## Health

- **"Heavy simulation tests" is red on the latest main** (~4 h). Post-merge only — the required CI check is green and the site is serving `a87e8f24`. A session owes the follow-up; not your call.
...
- Deploy, CI dispatch, merge queue, scheduled-task heartbeats and the worktree reaper: all green. Engine tick cost is 64 ms/tick, **27% below** the 7-day median.
