# Briefing
**Generated:** 2026-09-17 20:58 local (18:58 UTC) · keep-work-flowing-cc
...
- **[THR-1448](https://linear.app/threadbare/issue/THR-1448/a-held-town-is-a-faction-position-holding-opens-faction-encounters-and): say "design THR-1448" in a chat.** No decision is owed — the direction is already yours. The build queue is now completely empty, and a design session is the only thing that refills it. [THR-1479](https://linear.app/threadbare/issue/THR-1479/appointment-primitive-a-mortal-keeps-or-misses-a-meeting-at-a-place-by) is next in line. — from tb-orchestrator
- **Were this week's two stops deliberate?** The machine was off or asleep Sun 13th 21:00 → Wed 16th 15:55, and again today 07:40 → 17:55. Nothing broke; if that was you, nothing to do. Detail in [user-actions.md](https://github.com/christianspliid-ui/threadbare/blob/ops/Design/user-actions.md).
...
**Starved: nothing ready, nothing being built** — 0 waiting, 0 in progress, for the first time this cycle. The last item, the redrawn Meet The First scenes ([THR-876](https://linear.app/threadbare/issue/THR-876/regenerate-the-5-quarantined-meet-the-first-scene-assets-doctrine)), merged at 18:22 yesterday evening. The builder idles until a design session produces work.

- **29 items sit in the backlog and none can be promoted** — each one needs a design decision first, not a builder. The orchestrator re-read the three likeliest by hand this run and confirmed all three are design calls. Not your call: an agent picks them up once a session is running. — from tb-orchestrator
...
- **Everything else is green.** All 9 lanes are on schedule. Engine speed is 71 ms/tick, 4% above the 7-day median of 68 — normal range. The live site is serving the newest commit ([049dac50](https://github.com/christianspliid-ui/threadbare/commit/049dac50)). CI, GitHub's scheduled jobs and the auto-close workflow are all green, no PRs are waiting to merge, and the cleanup script ran 17 minutes ago.
