...
**Generated:** 2026-08-12 03:55 local (2026-08-12 01:55 UTC) · keep-work-flowing-cc

## The one thing

**Did you switch the two work lanes off?** [`tb-opus-pickup`](https://linear.app/threadbare) (the executor — picks up and ships tickets) and `tb-orchestrator` (decides what gets promoted next) are both **disabled**, not merely quiet. Between them they have now skipped **eleven slots across six hours**: pickup last ran 22:01 and has missed 23:01 / 00:01 / 01:01 / 02:01 / 03:01; the orchestrator last ran 21:26 and has missed 22:25 / 23:25 / 00:25 / 01:25 / 02:25 / 03:25.

**Nothing on the board moves while the executor is off** — the queue has sat at 22 ready / 0 in flight all night, including the work your other rulings below would create.
...
**Backed up — 22 ready, 0 in flight.** Every one of the 22 is cleanup: infrastructure, deferrals, prose/UI tidying. **Zero feature or content work on the shelf.** THR-907 / THR-974 above are the only asks that refill it with real work.
...
- **Home tree's `node_modules` is damaged** — `.bin` has no esbuild and no vitest. The reaper has flagged it on every run since 00:40 and will not auto-install. Repair is `npm install` in the home tree; an executor session's job, not yours.
...
- Deploy, CI checks, scheduled workflows, task heartbeats and the reaper: all green. The live site is serving the latest commit on main (`eabbc173`).
