# Briefing
**Generated:** 2026-09-25 14:56 local (12:56 UTC) · keep-work-flowing-cc
...
**Backed up: 25 jobs are ready and none are in progress. None are parked.** Most are bug tickets from this morning's research, not a stall. The builder picks up hourly; its next run is about 15:11 local.

- Merged since the last brief: **Mercenary Company encounters can now be drawn** ([THR-1612](https://linear.app/threadbare/issue/THR-1612), [#2045](https://github.com/christianspliid-ui/threadbare/pull/2045)). It is live on the site.
- **The living-world map has no research left** (no ask — *from tb-orchestrator*). The last answer, [write where the dice land](https://linear.app/threadbare/issue/THR-1598), says: first fill the missing "success, but at a cost" text and card hands in the ten most-drawn encounters, then the ones your First meets, then the next ten. Decided for you, veto welcome: filling gaps in existing encounters skips your "sample 2 of 6" check, and a reviewer agent checks it instead. Brand-new encounters still get your sample. The map's three remaining questions are design calls for the design lane, and none is marked as yours.
- Still High: [social, tavern and secret encounters never fire](https://linear.app/threadbare/issue/THR-1614), [six seed families wither on planting](https://linear.app/threadbare/issue/THR-1613), [the cold playtest loop](https://linear.app/threadbare/issue/THR-1610).
...
- **Heavy simulation tests are still red on the latest `main`** (about 5 hours). The next builder owes a fix. Not yours.
- **Tick cost drift** (the builder's job, not yours): tick cost 92 ms/tick steady, 35% above the 7-day median (68, 104 rows since 049dac50); top phase agent_decision, 494 agents. Name the merges between 049dac50 and 67c4bdf7: `git log --oneline --merges 049dac50..67c4bdf7`. The previous two readings were 77 ms, so the jump arrived with [#2045](https://github.com/christianspliid-ui/threadbare/pull/2045) or is one noisy sample. The next hour's reading will tell.
...
- **Worktree reaper:** 5 worktrees are waiting to be sorted (last run 14:40 local). That is routine.
- **Everything else is green.** The live site is serving the latest build (67c4bdf7). CI is green, and no pull requests are waiting. All ten scheduled lanes are on time. The design lane ran at 14:18 local and left no report, so it had nothing to decide. The home tree is on `main` and current.
