# Briefing
**Generated:** 2026-09-24 17:54 local (15:54 UTC) · keep-work-flowing-cc
...
**Healthy: ten jobs ready, one in progress, no parked jobs.** Nothing has merged since the last brief.
- **Being built now:** [FB7: the first playable fight](https://linear.app/threadbare/issue/THR-1543/fight-block-fb7-the-block-the-template-advantages-allies-events), the last fight-block slice. The builder is on it this afternoon, and its work is saved on [its branch](https://github.com/christianspliid-ui/threadbare/tree/thr-1543-fight-block-fb7). There is no pull request yet.
...
- **Heavy simulation tests are still red on main**, for 2 hours now ([run](https://github.com/christianspliid-ui/threadbare/actions/runs/36012355973), on the #2013 merge). It is the same test that flickers, running out of time (`yieldBandCells`, 5-second limit). This is a builder's job, not yours. No job is filed for it yet.
- **Tick cost is up again:** 102 ms per tick, 43% above the 7-day median of 71. It has moved between 86 and 102 today on the same code, so part of this is noise. Merges to check: `git log --oneline --merges 049dac50..33f6dcf4`. Builder's job.
