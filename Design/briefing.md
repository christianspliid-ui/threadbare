# Briefing
**Generated:** 2026-09-26 21:57 local (19:57 UTC) · keep-work-flowing-cc
...
- [Culture and spheres showing through](https://linear.app/threadbare/issue/THR-1599): culture and a place's sphere show in an encounter as **one extra sentence stating a local fact that bears on the test**. Samples: [seed 42](https://github.com/christianspliid-ui/threadbare/blob/proto/thr-1599-culture-sphere-layer/Docs/audits/2026-09-25-living-world-data/proto-thr-1599/sample-seed-42.md) · [seed 99](https://github.com/christianspliid-ui/threadbare/blob/proto/thr-1599-culture-sphere-layer/Docs/audits/2026-09-25-living-world-data/proto-thr-1599/sample-seed-99.md). Its 24 hours are up; this is its last hour on the list. *— from tb-design-lane*

Say "veto <title>" to reverse any of these.

## Queue

**Healthy: 9 jobs are ready, none being built right now.** The builder lane picks the next one up at its next hourly run.

- **Shipped this hour:** [a hex's lore says "not fully explored" once, not once per ruin](https://linear.app/threadbare/issue/THR-1621), via [#2079](https://github.com/christianspliid-ui/threadbare/pull/2079). It is live on the site.
...
- **The long simulation tests are red again** on the latest `main` ([run](https://github.com/christianspliid-ui/threadbare/actions/runs/36266108837)). Three of the last four runs failed, all in the same test file (who inherits a company or army when its commander dies), while this hour's change was a small wording fix that does not touch it. It flips between red and green on unrelated changes, so it is an unsteady test, not a broken game. It is already in the impediment log; fixing it is builder work, not yours.
...
- **Everything else is green:**
  - The live site is serving the latest `main` (with #2079).
  - Game speed is 79 ms per tick, 6% above the weekly median. That is within the normal range.
  - All ten scheduled lanes are on time.
  - The worktree reaper last ran at 21:40 local. 5 worktrees are waiting to be sorted, which is routine.
