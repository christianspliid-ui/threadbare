...
**Last updated:** 2026-07-21 (item 2 added by the THR-653 Cowork cutover; light refresh at 01:54 local by the hourly `keep-work-flowing-cc` CC task; last full rebuild was the 2026-06-23 retro)
...
**A second, distinct blocker surfaced at the 01:54 run — different fault, same symptom.** The tree is on `main` but **24 behind**, and the autosync has been skipping hourly since 21:56 with `you have uncommitted changes that would be overwritten`. The blocking files are **`Design/briefing.md` and `Design/user-actions.md`** — written into the home tree by *this task* before the THR-672 rule existed.
...
**Coverage gap worth a ticket:** THR-672's reattach guard fires only on the *detached* case. This is the *dirty-tracked-file* case, which still hard-blocks (`REATTACH_MAX_TRACKED_DIRTY` = 0) and will not self-heal. Extending the same self-heal to "the only dirty tracked files are this task's own outputs, and they match a committed blob" would close it. **Not filed** — the hourly PM task does not create work; surfaced here and in the briefing for a design session to pick up.

**Now self-limiting regardless:** from the 01:54 run onward this task writes both files in its own worktree and never in the home tree, so the residue stops regenerating. Clearing it is a one-time `git checkout -- Design/briefing.md Design/user-actions.md && git pull --ff-only`.
