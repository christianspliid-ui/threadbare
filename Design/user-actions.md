...
**Last updated:** 2026-07-21 (refresh at 20:54 local by the hourly `keep-work-flowing-cc` CC task — **item 2 changed shape: the home tree is now parked off-branch *as well as* carrying the leftover file, so the fix is three commands rather than one.** Nothing is stranded on it (0 unique commits, re-verified). The behind-count moved 41 → 5 because the reference point shifted under the tree, not because anything recovered — do not read it as progress. Item 1 re-verified against the live scheduler, unchanged: all three replacements present and `enabled: false`. Item 3 holds at three ready — THR-690 shipped 20:32, THR-693 filed 20:19 to replace it — which makes "the queue feeds itself on its own maintenance" a measured pattern rather than a warning. Item 4 unchanged at 14 open / 2 conflicting. Nothing pruned. The 19:54 refresh corrected the conflict over-count; the 17:54 refresh recorded the three replacements landing; the 14:54 refresh flagged the ticket bar. Last full rebuild was the 2026-06-23 retro)
...
**Status:** Open, **ninth consecutive run** (12:54 → … → 20:54) · **escalated in kind, not in risk.** As of 20:54 the tree is **parked off-branch in addition to being dirty** — the second half of the known harness pair (a bare `checkout HEAD` detaches the tree; a plumbing update moves `refs/heads/main` under it). Still loss-free, now needs three commands rather than one.
**Source:** Home-tree freshness ping at the 20:54 run — **detached HEAD** (was on `main` through 19:54), **`main` 5 behind `origin/main`**, **1 tracked modification** (`Design/user-actions.md`), 0 untracked, **nothing stranded (`origin/main..HEAD` = 0)**. Supersession re-verified this run, not assumed: the on-disk copy is still the 10:54 text plus a 3-line edit whose substance `origin/main` already carries; taking the published version is a net +62 lines of newer content.
...
```
The first discards the superseded file, the second re-attaches the tree to the main line, the third catches it up. All three are loss-free given the verified state above (`origin/main..HEAD` = 0 means no commit exists only on that tree). `--ff-only` is deliberate: it refuses rather than merges if the situation is not what was measured.
...
**Ticket bar reached (14:54) and still unfiled at 20:54, a seventh consecutive hour — this is agent work, not yours.** The fix is narrow and safe: extend THR-672's self-heal guard so it may discard a tracked modification whose content is **provably already on `origin/main`** (verifiable by diff, exactly as re-verified this run), rather than requiring a strictly clean tree. Filing it is outside this task's remit — it writes only these two files — so an executor or design session should pick it up. **The nearest ticket remains THR-678, which covers the *untracked*-file variant and sits in Idea; nothing covers the tracked-modification case.** **That an obviously-correct, seven-times-flagged fix has gone seven hours without a ticket is itself the finding**: this task can name agent-side work but cannot queue it, and nothing else is reading these notes. **The scope should now widen slightly** — tonight's park proves the guard must handle *dirty **and** detached* together, which is the combination it currently refuses outright. Escalation that would actually work: one sentence in a chat session — *"file the autosync self-heal ticket"*. **It is a standing item for the Friday retro** regardless. **Your one command still clears today's instance immediately.**

---

## 3. Name the next game-facing stretch · CHRISTIAN ACTION · WILL NOT SELF-HEAL

**Status:** Open, carried from 11:54. Its original premise ("the queue is empty and the executor idles") is long gone. **The 20:54 run turns the warning into a measurement:** the queue did not shrink this hour — THR-690 shipped at 20:32 and THR-693 was filed at 20:19 to replace it. The lane is not draining toward a decision point; **it is regenerating its own maintenance backlog at roughly the rate it clears it.** Waiting for the queue to empty will not produce the moment to choose.
**Source:** Board scan at the 20:54 `keep-work-flowing-cc` run: **3 Ready for Dev** (all unassigned, held at 3 across the hour), **1 In Dev** (THR-677, unassigned by design), 0 blocked, 0 stale — all three filed today, none `blockedBy` another.

**The three ready items, by what they actually do:** a union merge driver so idle PRs stop rotting (THR-691); a plan-doc lint scope fix (THR-692); and a newly-found gap where CI's type-check step runs a command that passes unconditionally (THR-693, filed 20:19 out of THR-690's closeout). The build-artifact freshness gate (THR-690) shipped out of this set at 20:32. **In development:** the Cowork prompt port (THR-677) — parked on your trial approvals, see item 1.
