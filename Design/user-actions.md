...
**Last updated:** 2026-08-15 08:56 local (2026-08-15 06:56 UTC). Standing asks only, per [THR-1077](https://linear.app/threadbare/issue/THR-1077). Run measurements, findings and narration live in the history: `git log -p origin/ops -- Design/user-actions.md`.
...
It is now the choke point on the feature pipeline, not just a stale ticket. As of this morning the last non-cleanup item on the shelf has shipped ([THR-1089](https://linear.app/threadbare/issue/THR-1089), [PR #1467](https://github.com/christianspliid-ui/threadbare/pull/1467)), leaving three Ready-for-Dev items that are all Low-priority `Deferral` workshop maintenance. The design lane stages one thing at a time, so nothing player-facing can be queued behind THR-1043 while it sits.
...
The working copy at `C:\Users\chris\Dev\Projects\TheFantasyWorldSimulator` last synced on 2026-08-12 and is now **27 commits behind, growing every hour** (25 an hour ago; 23 consecutive hourly attempts have now failed). Three locally-modified files block the automatic fast-forward, and per [THR-937](https://linear.app/threadbare/issue/THR-937) this shape never resumes on its own. No automated lane may run git operations there ([THR-672](https://linear.app/threadbare/issue/THR-672)), so it needs you or an attended session.
...
It has started costing lane time rather than just drifting: this morning's architecture-health sweep ran there and reported two documentation pages stale that were not — a false positive the orchestrator had to detect and re-run from a fresh worktree to clear.

Nothing is destroyed: `git stash push` keeps all three edits and `git stash show -p` reads them back. Two are local permission-allowlist entries in `.claude/settings*.json`; the third is an impediment row that needs re-filing under a fresh id afterwards — an agent's job, not yours. Nothing in the game is affected — worktrees fetch `main` directly.
