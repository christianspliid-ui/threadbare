...
**Last updated:** 2026-09-22 12:00 local (10:00 UTC). Standing asks only, per [THR-1077](https://linear.app/threadbare/issue/THR-1077). Board read live this run.
...
The lane that builds has failed twice in a row with *"You've reached your Fable limit."* — 10:11 local (after sixteen minutes, having just claimed a job) and 11:11 local (after seven seconds). Its next attempt is hourly and will keep failing the same way until the quota clears.
...
**Today's credit failure is the first hard evidence for the explanation you gave in August** — a usage limit stops the lanes and they resume when it clears. That fits every episode better than a machine fault, and matches the corroborating signs: the lanes fired together in one catch-up burst, and GitHub's own jobs stayed green because they don't run here.
