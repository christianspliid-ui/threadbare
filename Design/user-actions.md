...
**Note:** the branch conflicts with `main` in `src/engine/aftermathWords.ts` and its test — ordinary executor merge work, nothing from you. Nothing merged since 12 August touches the consequence chips, so the preview still shows what you need to see.

**Cost of waiting:** [THR-1096](https://linear.app/threadbare/issue/THR-1096/companion-attachments-a-person-in-the-retinue-granting-bonuses-who-is) (companion attachments) and [THR-1097](https://linear.app/threadbare/issue/THR-1097/consequence-content-pass-every-vertical-slice-ending-rewritten-as) (consequence content pass) are both High, both blocked behind it, and are the only feature work on the board.
