...
**Last updated:** 2026-08-25 14:05 local (12:05 UTC). Standing asks only, per [THR-1077](https://linear.app/threadbare/issue/THR-1077). Run measurements, findings and narration live in the history: `git log -p origin/ops -- Design/user-actions.md`.

## Standing asks

### 1. Who writes the prose rewrite? — [THR-1223](https://linear.app/threadbare/issue/THR-1223/rewrite-the-shipped-nudge-corpus-to-prose-doctrine-v2-narrator-mode)

You ordered every shipped encounter rewritten to the new narrator voice. The ticket says **"Fable — prose authoring."** An agent claimed it, read that, and stopped without writing a word — your standing instruction is that Fable writes prose, because Opus prose has twice passed every automatic check and still failed your read. Nothing is half-done; a resuming session starts clean.

**"Fable writes it"** → it waits for a Fable session you start; the scope sheet is on the ticket (*The Unclaimed Relic* first — it is the calibration case, so it is the cheapest read for confirming the register landed). **"Opus can write it"** → an agent starts on the next hourly pickup.

**This is the whole board.** Zero claimable items on the shelf. [The tooling](https://linear.app/threadbare/issue/THR-1224) is mutex'd behind it, [its split half](https://linear.app/threadbare/issue/THR-1225) natively blocked by it, batch 2 behind it, [your slice checkpoint](https://linear.app/threadbare/issue/THR-1220) behind all of them. And no gate can substitute for your read: `check:encounter --all` passes today on the un-rewritten corpus, including the template you called borderline unreadable.
...
It keeps growing as more UI work ships behind the same wall. This replaces four separate tickets (THR-1109, THR-1125, THR-1126, THR-1127), consolidated 2026-08-16 — a merge, not a prune. If you only get through part of it, say which, and the remainder is re-expanded rather than closed whole. Bundling it into whichever sitting you approve batch 2 in costs almost nothing extra.

### 6. More design hours — four sittings stacked

**Besides ask 1, this is the only thing that can put parallel work back on the board** — every other startable item is either blocked behind the rewrite or waiting on you. In the order you set:
...
Both design maps closed 2026-08-24: [the typed game-state map](https://linear.app/threadbare/issue/THR-1157) and [the vertical-slice map](https://linear.app/threadbare/issue/THR-902). **Nothing is being charted now.** The slice map's charter names its successor: **the hub — the player reaching factions, war, economy and divine actions from inside the encounter interface.**
...
- 2026-08-25: **the two encounters sent for your verdict are withdrawn** — both live in the file the corpus rewrite replaces wholesale, so judging them now would be judging condemned prose. The ask returns once, at [the integrated checkpoint](https://linear.app/threadbare/issue/THR-1220), in the new voice. The re-post was a briefing fault and is logged.
- 2026-08-25: **the batch-1 sample verdict is retired** — [THR-1130](https://linear.app/threadbare/issue/THR-1130) led this page for three briefs asking whether two encounters were worth meeting twice. You answered it by playing one and calling it borderline unreadable, then rewriting the prose rules.
- 2026-08-25: **you read the new encounters and ruled** — [THR-1221](https://linear.app/threadbare/issue/THR-1221) closed; your ruling became [Prose Doctrine v2](https://github.com/christianspliid-ui/threadbare/blob/main/.claude/skills/encounter-pipeline/reference/nudge-authoring-spec.md) ([PR #1606](https://github.com/christianspliid-ui/threadbare/pull/1606)) and two tickets, [THR-1223](https://linear.app/threadbare/issue/THR-1223) and [THR-1224](https://linear.app/threadbare/issue/THR-1224).
- 2026-08-25: **three faction pairs stopped flying the same banner** — [THR-854](https://linear.app/threadbare/issue/THR-854) shipped at 08:33, promoted on agent judgement an hour earlier under a veto nobody exercised.
...
- 2026-08-24: **four shelved tickets re-read against your 12 August rule, three came off your page without you** — [THR-831](https://linear.app/threadbare/issue/THR-831) (shipped the same hour), [THR-1095](https://linear.app/threadbare/issue/THR-1095), [THR-977](https://linear.app/threadbare/issue/THR-977); [THR-1198](https://linear.app/threadbare/issue/THR-1198) stayed yours. **Say the word if any should have come to you.**
- 2026-08-24: **you approved the first encounter batch** — *"Go agenda"* / *"Ahead"* at 18:08, the last gate in [the Encounter Factory](https://linear.app/threadbare/issue/THR-1043).
