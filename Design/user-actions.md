...
**Last updated:** 2026-07-21 (refresh at 17:54 local by the hourly `keep-work-flowing-cc` CC task — **item 1 materially advanced: all three Cowork replacements are now written, registered and deliberately disabled, so the ask has moved from "wait for the port" to "watch each run once and approve it."** Item 2 carried a **sixth** consecutive hour at **31 behind**, up from 22; ticket still unfiled for a fourth hour. Item 3 narrowed to five ready — THR-687 shipped at 15:41. Item 4 re-counted live, unchanged at 14. Nothing pruned. The 16:54 and 15:54 refreshes narrowed item 3's ready-list; the 14:54 refresh flagged the ticket bar; the 13:54 refresh escalated item 2; the 11:54 refresh pruned the upstream-report consent and narrowed item 1 to a single switch. Last full rebuild was the 2026-06-23 retro)
...
**Status:** DECIDED 2026-07-21 in chat · **two actions now, not one.** (a) disable `keep-work-flowing` in the Cowork app — unchanged, safe today; (b) **new at 17:54: the three replacements are built.** THR-677 landed them at 15:28 — all three written, registered, and **deliberately left `enabled: false`** — so the remaining step is the trial you asked for: run each once attended, approve its output in chat, then it goes live and its Cowork twin is switched off. Three of THR-677's four Done-whens are met; the fourth *is* this trial gate, and it is yours
...
**Status:** Open, **sixth consecutive run** (12:54 → 13:54 → 14:54 → 15:54 → 16:54 → 17:54) · **drift 4 → 14 → 18 → 20 → 22 → 31 behind.** The widening is measured across six hours rather than inferred, and the last step is the largest. Still small and mechanical, still will not clear itself.
**Source:** Home-tree freshness ping at the 17:54 run — on `main`, **31 behind** (22 at 16:54, 20 at 15:54, 18 at 14:54, 14 at 13:54, 4 at 12:54), **1 tracked modification** (`Design/user-actions.md`), 0 untracked, nothing stranded (`origin/main..HEAD` = 0). Supersession re-verified this run, not assumed: the working copy is a 3-insertion / 3-deletion delta against a base that `origin/main` moved past hours ago — it is still the 10:54 text, and this refresh republishes a strictly newer version of the same file.
...
**What breaks if not done.** Nothing is lost or at risk. The home tree simply stops getting newer, so a morning session there starts on progressively staler state — exactly the failure the freshness guard exists to catch. **The drift is measured across six hours: 4 → 14 → 18 → 20 → 22 → 31 behind.** **This is the known gap named in the home-tree repair entry below:** the self-heal guard needs a clean tree, so a stall landing on an already-dirty tree still needs a hand.

**Ticket bar reached (14:54) and still unfiled at 17:54, a fourth consecutive hour — this is agent work, not yours.** The fix is narrow and safe: extend THR-672's self-heal guard so it may discard a tracked modification whose content is **provably already on `origin/main`** (verifiable by diff, exactly as re-verified this run), rather than requiring a strictly clean tree. Filing it is outside this task's remit — it writes only these two files — so an executor or design session should pick it up. **That an obviously-correct, four-times-flagged fix has gone four hours without a ticket is itself the finding**: this task can name agent-side work but cannot queue it, and nothing else is reading these notes. Worth raising at the Friday retro if a fifth run carries it. **Your one command still clears today's instance immediately.**

---

## 3. Name the next game-facing stretch · CHRISTIAN ACTION · WILL NOT SELF-HEAL

**Status:** Open, carried from 11:54. Its original premise ("the queue is empty and the executor idles") is gone — the lane is fed *and visibly moving*, with one item shipped out of it this afternoon. **The urgency is gone; the question is not:** *every ready item is still self-maintenance.*
**Source:** Board scan at the 17:54 `keep-work-flowing-cc` run: **5 Ready for Dev** (all unassigned; was 6 — THR-687 shipped at 15:41), **1 In Dev** (THR-677, unassigned by design), 0 blocked, 0 stale. Blocked-status checked directly against the dependency links: the five cross-reference one another but none is `blockedBy` another.

**The five ready items, by what they actually do:** a headless tick bridge so automated browsers can advance the sim (THR-689); a union merge driver so idle PRs stop rotting (THR-691); generated artifacts brought into the build graph (THR-690); ticket-authoring rules (THR-688); and a plan-doc lint scope fix (THR-692). **In development:** the Cowork prompt port (THR-677) — parked on your trial approvals, see item 1.
...
All five sit in **Continuous Improvement**; the item in development is migration cleanup. **None of them adds a system, a piece of content, or anything visible in the game.** That is a legitimate state to be in — the tooling genuinely was lying about passing gates, and that is worth a day. But it should be a *chosen* state, not a default one, and choosing is yours by the standing rule (THR-608). **A second day of it would be a default, not a choice.**
...
**Status:** Open (not blocking) · **count unchanged at 14, re-verified 17:54** by live `gh pr list`, not carried forward — further merges landed since the last check (#681 THR-677, #683 the impediments audit) and the backlog still did not move, a sixth confirmation of the "does not retroactively rescue" prediction below. **Two of the fourteen read `DIRTY`** (#327, #553) — they need conflict resolution, not just a refresh. Oldest still #327 (2026-06-12). THR-691 (union merge driver for append-only closeout docs) sits in Ready for Dev and directly targets the conflict-rot mechanism.
