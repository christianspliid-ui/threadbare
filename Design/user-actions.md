...
**Last updated:** 2026-08-29 10:55 local (08:55 UTC). Standing asks only, per [THR-1077](https://linear.app/threadbare/issue/THR-1077). Run measurements, findings and narration live in the history: `git log -p origin/ops -- Design/user-actions.md`.
...
**First, play two — the batch-1 2-of-6 sample, open since 17 August.** The question is your own: *are they worth meeting twice?* Both were re-prosed since you last saw them, so a replay is honest. A no is as useful as a yes — it says what the retrofit bar still misses. Open the *good ending* links first (~5 min).
...
**Why it still matters:** the shelf is not short of work — it holds seven items. Take this on its own merit instead: it is the only High-priority content work on the board, it is written and waiting on one word, and **no encounter writing reaches the shelf until it clears.**
...
### 3. More design hours — still the constraint

Roughly twenty items sit in Todo and **every one needs a plan doc before an executor can touch it**. That is what the hours buy. **The shelf is full, so this is not urgent — it is structural.** The executor writes up problems it finds while building, which is what fills it; that source produces engine fixes, never new designed features. Only a design session produces those, and none can be prepared while the design column is full.
...
**New this morning, from the orchestrator lane:** [can background characters build things off-screen?](https://linear.app/threadbare/issue/THR-1348/ambitions-held-below-the-spotlight-tier-have-no-agency-path-10-of-the) — overnight measurement found ten strategic pursuits unreachable on one test world, the whole merchant-expansion family among them, because only characters below the attention threshold ever hold them and those characters have no way to act. Whether they *should* be able to is a question about what the game is, so it was filed as a design call rather than answered. Nothing is blocked on it today.
...
- **The agent handbook is on a diet** — [THR-1336](https://linear.app/threadbare/issue/THR-1336/claudemd-diet-gate-law-and-sandbox-lore-move-to-canonops-pages-with) merged 01:15; gate law and sandbox lore moved behind pointers. Nothing in the game changes.
