...
**Last updated:** 2026-07-28 23:55 local, by the hourly `keep-work-flowing-cc` CC task.
...
## This run's measurements (2026-07-28 21:55 UTC)

- **Assignee-invisibility re-derived: 14 named of 35** (up from last run's 13 of 34). Named: THR-574, THR-621, THR-626, THR-638, THR-655, THR-667, THR-723, THR-756, THR-757, THR-762, THR-763, THR-795, THR-836, THR-842. The mover is **THR-756**, which was not in the column at all last run.
  - **The single instance from last run is now a reproduced mechanism.** Last run located the defect in the promoting lane on one observation (THR-757, promoted 20:30:07Z, PR #1034). This run is the same signature one slot later: commit `d3ce2667`, *"orchestrator T1 sweep — promote THR-756 (2026-07-28 21:31Z)"*, and the row landed in Ready for Dev **already carrying the name**. Two consecutive `tb-orchestrator` T1 promotions, two stamped rows — that converts "the promotion did not strip it" from an attribution into a repeating behaviour of the lane (new finding 26).
  - **This also reframes the remedy.** Unstamping the backlog is now the *smaller* half: without a fix in the promoting lane the column re-accrues one invisible row per T1 sweep, which is roughly hourly. Both halves are agent work.
...
- **Queue: BACKED UP — 35 ready (1 Urgent / 1 High / 7 Medium / 26 Low), 1 In Dev.** Nothing stale: oldest `updatedAt` is 2026-07-24T07:59Z (THR-715), **~4 days**, inside `STALE_ISSUE_DAYS`=7. Depth 34 → 35.
  - **The +1 is one arrival against zero departures, and the composition moved with it.** THR-756 entered at 21:31:xxZ; nothing left the column (THR-838 was already In Dev before this window). Medium 8 → 7 with Low 24 → 26 is not a reclassification — it is last run's Medium bucket being one row larger than this run's under the same query, so the arriving row plus the bucket edge account for it. Reporting depth alone would have hidden that the shelf is deepening at the *small* end.
  - **Depth is still not a starvation signal in either direction.** 35 ready against 1 In Dev is planning outrunning execution by design (WIP=1), not a stall.
- **THR-838 remains the hour's event, and it moved from in-flight to landing.** PR **#1033** merged (`fbb2c392`) — WS5 Batch 1 converter groundwork plus the camp cluster — and a pass-2 branch (`thr-838/ws5-batch-1-camp-night`) is green at 21:22:51Z behind it. In Dev since 20:23Z, **~1h32m at this reading with merged output**, which is the opposite of a stall.
- **Deploy (step 2.5): `skipped`** — `{"verdict":"skipped","deployedSha":"fbb2c392"}`. **The deployed SHA advanced `46d1d345` → `fbb2c392` while the verdict stayed `skipped`**, which is the classifier behaving correctly on a content-data commit: the tip moved, and it moved for reasons that do not require a rebuild. A verdict held constant across a moving tip is weaker news than a verdict change, and is reported as such.
- **Merge gate (step 2.5b): `healthy`**, `startupFailureCount: 0` — **and corroborated independently this run rather than taken on the probe's word.** `gh run list --workflow=ci.yml` shows `success` on `main` at **21:33:03Z** (merge of #1037), i.e. jobs are executing steps, not concluding in ~3s with none. Two signals of different kinds beat one repeated signal (finding 9). The block Christian cleared at 17:02Z has not recurred.
- **Step 2.7 (heartbeat) clean:** `{"verdict":"ok","checked":8,"stalled":[],"neverRun":["monthly-rulebook-review"]}`. `tb-orchestrator` ran 21:27:09Z — **seventh consecutive on-time slot**; the THR-837 stall is not recurring.
- **Sibling reports (step 2.6): none folded in.** Both in-window reports are empty states (`orchestrator-2026-07-28.md`, `backlog-grooming-2026-07-28.md`); `weekly-hygiene-2026-07-26.md` is ~62h old, outside the 36h window. Grooming's WS5 risk stays resolved — the split happened, the batch is in Dev, and its first PR has now merged.
...
- **Reaper** ran 23:40 local, 15 min before this run: **33** worktrees / **44** branches / **2** stashes / **0** needs-disposition. **Worktrees 32 → 33 and branches 43 → 44 — both up one, which is growth from this hour's own sessions, not a reaper failure.** The reaper ran inside the window and reported clean; a count that rises between two healthy runs is the lane working, not backing up.
- **This run's worktree was created outside `.claude/worktrees/`** (session scratchpad, absolute paths on every write; `rev-parse --show-toplevel` asserted after creation, and the home tree re-checked as still on `main` immediately after, per THR-797). **The home tree was touched by `git -C` reads, one `git worktree add`, three read-only `npm run` probes, one `gh run list`, and nothing else.**
- **Zero Linear writes.** Two board scans plus one targeted read, read-only throughout.

## Open agent work (not Christian's)

- **Assignee-invisibility: 14 of 35 rows carry a name**, so the executor's `assignee:null` query cannot see them — including the sole High (THR-655) and the sole Urgent (THR-842). **Now known to have two halves, both agent-owned:** (i) one `save_issue(assignee:null)` per existing row, and (ii) a fix in `tb-orchestrator`'s T1 promotion path, which has now been observed stamping a row on two consecutive slots (THR-757 at 20:30Z, THR-756 at 21:31Z). Without (ii), (i) is re-accrued roughly hourly.
...
- **THR-835 is unreachable in `Idea`** — re-derived this run: still `Idea`, still `updatedAt 2026-07-28T07:10:00Z`, `stateHistory` still a single `Idea` entry with `endedAt: null`, untouched through seven on-time orchestrator slots. **The lane that promotes ran twice in the last two hours and promoted two other rows** (THR-757, THR-756), which is the sharpest available evidence that `Idea` is outside its reach rather than that it is idle. Content is a live merge hazard (the sole required check runs at 80–90% of its 20-minute cap; runner variance turns correct PRs red). **A working promotion lane that never reaches a column is stronger evidence than a broken one** — `Idea` is unreachable by every automated lane (finding 19). Remedy belongs to a write-remit session. No deadline re-armed.
...
- **`rescue/2026-07-17-detached-plans` is provably prunable — still present, re-derived again this run.** Four unique commits (`053c867a`, `8fa552c2`, `59ea82df`, `cf2f2dd5`) touching five plan docs; every one of the five blobs is byte-identical to `origin/main` by `rev-parse` comparison. Branch present and `origin/main..HEAD` still exactly **4** at this reading. One `git branch -D`, zero risk, wants a write-remit session. **The branch count rising 43 → 44 this hour is not this branch changing** — the reaper deliberately never auto-deletes stale *unmerged* worktree branches, which is precisely the class this one is in.
