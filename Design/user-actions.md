...
**Note:** the branch has since drifted and now conflicts in `src/engine/aftermathWords.ts` and its test. That is ordinary executor merge work and needs nothing from you, but the preview above is from 12 August and would want re-checking if the resolution changes any rendering.
...
- 2026-08-13: **the demo-ready checkpoint is closed and defect-free.** [THR-986](https://linear.app/threadbare/issue/THR-986) went Done this morning when its last player-visible defect shipped — the Chapter Ledger no longer prints `success_at_cost` at you ([PR #1422](https://github.com/christianspliid-ui/threadbare/pull/1422)), following yesterday's fix for encounters handing you raw `{adj}` placeholders. Nothing was needed from you.
