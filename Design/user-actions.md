...
**Last updated:** 2026-08-12 13:22 local (2026-08-12 11:22 UTC). Standing asks only, per [THR-1077](https://linear.app/threadbare/issue/THR-1077). Run measurements, findings and narration live in the history: `git log -p origin/ops -- Design/user-actions.md`.

## Standing asks

### 1. Did you switch the two work lanes off?

Two of the nine scheduled jobs are off — disabled, not merely quiet:

- **`tb-opus-pickup`** — the executor lane. Last ran 22:01 on 2026-08-11, shipped THR-1071, and has missed every hourly slot since (~15 h).
- **`tb-orchestrator`** — decides what gets promoted next. Last ran 21:26 on 2026-08-11, ~15 h and ~15 slots ago.

**It was not a machine outage.** From 21:26 to 03:57 the machine was awake — the briefing lane and the git housekeeping job both fired every hour across that window while these two did not. Only these two stopped, and they stopped while everything around them kept running. No pause marker records it (the marker on disk is an expired historical one from 2026-08-03).

**While the executor lane is off, nothing on the board moves** — including work your other rulings would create. The queue has sat at 21 ready / 0 in flight throughout.

**If you turned them off: just say so** and a marker gets dropped. It would be a coherent follow-through on your 2026-08-10 direction to stop the lanes filing cleanup work — switching off the lane that drains it is the same instinct. **If you didn't:** say that, and it gets investigated as a real stoppage.
