...
**Last updated:** 2026-08-18 20:57 local (18:57 UTC). Standing asks only, per [THR-1077](https://linear.app/threadbare/issue/THR-1077). Run measurements, findings and narration live in the history: `git log -p origin/ops -- Design/user-actions.md`.
...
The town you helped now keeps a door open — *A Standing Welcome*, ten game days, visible on the town's page and reachable from the chip. Live since 18:52. Nobody has written what happens when you walk back in.
...
It is the smallest ask on this list and unblocks the longest chain: return visit → payoff real → the sample verdict you have been holding → batch 2's nine encounters.
...
**One claim retired tonight:** this was the lead for a day on the grounds that it was the only ask supplying new feature work. Four feature jobs landed on the shelf at 18:30 (the nudge-library set), so that is no longer true. The sitting is still the largest-leverage ask here — it is no longer the one holding the pipeline up.
...
Every nudge card in every Meet-The-First hand shows the same picture — wisps settling over a plain clay bowl. All 424 cards. They were written for three different images (a crowd, a mercy, a blade) and none was ever made, so the game quietly falls back to one stand-in. [Open the route and see it](https://threadbare.vercel.app/?view=game&firstunmet&size=medium).
...
Nothing is blocked on the answer. It is on the list because it is the game's first impression.
...
**The encounter sample verdict stays held one more step, and not by you.** [The two encounters](https://linear.app/threadbare/issue/THR-1130/encounter-factory-pilot-volume-retrofit-the-15-nudge-era-encounters-to) are corrected and live, and your favour finding is fixed. But asking *"worth meeting twice?"* while the Grateful Kin's payoff opens onto an unwritten room would earn a no it has not earned. It returns with fresh links once ask 1 above has been through a session. The nine encounters behind it stay held meanwhile — the deliberate cost of not writing them against a bar that is still moving.

**Nudge-card art, when it next comes up.** Standing recommendation the lane follows unless you say otherwise: *remap where a match is honest, come to you only when it is not.* That is what surfaced ask 3, where no honest match exists for two of the three.
