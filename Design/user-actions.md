...
**Last updated:** 2026-07-19, 06:30 local (light refresh by the hourly `keep-work-flowing-cc` CC task; last full rebuild was the 2026-06-23 retro)
...
**Status:** Open · ~60 days · **~83 non-`.codesight` files uncommitted** on the home tree as of 2026-07-19 06:30. The tree is on `main` but has **drifted 45 commits behind** `origin/main` — now **well past the alarm threshold of 10** (37 last cycle, climbing steadily) — because it is too dirty for the hourly auto-sync to fast-forward, so it slips further behind each cycle until the dirt is cleared. A `git fetch && git pull` (after triage below) catches it up. The dirty set is **larger than earlier briefings assumed** and is no longer just `Docs/plans/` drafts — it now includes a stack of `src/engine/army*`, `battle*`, Codex/AscendantBar/GameView/DebugTab component edits, plus staged plan-doc + script deletions. These are almost certainly stale local echoes of work that already shipped cleanly through `origin/main` (the TB-073 war system and the orphaned-card inspector both merged), but the volume warrants a **careful** triage, not a blind `git checkout -- .`
