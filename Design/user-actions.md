...
**Last updated:** 2026-08-28 07:58 local (05:58 UTC). Standing asks only, per [THR-1077](https://linear.app/threadbare/issue/THR-1077). Run measurements, findings and narration live in the history: `git log -p origin/ops -- Design/user-actions.md`.
...
Roughly twenty items sit in Todo and **every one needs a plan doc before an executor can touch it**. Your [shared-anchor design](https://linear.app/threadbare/issue/THR-1212) is five slices into being built and [hunger vocabulary](https://linear.app/threadbare/issue/THR-1213) is queued behind it — that is what the hours buy.

**The shelf looks healthier than it is, and got worse this morning by growing.** Eight jobs are queued as of 07:58, but **seven of them are small follow-ups the last two builds filed on themselves as they finished** — two arrived overnight from your own anchor-machinery build. **[Hunger vocabulary](https://linear.app/threadbare/issue/THR-1213) is the only new feature work on it.** When the builder reaches it, the shelf is cleanup only — which is what the blocked design column actually costs, expressed in jobs rather than in days.
...
**All three remaining carve-up docs are now unblocked** *(orchestrator lane, 28 August)*: with the action library landed, [the reactive loop](https://linear.app/threadbare/issue/THR-1298) and [the calling & the surfaces](https://linear.app/threadbare/issue/THR-1299) can run in parallel, and [the undertaking factory](https://linear.app/threadbare/issue/THR-1300) — which was waiting on the action library specifically — is free to be last. Nothing blocks any of them.
...
- **Your shared-anchor design is being built, five slices deep** — [THR-1212](https://linear.app/threadbare/issue/THR-1212) walked Todo → designed → queued → claimed overnight, and slices 1–5 merged between 23:37 and 03:30 ([PR #1678](https://github.com/christianspliid-ui/threadbare/pull/1678) … [PR #1682](https://github.com/christianspliid-ui/threadbare/pull/1682)), all deployed. A thing in the world now has one way to be named, the catalog is checked in both directions, the chips the anchor gate cannot see sit under a ceiling that cannot quietly rise, the ledger asks what a reader does with an anchor rather than whether one exists — and the newest slice builds a whole world and asks whether the content is really there, so the machinery is checked against a world instead of against itself.
