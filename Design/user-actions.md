...
**Last updated:** 2026-08-16 00:54 local (2026-08-15 22:54 UTC). Standing asks only, per [THR-1077](https://linear.app/threadbare/issue/THR-1077). Run measurements, findings and narration live in the history: `git log -p origin/ops -- Design/user-actions.md`.

## Standing asks

### 1. An attended design session for [THR-790 — Traits wave 2](https://linear.app/threadbare/issue/THR-790/traits-wave-2-locations-artifacts-and-draw-by-trait-pools)

*— from tb-orchestrator.* The longest-waiting agreed item on the board: created 2026-07-26 as part of the Traits program you settled that same day, blocker `Done` since then, unstaged for 20 days. Staged now because the Ready-for-Dev shelf ran thin.
...
### 2. Veto window: two of the six on [THR-1131](https://linear.app/threadbare/issue/THR-1131/retrofit-batch-1-the-slice-six-through-the-factory-line-contract-thr) have no contract shape

You chartered batch 1 as the slice six. Two of them — `grateful_kin` and `full_moon_collection` — are the sequels the [brief you approved](https://github.com/christianspliid-ui/threadbare/blob/main/Docs/plans/encounters/2026-08-15-retrofit-batch-1-brief.md) explicitly held out of every batch: they carry aftermath, cast and rewards but no test step, so the hand block has nothing to check, and your ruling 3 forbids waiving a block.

**The reading, unless you say otherwise:** the four testable ones (`unsafe_bridge`, `swindled_family`, `swindler_found`, `bargain_at_crossroads`) run the full factory line; the two sequels park with a salvage note per your ruling 4, and the sequel contract gets designed as its own artifact. Batch 1 delivers four. Nothing is discarded — a forced retrofit would be.

No reply needed to proceed; one word stops it.
...
It is no longer blocking: an agent verified the spec is on `main` and started the retrofit past the mutex, recording the reversal. This is hygiene only.
