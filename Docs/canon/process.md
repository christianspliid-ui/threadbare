...
- **Sandbox limitations reference:** [CLAUDE.md → Known Sandbox Limitations](../../CLAUDE.md). Read before debugging environment failures — `rg` blocked/absent in the agent sandbox, PowerShell scope leaks, `npm test` timeouts, Linear `save_issue` silent drops, vault writes filesystem-only via `OBSIDIAN_VAULT_PATH` (no Obsidian MCP — THR-654), etc.
