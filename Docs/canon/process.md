...
- **Pre-commit gate:** [`Docs/canon/verification-gates.md`](verification-gates.md) — **authoritative** since THR-1336 (this bullet was itself an ~4.5KB restatement until then, which is the disease the single-authority rule exists to stop). The two-sentence shape: classify the diff first (`npm run classify:diff` — docs-only owes only the doc gates, code owes the full gate incl. the typecheck ratchet, never `npx tsc --noEmit`); tree-diffing freshness gates run **last**, immediately before `git push`. Everything else — track membership, ratchet mechanics, CI job layout, required checks, the incidents — lives on the gate page.
...
4. **An agreed outcome delegates its consequences (Christian, 2026-08-12).** When work already agreed — a shipped design, a locked format, a quality bar with a stated purpose — forces a code or test change, the agent makes the change; the gate is verifying the agreed outcome survives, not asking permission to act on it. Two named cases from the ruling:
   ...
   **Four more named cases (Christian, 2026-09-11)**, and a fifth (2026-09-25). The hourly briefing had accumulated thirteen standing asks; put to him in one list, his answer was *"you are approved to unblock everything here. none of it seems dangerous or problematic or requires an important verdict from me."* Each is now the agent's call, veto retained, and none returns to the briefing as an ask:
   - **Glossary seating is delegated.** An agent may seat a `UL-proposal` — read the proposal, judge it against the shards and the code anchors, record `seated by delegation <date>` on the issue, and land the entry. The human gate that `Approval is always human` used to name is satisfied by this rule; a drift-scan filing still never auto-merges, because *someone* must read it. Christian vetoes by chat, and a veto is a rename PR, never a rewrite of the process. First applied: seven words on 2026-09-11 (Realm, hold, cast, Agreement, Motive gate, Composition Contract, Motive Receipt).
   - **Spend inside an approved ticket's stated batch is the executor's.** Image-generation credits for a ticket that names its count (THR-876: five images plus retries) are spent without a further ask; the closeout discloses the actual count. A ticket that does not state its batch still discloses cost before running (the `disclose costs` rule stands).
   - **Wayfinder decision tickets are resolved by design sessions.** A `wayfinder:grilling` / `wayfinder:prototype` ticket on a charted map is decided by the session that works it, recorded on the ticket with the outputs attached so a veto has something to look at — never listed as "yours" in the briefing (the fight, powers and items maps, 2026-09-11).
   - **Design work runs unattended (Christian, 2026-09-25, THR-1611).** *"I think we have reached a state of the game where you can probably iterate and progress designs without me in many situations, for example where we have agreed on a wayfinder map."* The `tb-design-lane` routine (`design-lane` skill) decides unreserved wayfinder tickets, closes cleared maps, and authors and hands off plan docs for agreed work — the full `design-session` gates, no chat gate. Christian keeps charting (naming a destination), tickets a map lists under `## Reserved for Christian`, forks with no agreed outcome, and "not fun" calls. The lane's decisions reach him as a `## Decided for you` veto line in the briefing — never an ask — and no plan doc starts on a lane decision younger than 24 h, so a veto lands before anything is built on it.
   - **Quiet spells are normal, overnight and weekend alike.** The 2026-08-08 overnight ruling extends to weekends; the briefing lane declines both shapes with one visibility line, and a `recovered` lane-silence verdict is an ask only on a weekday gap with no pause marker.
...
- **Sandbox limitations reference:** [`Docs/ops/sandbox-limitations.md`](../ops/sandbox-limitations.md) — the full catalog (authoritative since THR-1336; CLAUDE.md § Known Sandbox Limitations keeps the five-rule summary). Read before debugging environment failures — `rg` blocked, PowerShell scope leaks, `npm test` false-red shapes, Linear silent drops, vault filesystem-only, the home-tree/worktree git hazards.
- **UL-proposal flow:** [`UL/Process` → UL-proposal](../ubiquitous-language/Process.md). When you encounter an undeclared concept or a UL-vs-source disagreement, open a Linear issue in Continuous Improvement labeled `UL-proposal` containing: proposed term + definition, where/why it was encountered (with quote), relationship to existing terms, content-adjacency assessment. Seating is delegated to agents with Christian's veto retained (rule 4 above, 2026-09-11) — a session reads the proposal, records the seat on the issue, and lands the entry; a drift-scan filing still never auto-merges. UL wins on terminology disagreements.
