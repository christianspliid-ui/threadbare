...
- **Pre-commit gate:** [CLAUDE.md → Testing](../../CLAUDE.md). **Two tracks since THR-917 — classify the diff first.** `git diff --name-only origin/main...HEAD | grep -vE '(\.md$|^Docs/|^Design/|^\.planning/)'`: empty means docs-only, which owes only `check:generated-freshness`, `lint:plan-doc`, and `check:impediment-ids`. A **code** diff owes the full gate: `npm test`, `npx vite build`, `npm run check:process`. The predicate matches CI's own `detect` filter, which skips the whole `Test · Typecheck · Build` job on docs-only PRs — so running the suite on markdown is stricter than every gate it exists to pre-empt. Caveat (THR-909): the three doc-validating gates live inside that skipped job, so on a docs-only PR **local is the only place they run**. **Not** `npx tsc --noEmit` — it is a no-op here (root `tsconfig.json` has `files: []`); CI's `Test · Typecheck · Build` is the type gate, and local type evidence is a `tsc -b --force` net-new diff (THR-686). Verification evidence (raw output) is mandatory in the closing commit body or completion comment.
