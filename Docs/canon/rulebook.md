...
**A threaded company earns authored moments.** When a company holds at least one mortal you have threaded, three lifecycle transitions rise from the silent systemic line into an authored beat: **Seeking Companions** at formation, **The Parting** at dissolution, and **The Shared Spoils / Old Wounds** the first time it crosses into fray [IMPL for The Parting and the fray moment — `groupParting.ts` / `groupFray.ts`, fired from `phaseGroups`; each fires on the *transition*, not the state, and only for threaded companies; an untethered company's end stays the systemic line]. Seeking Companions — a branching formation spotlight that *creates* the company as its outcome — is designed but its firing path is not yet wired [DESIGN — formation-spotlight escalation, remaining THR-74 scope].
