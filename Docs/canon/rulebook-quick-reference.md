...
Resolution: **sigmoid → d100**, unified. No alternative dice. Outcomes land on a five-band ladder — *clean · at-cost · failure · crit-success · crit-failure*. **Success-at-cost (scraping through) is the dominant, expected texture**; clean and critical success are rare signals a god notices. **A mortal takes on challenges it forecasts at ~50–65%; skill decides which ones** [DESIGN — THR-1575]: success stays level while what a mortal attempts grows with it, failure never costs reach, a failed challenge is not retried straight away, and long odds are the god's to impose. Critical failure survives at every scale (only its severity scales) **except under an active `no_crit_fail` nudge** — an authored, essence-priced, per-step exception, never a global gate. **Every failure leaves a story artifact** — no outcome is dead air.
