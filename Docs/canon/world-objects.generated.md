...
| `trait` | Trait | node · `trait` · `subcategory` ∈ {innate, mastery, reputation, personality, core, destiny, cultural, experience} | — | Traits & attachments | `gameInit`, `culturalTraits`, `capabilityGrowth`, `reputation` | live | 202 | 🟢 LIVE |
...
- **Condition** (`condition`, UL `Traits.md#trait-category`, chip kind `attachment`) — Wounds, diseases, strains; blessings and curses as signed conditions; scars as permanent ones. Per-bearer state belongs on the `has_trait` edge — today conditions are minted one node per bearer, the repair is its own ticket.
- **Trait** (`trait`, UL `Traits.md#trait`) — The graph's vocabulary of what a thing *is*: shared definition nodes, per-bearer state on `has_trait`. Tags refine traits; they are not a general object taxonomy.
