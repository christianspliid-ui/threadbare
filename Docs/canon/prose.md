...
- **Exemplars:** [Docs/exemplars.md](../exemplars.md) — `swollen-ford-exemplar.ts` is the encounter format exemplar; `rival-shrine-betrayal.ts` / `flawed-steel.ts` are **wiring references only** (their prose and choice model predate the nudge pivot and Doctrine v2); plus inline **baseline-register** and **peak-register** prose exemplars (THR-609).
