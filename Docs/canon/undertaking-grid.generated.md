...
| **Ambitions & Undertakings** | `ambition`, `undertaking` | · | · | · | · | · | · | · | · | READS | — |
...
- **Use × power** — `activate_spell` — Casting: the spell op, its costs and its backlash. A bestowed power that names no spell template is refused — this cell measures how often one can be cast at all (the Power kind is dormant until create × Power writes `knows_spell`). **Read by:** Both costs land now: the cell refuses with `spell_exhausted` while `exhaustedUntilTick` is ahead of the tick, and the soul-price leaves the `doom` health meter to become a `QuintessenceEvent` on `pendingQuintessenceEvents`, consumed by `phaseQuintessence` where the threshold gates already bite. `health_sacrifice` still writes `doom` — that cost genuinely is health. (THR-1428 R4.)
...
- **Create × power** — Learning a spell — the first writer of the `knows_spell` edge (registered in the schema, written by nothing) to an existing spell template; use × Power then reads it as one way of being able to cast. Wakes the kind; the scholar's tier-one work. Op needed: `learn_spell`. **Reader:** The use on a Power cell, which reads `knows_spell` to decide whether a mortal can cast at all — and, since THR-1428 R4, charges the cast' soul-price to quintessence. _(THR-1397, Christian in chat, 2026-09-03)_
- **Destroy × power** — Stripping another's power inherits the curse: a curse-class condition that suppresses the power, motive-gated — create × Condition's op with a suppression sign. Unlearning one's own is not worth a work. **Reader:** The condition layer: a suppression-signed condition is read by the effect resolver and by the use × Power cell's prerequisite check. _(THR-1397, Christian in chat, 2026-09-03)_
- **Create × condition** — One signed cell. Against another it is a curse: motive-gated, Veil-leaning, a signed condition with a duration on the bearer's edge, and its counter-play ships in the same commit because destroy × Condition (curing) is live. For oneself or an ally it is a blessing, the same cell un-gated. Op: the reward-pool condition mint with the target as the site. The first cell whose object is made against a person. A plague on a town is lower × Location, not this. **Reader:** The condition layer and its expiry phase, plus the destroy-on-Condition counter-play shipping in the same commit — the cure is the reader that makes the curse a move rather than a state. _(THR-1397, Christian in chat, 2026-09-03)_
