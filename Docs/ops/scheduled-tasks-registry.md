...
Current recurring task registry. **Verified against `list_scheduled_tasks` and `Get-ScheduledTask` on 2026-07-27 (THR-794 — both lanes re-checked row by row); `flush-plan-docs` removed 2026-07-21 (THR-654 demolition). Re-verified 2026-07-31 after a pause/resume cycle in the desktop app silently reset four tasks' crons to generic defaults (impediment #359) — `tb-orchestrator`, `keep-work-flowing-cc`, `weekly-retro`, and `daily-backlog-grooming` were restored from this file's crons; after any pause/resume, diff `list_scheduled_tasks` against these tables before trusting the lanes. **The wipe recurred on the 2026-09-03 pause/resume (#359 ×2) and was repaired the same way — treat the diff-after-resume check as mandatory, not precautionary; the 2026-09-06 weekly-retro run re-verified all nine crons against these tables and found them restored.** **Re-verified 2026-09-23 (impediment #359 ×3), which falsified two claims this page used to make.** (1) **The MCP `update_scheduled_task` path is not safe either** — `tb-orchestrator` came back `"Manual only"` with no cron from a plain `enabled:true` call, while the other eight survived it. The rule is not "app UI wipes, MCP preserves"; it is **any state toggle may drop a cron, on any path, non-deterministically**, so the diff-after-toggle check below is the only thing standing between a wipe and a lane that silently never fires. (2) **The per-task jitter is NOT deterministic** — it drifts between runs (measured 09-19→09-22: pickup 53→653s, orchestrator 76→196s, weekly-retro 571→211s, daily-grooming 519→39s). The scheduler adds a per-task jitter of a few minutes to the cron minute, so **the slot name, the cron minute, and the actual fire time are three different things** — but because that jitter moves, **the `Fires` column is advisory only, accurate to roughly the quarter-hour that determines collision-freedom, never to the second.** Check the `Cron` column when restoring a task; read live `jitterSeconds` when you need an actual fire time.
...
>
> **Measured 2026-09-23: the per-lane model above is aspirational, not enforced.** Grepping the same day’s run transcripts shows `claude-fable-5-1` and `claude-opus-5` / `claude-opus-5[1m]` **interleaved across lanes and across runs of the same lane** — no lane is pinned, including the one named `tb-opus-pickup`. Nor is there any agent-reachable way to pin one: `update_scheduled_task` exposes no `model` parameter, no `SKILL.md` frontmatter carries a `model:` field (all 13 checked), and no key in `.claude/settings.json` / `.claude.json` governs it. **Changing a lane’s model is therefore a Christian-owned action in the desktop app, if that control exists there at all** — an agent can report the model a run used, never set the model a run will use. Treat this block as a record of intent, and read the transcript when you need the fact.
...
## Unversioned sources are mirrored into the repo

**Two classes** of file this registry names live **outside** version control: CC-lane **prompts** at `C:\Users\chris\.claude\scheduled-tasks\<id>\SKILL.md`, and host-lane **script bodies** at whatever path the Windows task invokes. Both share the same two failure modes — merging a repo change does not deploy them, and a disk loss takes them with it — so both take the same rule:

> **Every unversioned file this registry names carries a mirror under `Docs/ops/`, or an explicit note in the inventory below saying why it does not. When you edit the live file, update its mirror in the same PR.** The mirror is a copy for review and recovery, never the source of truth.

**Why the rule is stated here rather than per-row (THR-824).** It used to be stated twice in ad-hoc form — once for prompts in this section, once inside the reaper's own bullet — and **neither statement was where someone registering a new host task would look**. That is precisely how autosync went unmirrored from its registration (2026-07-18) until THR-937 mirrored it: the reaper's row asserted the obligation for the reaper, the prompt section asserted it for prompts, and the new host task fell in the gap between them. It was the worse of the two to lose — the reaper is a cleanup convenience, whereas autosync is the only automated repair for a documented harness bug that parks the home tree on a detached HEAD ([anthropics/claude-code#79713](https://github.com/anthropics/claude-code/issues/79713)), so reconstructing it would have meant re-deriving the loss-free-park predicate (detached ∧ 0 unique commits ∧ 0 tracked modifications) from CLAUDE.md prose rather than restoring known-good code.

### Inventory — every unversioned source this registry names

| Live path | Class | Mirror |
|---|---|---|
...
A mirror's fidelity is checkable in one pass: extract the fenced block and compare it to the live file. Both host-script mirrors were verified byte-identical on 2026-08-08 (THR-824).
