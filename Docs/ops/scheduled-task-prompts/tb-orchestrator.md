...
description: Hourly Threadbare orchestrator — promotes unblocked work to Ready for Dev (T1), burns down wayfinder research tickets and surfaces reserved ones (T1.5), stages design requests for the design lane when the program shelf runs thin (T2 — stages, never authors; tb-design-lane authors, THR-1611), owns daily architecture-health surfacing (T3). Never claims an issue (sole exception: AFK wayfinder tickets).
---
...
Wayfinder maps (THR-900, `wayfinder` skill) chart multi-session design efforts as decision tickets in Linear. Research tickets are auto-resolved here; grilling/prototype tickets are decided by the design lane (`tb-design-lane`) under process.md rule 4's 2026-09-11 case; only tickets the map lists under `## Reserved for Christian` go to him (THR-1611 superseded the 2026-07-31 "briefing for HITL" routing). Full procedure: orchestrator skill § T1.5 — this is the condensed run order.
...
Take the top agreed-but-undesigned item and **stage it for the design lane — never author the plan doc in this lane**. Staging = post a design-request comment on the issue (why now, shelf depth, what makes it agreed, the canon/Step-0 loads a design session needs) and move it to `In Design`, **unassigned**; `tb-design-lane` picks it up and authors it. **Do not** put `design session wanted` under `## Needs Christian` — authoring agreed work is no longer his to start (THR-1611); surface it there only if the design lane released it as a reserved fork. Why this lane still does not author, now that the Sonnet premise is dead (no lane is pinned to a model, registry 2026-09-23): an hourly run that also promotes, sweeps and runs T3 has neither the time nor the context for a governed plan doc, and a long run here eats later hourly slots (THR-837). The `ORCH_MAX_IN_DESIGN` bound counts staged items still live by the predicate above; an item still unpicked after 48h is re-surfaced in the report, not re-staged.
