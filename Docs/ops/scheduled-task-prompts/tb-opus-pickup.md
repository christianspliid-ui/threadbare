...
Browser-verify (only if the change touches the UI pillar — src/components/, which holds every HexMapV2/Three.js surface, or src/hooks/, src/contexts/, src/index.css): screenshot at 1920×1080, console output, and one window.__DEBUG.* state assertion. Use Claude-in-Chrome for any WebGL/canvas surface (Playwright can't see it).
