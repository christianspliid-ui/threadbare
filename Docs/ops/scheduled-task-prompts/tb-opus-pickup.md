...
npm run check:wiki-freshness:blocking — BLOCKING in CI (THR-730). Fails if a changed file matches a wiki page's sources glob (public/wiki-manifest.json) without updating that page. Fix by updating the page in the same PR, or add a `Wiki-freshness-exempt: <reason>` line to a commit body for a behavior-neutral change.
...
Browser-verify (only if the change touches the UI pillar — src/components/, src/views/, index.css, or any HexMapV2/Three.js surface): screenshot at 1920×1080, console output, and one window.__DEBUG.* state assertion. Use Claude-in-Chrome for any WebGL/canvas surface (Playwright can't see it).
