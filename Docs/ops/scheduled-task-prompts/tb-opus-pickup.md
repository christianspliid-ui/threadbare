...
Upstream-shipped check after claim: git fetch origin main then grep origin/main for Fixes/Closes/Resolves <id>. If already shipped, comment the commit SHA, unassign yourself — `save_issue(id, assignee:null)`, state STAYS In Dev, verify-after-write, **re-asserting the null once if the assignee key comes back** (THR-1058: a park is only readable as assignee-null ∧ In Dev, and a null that does not hold hides finished work from `keep-work-flowing-cc` indefinitely; bundle one other field at its current value, since a sole-field null can no-op — impediment #380) — and exit clean. Do NOT release it to Ready for Dev (THR-958): the fresh-claim and resume paths discover the same state and get the same disposition, because disposition follows the state discovered, never the path taken to discover it. A completed ticket returned to the queue is re-claimed and re-investigated every hour forever, costing a drain slot per occurrence and looking like healthy queue membership the whole time. The park stays In Dev because `keep-work-flowing-cc` reads that exact shape (assignee null, state In Dev) and surfaces it to Christian to close.
...
Classify the diff — `npm run classify:diff` — BEFORE choosing the gate track, and before the first commit (THR-1513). With nothing committed on the branch the classifier falls back to the working tree (tracked edits plus untracked paths) and says so in its header, so the verdict — and the UI-pillar browser-verify reminder — can render at the moment the route still costs nothing to choose. A `code` verdict owes every gate below; a `docs-only` verdict owes only the docs track (step 7's list). The reminder is path-triggered; the route rule is Done-when-triggered (THR-1494 owed browser evidence with zero UI-pillar paths) — if the ticket's Done-when asks what a surface shows, the route was owed in the claim comment at step 2, whatever this prints. Re-run after committing: the committed range is what CI classifies.
WIP push BEFORE the gates below (THR-1529, pull-work § "Mid-run — WIP push before the verify phase"). On a code-track ticket, once the implementation is written, run `git add -A && git commit -m "wip(thr-XXX): <summary> — checkpoint before verify"` and then `git push -u origin HEAD`, before npm test / test:heavy / the ratchet / vite build / Playwright. Put NO close keyword in it (bare THR-XXX only) and open NO PR yet. The closing commit comes later as a new commit on top: never amend a pushed WIP commit. Why: a session killed by a usage limit never gets to post the unfinished-pass checkpoint. THR-1521's run (2026-09-22) died with its whole 39-file slice uncommitted in a local worktree, and it sat invisible for ~19.5 h. A pushed branch survives the dead session and the reaper. A failed push logs one line and continues, because the local commit is the fallback.
npm test — all pass
...
On resume, read the latest checkpoint comment FIRST and continue from it - do not re-implement from scratch. No checkpoint does not mean no work. Before re-implementing, run pull-work Step 1.8's stranded-work probe (THR-1529): look for pushed branches naming the id, then filtered dirty worktrees (`git worktree list --porcelain` + `git -C <wt> status --porcelain`). A dead run's slice may exist only there. Commit it in place, push, and continue on it. If 3+ prior checkpoint comments exist without a ship, the ticket is too big for this lane: post a final comment recommending a split (name the seams), then move it to `Todo` AND unassign - `save_issue(id, state:"Todo", assignee:null)`, verify-after-write - and exit clean. `tb-orchestrator` re-scopes from `Todo` (T2) and promotes it back to Ready for Dev once the split is authored. The destination is the whole point: parking this one In Dev strands it, because the orchestrator never reads In Dev and a deliberate unassigned park is not a stale claim, so no lane can see it (THR-846 — THR-838 sat ~13h that way).
