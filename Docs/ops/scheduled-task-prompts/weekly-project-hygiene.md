...
2. **Silent query sites.** A site with no hits over the seeded run is either *unreachable from the live board* or *unauthored* — opposite problems with opposite fixes. Check reachability before recording a silent site as a content gap. (`undertaking_catalyst` was unreachable under `UNDERTAKING_MODEL: 'cells'` until THR-1497 shipped 2026-09-16; it is reachable now, and the census reads it off state.)

**Read the `Source` column before filing anything (THR-1514).** The two seeding sites — `encounter_seed`, `undertaking_catalyst` — are counted **off state** (one per seed that spawned or withered) and their numbers, including a zero, are exact. Every other row is `ring`: harvested from the 2000-entry ring after every tick, and a **floor** — the report prints how many traces were emitted and evicted before any read, and a `ring` zero means *not seen*, never *did not fire*. Re-run at a lower `--ticks` to separate *alive-but-early* from *actually silent* on a `ring` row before a finding leaves this lane; never file a `ring` silence as a content gap on its own. The report's `Seed consumption (state)` block flags any seed that left the pool with no consumption event as **unknown** — that is the only shape that could ever mean an engine defect, and it is not evidence of one until the seeding phase has been read.

**The `Appointments (state)` block is the reachability row for THR-1479 (THR-1518).** It prints templates that author an appointment, seeds planted on the run, kept, missed by reason, and one verdict: **HIT** (the live board planted one — the two appointment interface contracts stand on this), **UNREACHED** (authored, never planted on this seed — re-run at another seed before reading it as dead), or **DEAD** (nothing authors one). Two consecutive batches at zero authored is the retro's *dead primitive* finding (THR-1479 § Kill criteria); the die-B `appointment` floor and the systems-prompt *Live primitives* entry are the first suspects. All counters are off state — exact, never a ring floor.
