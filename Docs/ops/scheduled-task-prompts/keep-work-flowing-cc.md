...
Hard rules: read-mostly; never claims/implements issues; never sets Linear issue state; the briefing file is the canonical output surface — no Slack, no Linear comment to Christian; the change-gated Discord DM ping (skill step 6) is the one push channel and always points back at the briefing. Execute autonomously.