...
**The observation.** `sweep:rank-reach` (seed 42, medium, 900 ticks — the identical command all five days) has run **38 minutes in this session and emitted zero `[DistanceMatrix]` warnings.** On each of the prior four sweeps that same command emitted them repeatedly, at a count this tier reported as *moving*: `+151` on 08-27c, `+142` on 08-28b, where 08-28b noted it *"was caught rising within a single run."*
...
**The honest limit, stated rather than smoothed over.** This is a *strong* reading, not a completed measurement. Pre-fix the inflated count had to grow past 1200 during the run, and I do not know at which tick it first crossed — so "zero at 38 minutes" is consistent with the fix but does not by itself exclude a late-firing warn. **The PASS/FAIL verdict is therefore recorded as not measured this sweep**, exactly as run d recorded it, and confirmation is handed to tomorrow's daily sweep, which will have the complete run. What is not in doubt is the code change and its date; what is provisional is only whether the warn count is now exactly zero for the full 900 ticks.
