...
> **Generated:** 2026-09-22 by scripts/build-authoring-brief.ts
> **Sources:**
>   - Docs/plans/2026-04-16-systemic-wiring-guide.md (sha1: ca3a06aca943e374c8f97c4aea259a067553334e)
...
>   - Docs/canon/undertakings.md (sha1: fb599538e7f33356509101df5708cba8da04a268)
...
- **Catalysts** — `catalystQuery`, the encounter family a completed work may seed, named by kind and tags: `{ kind: 'encounter_template', tags: ['#consortium_errand'] }`. Read by `maybeSeedCatalyst`, which stores the query on the planted `PendingEncounterSeed` so the *one* seeding site resolves it when it comes due — the same path an aftermath `encounter_seed` takes, which is why the two planters cannot drift about what a dead reference does. The deprecated `catalystEncounterIds` is kept one release for un-migrated packs. **This field had never once fired** (THR-1488): every id the seven packs spelled was `encounter_<name>` while the corpus spells encounters `encounter.<name>`, so all thirty-three resolved to nothing and every catalyst seed withered on arrival — DORMANT in THR-1481's substrate inventory for exactly that reason. The `catalysts` block is now fatal on both operands, which is what keeps it from going dormant again, and the write set records the query so a template whose only declared write was its catalyst does not read as vacuous after migrating. **Reachable from the live board since [THR-1497](https://linear.app/threadbare/issue/THR-1497) (2026-09-16).** THR-1488 measured that all 35 carriers were pack templates, which `UNDERTAKING_MODEL: 'cells'` never walks. The decision: the catalyst belongs on the **cell** — `UNDERTAKING_CELL_CATALYSTS` (`src/data/undertaking-cells.ts`), a bounded table by cell id read once at synthesis, overridable per package via `UndertakingCellOverride.catalystQuery`; not on the registry's verb semantic (an op, not content) and not by starting the legacy pack arm. Eleven cells carry one, each family chosen for **what the work disturbs**, never who does it: building or raising a settlement → `#fellowship_errand`; a lane opened, widened or blockaded → `#consortium_errand`; an army raised or reinforced → `#company_errand`; a masterwork → `#craft_commission`; a ring founded → `#thieves_errand`; a settlement razed → `#watch_errand`. Two defects fell out on the way and are fixed in the same PR: the seeding site traced every seed at `encounter_seed`, so `undertaking_catalyst` was a registered site nothing emitted (now derived from the seed's `sourceReactionId`, `STRATEGIC_CATALYST_REACTION_ID`); and it read the location subtype off the raw `located_at` node, so a target inside a **Place** (a market district, an inn) had no subtype and every gated family withered — now resolved to the containing Location (`seedTargetSubtype`), the position model's own rule. Live claim: seed 42 / medium / 200 ticks, 14 catalyst seeds planted, 6 spawned (`tg.quest.pocket_run` ×2, `tg.quest.warehouse_raid`, `bf.quest.lay_foundation`, `bf.quest.repair_wall`, `mc.quest.siege_work`), three of them from inside a Place; `check:undertaking-live cell.create.network --seed 3` passes `catalyst_seeded`. **The wake is judged where the work stands, then where the actor does ([THR-1511](https://linear.app/threadbare/issue/THR-1511), 2026-09-18).** An undertaking completes wherever its actor happens to be — a route laid from a fort, an army raised in the field — while every family above is gated to settlements, so judged at the actor's feet 8 of those 14 seeds withered on a family that had eligible members. `maybeSeedCatalyst` now stamps `PendingEncounterSeed.resolutionLocationId` with the completed work's own Location (`catalystAnchorLocationId`, `src/engine/undertakingCatalystAnchor.ts`: created nodes first, then the object, then the target; a route resolves to the settlement end it reaches, a Place to its parent, a group to where it stands; a masterwork to nothing), and `resolveSeedByQuery` judges the `locationSubtypes` gate at that anchor first and the actor's feet second — the wake of a road reads "the Consortium comes to the town your road now reaches". The `content.query_*` trace at `undertaking_catalyst` names `judgedAtLocationId` and `judgedAt` (`resolution_anchor` / `target_location`). A catalyst still withers only when *neither* place is one the family accepts (a ring founded at a fort by an actor who stays at the fort). **Every family carries a member a hamlet accepts (THR-1515)** — `cell.create.location` *founds* a hamlet (`UNDERTAKING_DEFAULT_LOCATION_SUBTYPE`), so a family with no hamlet member rejected the settlement the work itself made; `undertakingCellCatalysts.test.ts` holds that for hamlet, town, city and capital, off the constant. The wild (`wilderness`, the transient Location a mortal in open country stands at) is still accepted by no family. The spawned action now carries `spawnedFromSeedId` on every seed spawn, the durable record once the 2000-entry trace ring has evicted the `content.query_resolved`.
