...
- ✅ 2026-05-09: THR-391 shipped — `scripts/session-precheck.ts` now emits `freshness=` branch-staleness fingerprinting against `origin/main` (including behind/stale-branch/detached/unknown states), covered by new deterministic tests and enforced by a new Session Workflow callout rule in `CLAUDE.md`.
