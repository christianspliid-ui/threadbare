...
All SVG/canvas-based components (hex zoom, breadcrumbs) and the Three.js hex map (HexMapV2) extract every magic number into module-level typed constant objects. The sole hex map renderer is HexMapV2 (Three.js InstancedMesh, `?view=game`).
