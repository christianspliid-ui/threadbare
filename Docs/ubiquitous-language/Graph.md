...
**The Areas partition the land** (THR-1155, 2026-09-10). Every land hex belongs to exactly one, stamped at worldgen on `tile.regionId` from the one detector (`detectRegionsBorderCost`), whose nearest-cluster fill makes the cover total. That is what lets an Area be *acted on* rather than only read: an effect scoped `region` resolves to its real members, a reference of kind `area` routes to it, and the map draws a projection of the same nodes (`areaProjection`) instead of a partition of its own.
