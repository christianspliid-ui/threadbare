...
**Aliases:** the `spotlightTier` node property (engine); ambient · notable · spotlight; the attention budget; the deciding tier
**Also see:** `[[Agent]]`, `[[Ambition]]`, `[[Undertaking]]`, `[[Thread]]`, `[[Retinue]]`, `[[Chronicle Entry]]`
**Status:** canonical (seated by THR-1348 under delegated seating, 2026-09-22; veto retained)

How much of a mortal the world simulates in full. Three tiers, in order: **ambient** (a face in the crowd — no individual AI), **notable** (named and tracked, promoted by importance), **spotlight** (a fully living individual with their own values, ambitions and choices). **Only spotlight mortals run the decision loop** (`isAutonomousDecisionActor`, shared by the loop and its census), so an ambition held below the spotlight is never carried to the board — it is *silenced*, and `npm run census:reachability` reports it as such. A node with no tier reads as spotlight; that default is pinned and is not to be flipped — mint sites stamp the tier instead.

**Attention follows ambition** (THR-1348, Christian's ruling 2026-09-10). The tier's headcount is the intended attention budget and is not widened. Instead, **an ambition whose template carries a strategic profile pulls its holder into the spotlight** at assignment — the world's builders become the people the player can watch — and, to hold the deciding population flat, **the least-recently-witnessed spotlight mortal with no strategic ambition steps back to notable**. A mortal is *witnessed* when their encounter resolves (`lastWitnessedTick`); one never witnessed steps back first. **A threaded mortal — followed or retinue, any court position but dormant — is never demoted**, nor is the avatar, a mortal commanding an army or seated as a faction's leader or commander, a mortal mid-encounter or mid-undertaking, or one who was themselves pulled. A mortal is pulled **once** (`spotlightPulledTick`); when nobody can step back the pull may run net-additive up to an allowance scaled to the attention budget (`SPOTLIGHT_AMBITION_PULL_OVERFLOW_SHARE` of the deciding population, capped by `SPOTLIGHT_AMBITION_PULL_MAX` — one on a small world, two on a medium one), past which it is refused and the holder stays silenced. Demotion strips nothing the promotion minted. The pull writes one `[[Chronicle Entry]]` (*sets their mind to …*); a demotion writes none.

**Unwatched builders step back** (THR-1523). When no mortal without a strategic ambition can make room, a spotlight **builder** (a strategic-ambition holder, pulled or not) may step back once they are **unwatched**: not *witnessed* **and** no progress on any of their undertakings for `SPOTLIGHT_UNWATCHED_BUILDER_TICKS` (six days), counted from the latest of their witness, their own pull, their work's progress and tick 0. *Unwatched* is not a synonym of *not witnessed*: a builder whose work advances off-screen is not unwatched. A builder who is travelling, **followed** (the Follow button) or threaded never steps back; at most `SPOTLIGHT_UNWATCHED_SWAPS_PER_WINDOW` step back per day. A mortal you kindled is treated like any pulled mortal — six days from the pull, extended by any progress, kept by following them. The stepped-back builder keeps their ambition. The retained dead hold no slot. Behind `SPOTLIGHT_UNWATCHED_BUILDERS_ENABLED`; the ledger's `unwatchedDemotions` and the trace's `demotedReason` name each one.

Code anchors: `src/types/npc.ts` (`SpotlightTier`), `src/engine/npcGraduation.ts` (`hydrateToTier`, `demoteToTier`, `phaseNpcGraduation`), `src/engine/spotlightPull.ts` (`pullHolderIntoSpotlight`, `demotionCandidates`, `readSpotlightLedger`), `src/engine/strategicKindReachability.ts`, `src/data/agent-behavior-constants.ts` (`SPOTLIGHT_AMBITION_PULL_*`, `SPOTLIGHT_WITNESS_WINDOW_TICKS`), trace `spotlight_pull`, `window.__DEBUG.getSpotlightLedger`, CLI `spotlight`.
...
**Upkeep is live (THR-1287, shipped 2026-09-10).** A completed `use` (the harvest) or `change:raise` on the held Location resets `neglectTicks` and takes back `STRATEGIC_CONTROL_RENEWAL_RECOVERY` of the degradation — `renewControlStance` in `src/engine/strategicActionLifecycle.ts`. A hold nobody works still collapses on the fixed clock. *(The earlier reading of this paragraph — "upkeep is the live gap" — described the state before that ticket landed; the word was seated on what the model means, so the entry did not move.)*

**A hold on a Realm's ground opens a standing with that Realm (THR-1448, shipped 2026-09-22).** The keeper is, in the crown's eyes, the one who keeps that town: the hold opens a `member_of` edge with the Realm whose *ground* the town sits on (the political map's `hexRealmId`, never the town's holder — a claimable town has no faction edge by rule), seeded inside **subject**, the ladder's second rung. From there the rank climbs or fades by the court's own work, while the town's business keeps coming to the keeper's door whatever the rank — content carrying `requiresHold` is gated on the hold, never on the rank, and is supplied past the rank's `encounterAccess` allowlist. The standing is **read from the hold** (`holdStanding`, a pure reading over the stances and the map; no field on the stance) and ends with it; the membership it opened outlives it and fades by the court's own decay. A hold in unclaimed wilds opens no standing — a hold in the wilds is just a hold. The player word is the ladder's (*a hold makes you a subject of the Realm*; the sheet reads *keeps Ashford for the Realm of the Vael · grip firm*); **position** is not used on a surface — it names the divine `[[Court Position]]`. The grip is a word (*firm · slipping · failing*, `HOLD_GRIP_WORDS`), never the number.

Code anchors: `src/types/strategicAction.ts` (`StrategicControlState`), `src/types/graph.ts` (`'controls'`), `src/engine/undertakingMotive.ts` (`resolveTargetOwners` reads `controls` first when asking who holds a target), `src/engine/holdStanding.ts` (`holdStanding`, `heldTownAffinity`, `gripWord`), `src/engine/strategicActionLifecycle.ts` (`reconcileHoldStandings`, the one write), `src/types/unifiedAction.ts` (`requiresHold`).
