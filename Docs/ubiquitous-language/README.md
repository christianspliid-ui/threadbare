...
- **[Duel](./Agents.md#duel)** — a fight in which both sides are mortals who roll (agent mode); *Old Blood* is the systemic one, spawned when a grudge boils over — not the single-roller `duel` encounters
...
- **[hold](./Agents.md#hold)** — a place kept by a mortal's ongoing commitment (`controls` + `StrategicControlState`), renewed by working it; never the `owns` edge, which is a Freehold
- **[Agreement](./Agents.md#agreement)** — a standing obligation as an *edge*, never a node: a favour (`owes_favor`) or a mark (`knows_secret_of`); the leverage economy, held from opposite ends
- **[Means](./Agents.md#means)** — the player word for the engine's `wealth`; always a tier word, never a number (Laws 4/13)
- **[Motive gate](./Agents.md#motive-gate)** — the licence a destroy verb needs: grudge, rivalry, contested ambition or faction war toward the owner; opt-in, and an unowned target is refused
...
- **[Motive Receipt](./Prose.md#motive-receipt)** — the decision-time causal record behind an encounter choice; prose *renders* it and never reconstructs it, so every surface gives one answer
...
*v1.17 — 8 shards, **186 terms: 178 canonical, 5 deprecated, 1 rejected, 1 proposed, 1 retired** (THR-1457 seated the eight delegated words: `Agents#hold`, `Agents#agreement`, `Agents#means`, `Agents#motive-gate`, `Encounters#cast-verb`, `Encounters#forecast-tier`, `Encounters#composition-contract`, `Prose#motive-receipt`). Per-shard census: Agents 56, Encounters 40, Graph 20, Process 17, Traits 16, Coordination 14, Prose 14, Cosmology 9. **The v1.16 tally was stale by 1** — it read `177 / 169 canonical` with `Agents 51` while the shards held 178 headings (`Agents 52`), because `Agents#realm` was seated by THR-1155 without the recount this very line mandates; that is the **fifth** time the count has been caught stale, and every one of the five was an addition seated without recounting. **Recount after any term change** with `grep -hoE "^\*\*Status:\*\* [A-Za-z]+" Docs/ubiquitous-language/*.md | sort | uniq -c` — count every `**Status:**` line, not only the statuses you expect (twice caught stale by an unexpected status). The `-oE … [A-Za-z]+` tail matches the **leading status word only**, so the annotated form and the bare form tally as one row (THR-1470; the previous `grep -h` recount reported `canonical (seated by …)` and bare `canonical` as two rows and told you to strip the notes by hand). **The annotated form is the recommended one** — `**Status:** canonical (seated 2026-09-03, THR-1390)` records when and why a term was seated, where a bare `canonical` records nothing; the `?view=ul` dashboard parses the leading word as the status and renders the parenthetical as a provenance note beside it, and this recount reads it the same way. When cross-checking against `### ` headings, exclude this README — it carries its own section headers. Coverage expands via the propose-new-term flow. UL wins on terminology disagreements.*
