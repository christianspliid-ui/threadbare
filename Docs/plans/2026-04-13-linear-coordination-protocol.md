...
A **park** is `state: In Dev` ∧ `assignee: null`, carrying the `Parked` label. It is not a bookkeeping preference — it is the *only* shape `keep-work-flowing-cc` matches to surface a question under `## Needs Christian`, so a park that decays stops the ask reaching the one surface he reads, silently and indefinitely.

**Park human-gated tickets; do not consolidate them into a queue slot.** Consolidation is right for tickets that are individually claimable. A ticket blocked on *"a human must be present"* is not claimable by any unattended run, and leaving it in `Ready for Dev` makes the queue *look* healthy while nothing in it can move: THR-1133 batched four human-gated pixel passes into one queue item and drew six days (2026-08-16 → 08-22) of hourly scan-and-exit, while `keep-work-flowing-cc` judged queue health on count and reported the queue non-empty throughout. If the gate is "Christian must be at a keyboard", the destination is a park, not the queue — and per THR-846 the park comment must name the lane that reads it.

**A `Parked` issue found in `Ready for Dev` is restored, not claimed and not bounced.** Its queue membership is an artifact of the sweep defect above, not a decision. `save_issue(id, state: "In Dev", assignee: null, <one other field at its current value>)`, then `get_issue` to verify both keys landed — the bundled field matters, since a sole-field `assignee: null` can silently no-op (impediment #380). The executable form of this rule lives in `.claude/skills/pull-work/SKILL.md`.

**Before writing an `assignee` on a ticket that is not yours, prove nobody is shipping it.** Re-run `gh pr list --state open` and look for a branch matching `*<issue-id>*` or the id in a PR body. `attachments: []` from `get_issue` is *no evidence*, never evidence of no PR — the attachment can lag the PR by minutes. Quoting an empty array as proof cost one lane another session's assignee, stripped twice mid-ship, plus a false finding published to `main` (impediment #755).
