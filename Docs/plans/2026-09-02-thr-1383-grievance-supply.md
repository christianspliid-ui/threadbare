...
## Executor amendments (2026-09-02, same-day execution)

Four things the plan got wrong or did not know, corrected at execution and recorded here so the plan and the code do not disagree:

1. **A grave harm is never a coin flip.** The plan kept `MINT_BASE_CHANCE` (0.6) in front of the funnel for every offer. With the passes tiling, a harm is offered exactly once, so a 40% miss loses it forever — and the first organic run on a seed with harms (123) lost all three grave harms in its first window exactly that way. The roll now applies to soft drives only; an offer carrying a grievance-class candidate at or above `GRIEVANCE_DISPLACE_MIN_MAGNITUDE` always reaches the temperament funnel. Deterministic (the roll is skipped, not re-seeded); a veto is a one-line revert of the `graveHarm` branch in `mintAmbitionsFromEvents`.
2. **The window is half-open on the far side.** `(tick − window, tick]` rather than inclusive: with the window equal to the cadence, an inclusive lower bound hands the event that landed on a pass tick to the next pass as well. The Done-when's "offered exactly once" is only true with this.
3. **The arc-panel word has no home.** `agentDetail.ts` excludes abandoned ambitions from the arc panel by THR-721's decision (biography, not a failure ledger). The word lives on the chronicle line the plan already specified; `abandonedReason` stays on the edge for the trace and the CLI. No `src/components` file changed.
4. **The four trace registration sites are all in `src/types/trace.ts`.** Union member, `TRACE_CATEGORIES`, the payload interface, and the `TraceEntry` union — the "viewer label" and "CLI filter" sites the plan named do not exist for `grievance_transition` either (the DebugPanel seeds its filter from `TRACE_CATEGORIES`). The typecheck ratchet catches the fourth when it is missed (it did: +1 until the `TraceEntry` member was added).

**Acceptance outcome.** The 300-tick runs on seeds 42 and 99 mint zero — because under the decision-board cutover and the per-mortal cap that landed earlier the same day, **no culprit-carrying harm is emitted on either seed in 300 ticks** (thirteen that morning under contest B). The kill criterion tripped upstream of every rule in this plan; filed as [THR-1388](https://linear.app/threadbare/issue/THR-1388). The mechanism is accepted on the eleven falsified guards and on seed 123, where the loop turns twice in 150 ticks.
