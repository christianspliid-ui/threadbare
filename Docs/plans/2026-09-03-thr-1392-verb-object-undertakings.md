...
| Control (`claim_control` execution mode, `StrategicControlState`, upkeep, degradation, collapse) | 🟢 ACTIVE | **untouched** — control is not a completion verb; it is its own execution mode that never reaches the mutation switch (`strategicActionLifecycle.ts:488`). The six `control` templates stay on that mode; a `controls` edge is an *outcome* a later plan may register as an object type |
...
interface UndertakingObjectType {
  ...
  verbs: Partial<Record<UndertakingVerb, ObjectVerbSemantic>>;  // what found / improve / use / seize / undo / survey do to THIS type
...
**Verbs.** `UndertakingVerb = 'found' | 'improve' | 'use' | 'seize' | 'undo' | 'survey'`: `create` → found, `change` → improve **or use**, `destroy` → undo, `gather_info` → survey, plus **seize** (transfer of ownership, which the world has as `transferHolding` and no template ever offered). **`use`** is the self-spend the kind rows already distinguished (*"a self-spend is a use, not a counter"* — the mark row): pressing a mark, following a chart, burning one's own mark — own object only, no gate, no harm. **`hold` is not a verb**: the six `control` templates run `executionMode: 'claim_control'`, which yields a `StrategicControlState` and never reaches the mutation layer; they stay on that mode, untouched. `undo` and `seize` carry the motive gate and a harm class by rule; `undo` and `seize` require `ownership: 'other'`, `use` requires `'own'`, and the cross-family rule on the rows (a kind's destroy comes from another family) is kept as a **per-type rule** on `undo` so a holder cannot undo their own object through the counter-play verb.

**Execution mode by verb.** `survey` and `use` default to `instant` (the 19 instant templates today are 9 gather_info, 5 destroy, 5 change; the 5 instant destroys become checkpointed `undo` deliberately — an instant destroy was the shape that shipped five hollow ones); `found`, `improve`, `seize`, `undo` are multi-tick with `UNDERTAKING_VERB_DURATION`. A cell override may pin the mode.
...
| `recruit_warband`, `reinforce_warband`, `suborn_warband`, `establish_spy_network`, `extend_reach`, `sever_network` | `found / improve / seize / undo × company` (`GroupKind` company and network — an order is a faction, `foundFaction`, so `found_order` → `found × faction`) |
...
| 6 `control` (`claim_territory`, `maintain_*`, `police_doctrine`, `guard_knowledge`) | **untouched** — `claim_control` is its own execution mode, not a completion verb |
...
N/A — no encounter content; the harm an `undo` or `seize` emits feeds the catalyst encounters the shipped templates already name, and those ids move onto the cell overrides that keep them.
...
|----------|---------|---------|
| `UNDERTAKING_VERBS` | `['found','improve','use','seize','undo','survey']` | the closed verb set |
| `UNDERTAKING_VERB_DIFFICULTY[verb][tier]` | found 0.45/0.5/0.55 · improve 0.4/0.45/0.5 · use 0.35/0.4/0.45 · seize 0.55/0.6/0.6 · undo 0.5/0.55/0.6 · survey 0.35/0.4/0.45 | checkpoint difficulty by verb and object tier — inside the existing tier bands |
| `UNDERTAKING_VERB_PAYOFF[verb][tier]` | inside `UNDERTAKING_TIER_PAYOFF_BANDS` per tier; undo and seize at the band's top | the board's payoff by verb and object tier |
| `UNDERTAKING_VERB_DURATION[verb][tier]` | found/improve/seize/undo 4/6/8 checkpoints; use and survey instant by default | project length |
| `MOTIVE_GATED_VERBS` | `['seize','undo']` | which verbs require a quarrel |
| `OWNERSHIP_BY_VERB` | found `any` · improve `own` · use `own` · seize `other` · undo `other` · survey `any` | the cell's default ownership rule |
...
| `grievance-reaches-the-mortal-sheet`, the THR-1388 covet rivalry | preserve — `undo`/`seize` emit the same harm classes |
| holdings `owns` single writer | extend — `seize × attachment/room/settlement` is the first production caller of `transferHolding` |
...
- [ ] This plan **does** change a rule of play: the undertaking verb set — stated in `Docs/canon/undertakings.md:11` as "a thing built, changed, taken or undone" — becomes found / improve / use / seize / undo / survey (control keeps its own mode), and "a kind is not a kind until it can be undone" becomes "an object type is registered with its undo". The rulebook's undertaking rules live in `Docs/canon/rulebook.md` **§10.7 The Reactive Loop** and **§10.8 Following, Moments and the Calling** (§10.6 is Reputation — an earlier draft of this line mis-cited it; the judge caught it); those two sections and `Docs/canon/undertakings.md` are updated in the same PR as the registry.
