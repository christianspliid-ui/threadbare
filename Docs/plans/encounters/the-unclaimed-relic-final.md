...
> Scale: short (Single Test) | Slug: the-unclaimed-relic | Pass: final
> Date: 2026-08-24 | Pipeline version: 2.0 (line: Encounter Factory v3, nudge-native, THR-1045 Composition Contract)
...
| Pass | Verdict | Notes |
|------|---------|-------|
| Draft | Complete | Design-block-first single-step `stone` endurance test: a relic in the open, a cold that stopped everyone before, one other claimant at the ring. Hook drift (haunting → faceless cold) judged and ratified in-draft. |
| Editorial | PASS WITH REVISIONS | Nineteen ids re-verified with no rot; the draft's seam table checked only 2 of 5 seam classes and both new checks it ran turned up hits. Five REVISE-trigger instances across four triggers (a detector hit, a seam echo appearing twice, class scenery, a two-reading collision) — all sentence-level, all repaired in the revised file. Three judgment calls independently re-derived and ratified, one self-audit overclaim (anchor count) and one gate-question overclaim (13, stance choice) corrected for honesty. |
| Systems | READY FOR IMPLEMENTATION | Every id, effect kind, field name and constant re-verified directly against live source (not against either prior pass's citations). All nine Composition Contract blocks pass under direct re-execution of the gate's own logic. One citation imprecision found and corrected (stronghold reuse-role sourcing cites the wrong roster keys, though the underlying conclusion holds and materializing delivery makes it non-load-bearing regardless). One Pass-2 flag resolved outright (step-outcome metadata is already counted by every contract block). One flag correctly carried forward as a pre-implementation check, not a blocker (confirm `$target` binds to a location for this template family before merge). No missing primitives; no new engine, type, or catalog work required. |

### Caveats / Blockers

None blocking. One pre-implementation verification step:

- **Confirm `$target` binds to a location for `encounter.border.*`** before merging the `success` band's location-anchored `condition_attachment`/chip. Precedent is live in the vertical slice but unconfirmed for this template family. If it does not bind to a location, the fix is to bind the location through the support bundle rather than softening the chip (both prior passes and this one agree on the remedy).

One documentation-only correction, non-blocking:

- The packet's § 11 support-bundle table cites `LOCATION_ROLE_ROSTERS.castle` and an implied `fort:` roster row as the source of the stronghold class's `scout`/`mercenary` reuse capability. The live NPC-seeding pipeline (`src/engine/npcSeeding.ts`'s `SUBTYPE_TO_ROSTER_KEY`) actually maps `castle → capital` roster and `fort → military_outpost` roster; there is no `fort` key in `LOCATION_ROLE_ROSTERS` at all. Both real rosters still carry the claimed roles (`military_outpost` has `scout` and `mercenary`; `capital` has `mercenary`), so the conclusion is unaffected, and the encounter's correctness does not depend on it — the support bundle's delivery mode is `lazy-materialize-on-trigger`, which spawns the claimant regardless of whether any live NPC is available to reuse. Full detail in the systems audit § 1.

### Editorial Notes Summary

Editorial ran the full checklist against the draft and found the packet's research strong (nineteen ids re-verified, no rot) but its seam-echo pass incomplete: the draft's own seam table checked only `opening → spine` and `spine → afterimages`, and stopped before the two seam classes where surfaces actually render together — base band text next to a card fragment in the same paragraph, and a step's afterimage seconds before its aftermath overview. Both classes were checked for the first time in editorial and both had hits, the worst being the `failure` band's afterimage and overview reading as the same sentence with one word changed. Five REVISE-trigger instances (a `something`-class detector hit, the seam-echo class recurring twice, one instance of setting-neutral prose naming "the room" as class scenery, and one two-reading collision on a reused word) were found and repaired in the revised file, all sentence-level. All three of the draft's flagged judgment calls (hook drift, no trait card, one reaction per band) were independently re-derived from source rather than rubber-stamped, and all three were ratified — but two of the packet's own self-reported claims were corrected for honesty rather than left standing: the self-audit's anchor-kind count (claimed four, is actually three) and gate question 13's answer (claimed the god's varying posture across bands is a player-facing stance choice; corrected to note the player is offered exactly one reaction per band and therefore never elects a stance, though the structure and its Law 56 justification are sound). Full detail: `Docs/plans/encounters/the-unclaimed-relic-editorial.md`.

### Implementation File Map

**Create:**
- `src/data/encounters/the-unclaimed-relic.ts` — the template file, exporting `THE_UNCLAIMED_RELIC_TEMPLATE: UnifiedActionTemplate`. Transcribes the Encounter Packet below field-for-field: `openings` (§ 6), the setting-neutral spine as `steps[0].narrativeTemplate`, `steps[0]` (reach, difficulty, duration, failBehavior, purposeLine, five afterimages, `successMetadata`/`failureMetadata`, `nudges` [6 cards, § 8], `traitVariants`, § 10), `supportBundle` (§ 11), `aftermathConfig` (§ 12), and the template-level fields (§ 16: `id`, `name`, `reach`, `rarityTier`, `intrinsicTier`, `crudType`, `scale`, `apCost`, `actorAffinities`, `motivations`, `consequenceDraw`, `description`).
- `src/data/encounters/__tests__/the-unclaimed-relic.test.ts` — structural test in the shape of `road-ambush.test.ts` / `flawed-steel.test.ts`: template shape (1 plain step, no branch node), aftermath variant resolution for all five authored bands, support bundle actor resolution for the `claimant` key, `traitVariants` resolution for `trait.core.core_humility.vice`, and a direct `checkCompositionContract(THE_UNCLAIMED_RELIC_TEMPLATE)` assertion of zero violations.

**Modify:**
- `src/data/unified-action-templates.ts` — three edits, confirmed against the live file in this pass:
  1. Add `import { THE_UNCLAIMED_RELIC_TEMPLATE } from './encounters/the-unclaimed-relic';` near the existing encounter-file import block (alongside e.g. `import { ROAD_AMBUSH_TEMPLATE } from './encounters/road-ambush';` at line 191).
  2. Add `THE_UNCLAIMED_RELIC_TEMPLATE,` to the first registration array (alongside `ROAD_AMBUSH_TEMPLATE` around line 5590).
  3. Add `THE_UNCLAIMED_RELIC_TEMPLATE,` to the second registration array (alongside `ROAD_AMBUSH_TEMPLATE` again around line 5678).

**Not required:** no `src/engine/**` changes, no `src/types/**` changes, no new catalog entries — every effect kind, field name, and referenced id this packet authors already exists live. No Design Reference Wiki page update expected (new content, not a core-system behavior change) — confirm with `npm run check:wiki-freshness` at implementation time as a matter of course.

Full block-by-block Composition Contract re-verification, chip-backing and chip-anchor traces, and the disposition of every flag: `Docs/plans/encounters/the-unclaimed-relic-systems.md`.
...
> **Pass 2 ruling on the drift (ratified, with the load-bearing reason added).** The drift is legitimate — the spec is explicit that the hook is a starting point and not a contract, and that only the *roll* is mandatory to record. It also improved the encounter, for a reason worth stating: **a face turns the scene into a negotiation, and a negotiation is not what `stone` tests.** Give the thing a will and the encounter immediately wants to be `veil` (read it), `heart` (talk to it) or `iron` (fight it) — all different encounters, two of them already allocated elsewhere in this batch. The cold is the only antagonist under which *how long can a pair of hands stay closed* is the real question. The haunting's unease survives without the face, carried by `fallback.overview`: frost lying at a radius nobody set, identical every day.
...
|---|---|---|
| `hook.haunted_relic` | The premise's shape — the prize and the reason nobody has it are the same fact. | The haunting itself. A named revenant reads at a `ruin` and nowhere else; a cold with a radius reads at all four classes, and refuses the negotiation a face would invite. |
| Swollen Ford exemplar | Structure only: the design-block header, the setting-neutral spine with the cast introduced in it, band fragments carrying the god's hand, the `byOutcome`-on-`fallback` pattern, and the two-Boost pattern (same verb, different physics). | Its images. The seam check below explicitly hunts for exemplar echoes — two card fragments were rewritten because they had drifted into the exemplar's *"The X held. The Y did not"* and *"There was one more pull in them"* house mannerisms. Pass 2 re-checked both against the fixture. |
...
**One further contradiction repaired at Pass 2:** the Cache `near_miss` fragment read *"The tongs found the ring of it"*, using *ring* for a loop on the relic three sentences after the spine established *ring* as the ring of dropped gear. Two referents, one word, one scene. Rewritten.
...
*"Do they come out of the cold holding it, or do they let go and join the ring of people who could not?"* A good outcome: the relic in their possessions and their hands working again by evening. A bad outcome: the relic still sitting there, their own pack on the ground with the rest, a fright they carry off the hex, and — at the bottom — being dragged out of the cold by the one person who was decent enough to do it.
...
> Nobody who has come here has carried it out. That is the whole of what is known about it. Take it and it is theirs, to sell or to keep. Leave it, and the road is a road again by nightfall and the relic waits for whoever comes next.

> *Pass-2 note.* The closing noun was *the thing*; a natural indefinite, legal in scene class and firing no detector, but the relic is named one sentence earlier and the concrete noun is plainer (plainness move 2). Traded. The engage/decline sentences stay: those are the stake the initiation exists to state (checklist C9, D12), not pass/fail band framing, so trigger 25 does not reach them.

### Seam checks (rejection trigger 22 — run explicitly, at **every** seam where two surfaces render together)

The draft's table checked `opening → spine` and `spine → afterimages` and stopped. Those are the seams a drafting agent thinks about. The two it did not check are the two where surfaces render *together* — base band text and its fragments land in the same paragraph, and a step afterimage and its aftermath overview land seconds apart, in order. Both classes are now checked, and both had hits.
...
| **base band text → card fragment** *(class not previously checked)* | **HIT, repaired.** `narrativeTemplates.failure` reads *"The relic sits where it sat"* and the Cache `failure` fragment read *"and the relic sat where it sat"* — near-identical phrasing rendering **in the same paragraph**, one tense apart. The fragment was rewritten to pay the tongs' `breakable` loss condition instead. ✓ |
| **afterimage → band overview** *(class not previously checked)* | **HIT ×5, repaired.** All five overviews opened by restating their own afterimage; at `failure` the two were the same sentence with one word changed (*dropped* / *went*). All five rewritten to land somewhere the step's last image did not. Per-band detail in § 12. ✓ |
| **chip → chip, within one band** *(class not previously checked)* | **HIT ×2, repaired.** On `success_at_cost` both chips carried `causeClause` *"the iron kept what it touched"*; on `critical_failure` both carried *"They had to be dragged clear of it"* — verbatim, rendering side by side. Differentiated. ✓ |
| corpus echo (exemplar) | Two card fragments were caught reproducing exemplar mannerisms and rewritten — see § 8, Boost (core) `failure` and Balm `failure`. Pass 2 re-verified both against `swollen-ford-exemplar.ts`. ✓ |
...
**Card faces are the library's own authored faces.** `title` and `quote` are taken verbatim from `CARD_CONTENT` in `src/data/nudge-card-library.ts`, so the face a player learns on this card is the same face everywhere the member deals. Scene grounding lives in the band fragments and the prose, never on the face. **Pass 2 verified all six against source, character for character** — `card.boost.core` (`:557`), `card.insurance.signature.order` (`:587`), `card.undertow.signature.darkness` (`:599`), `card.cache.signature.matter` (`:607`), `card.boost.signature.energy` (`:611`), `card.balm.signature.life` (`:615`).
...
> **Pass-2 repairs, both flagged in § 6's seam table.** The `near_miss` fragment read *"The tongs found the ring of it and held"* — a second, different *ring* three sentences after the spine established the ring of dropped gear, which forces a reread (trigger 29). The `failure` fragment ended *"and the relic sat where it sat"*, near-identical to `narrativeTemplates.failure`'s *"The relic sits where it sat"*, which renders in the same paragraph (trigger 22). The new `failure` line pays the tongs' own `breakable` loss condition instead, which the grant already carries.
...
**Liveness.** `trait.condition.terrified` is a live `CONDITION_TRAIT_DEFINITIONS` entry (`src/data/condition-trait-content.ts:175`), which is the exact set `validateNudgeGrantRefs` checks `remove_condition.conditionTraitId` against.
...
> **Pass-2 repair.** The `success` fragment read *"Unafraid, they took their time about it…"* — an adjective phrase ahead of the subject, which is the inversion plainness move 1 names by example. Rewritten subject-first, keeping the closer intact.
...
> **Pass-2 repair (trigger 18).** The `critical_success` fragment read *"…as if the cold were not **in the room**."* Band fragments are setting-neutral outcome prose rendered under all four openings, and there is no room at a `wayside` hollow or on a `battlefield` slope. This is the specific way a four-class envelope fails — not in the openings, which get the attention, but in the outcome prose written last. Every other fragment, afterimage, narrative template, overview and chip detail was re-swept for class scenery; this was the only instance.

**Why two Boosts do not answer the same question.** Card 1 buys **duration** — it holds the grip closed at the moment it would open, and it is the sphere-less floor every god can afford. Card 6 buys **a burst against the specific opposition** — heat, aimed at cold, at one instant, and it is priced and gated as a sphere card. A god who holds `energy` chooses between paying more for a bigger single moment and paying less for a longer one. That is a decision, not two names for the same certainty, and it is the exemplar's own Steady Breath / A Break of Light pattern: same verb, different physics.
...
| Every `libraryCardId` set | 6/6 ✓ (verified against source at Pass 2) |
| Every `imageTag` resolves to an `ENCOUNTER_IMAGE_LIBRARY` row | 6/6 ✓ (verified at Pass 2 — see § 13) |
...
**Vagueness sweep, outcome class (evasive **and** natural indefinites at zero) — re-run at Pass 2, with one hit found and repaired.**

The draft asserted a clean sweep across *"all five afterimages, both `narrativeTemplates`, every band fragment, every `overview`, every `detail` and every `causeClause`."* That assertion was false at one site, and it named the offending word first in its own banned list:

> `success_at_cost` overview — *"…going through the dropped packs for **something** clean to wrap a hand in."*

`something` is the outcome-hider: **evasive, not merely indefinite, and therefore banned in every field class** — and an aftermath overview is outcome class besides. `check:encounter` would have failed on it. Repaired to *"for a clean rag"* (§ 12).

The sweep was then re-run by hand across every outcome-class surface. That was the only hit. Confirmed absent everywhere: `something` · `someone` · `somewhere` · `things` · `thing` · `stuff` · `way` · `ways` · `nothing` · `anything` · `whatever`, plus every hedge and nominalised placeholder. Two fragments and one band overview had already been rewritten during drafting to reach that state — recorded at the point of each. Intensifiers: none (`very` was removed from a reaction intent).

**Scene class (evasive only): clean.** `Nothing has been burned in the pit`, `used by nobody`, `Someone chalked a line`, `nothing crosses it`, `Someone else is already here`, `Nobody who has come here` are all natural indefinites in scene setup, which THR-899 explicitly legalized. No contortion is written around them.

**Divine outcome-authorship: zero, every field class.** No decision verb followed by `whether`/`what`/`which`/`who`/`if` + a clause, and no bare "the outcome", in any authored field. § 5's framing was additionally softened from *decide* to *press on* — it passed the detector either way (`how` is deliberately outside the match set), but rule 5b's whole point is that the god sways rather than authors, and packet documentation should model the verb it will be read for.

**Abstraction-as-subject spot check (by hand):** subjects across the packet are `a cart track`, `half a hall`, `the field`, `the fort's yard`, `the relic`, `the air`, `packs and a dropped boot`, `their grip`, `the tongs`, `the heat`, `{cast:claimant}`. All concrete. *"The cold gave it up"* and *"The cold won the argument"* are the closest to abstract, but the cold is this scene's antagonist with a measured radius — a concrete subject doing concrete work.
...
**Liveness.** `trait.core.core_humility.vice` is built by `CORE_TRAIT_DEFINITIONS` (`src/data/core-trait-content.ts:106`) from the canonical `CORE_CONTINUA` registry — continuum `core_humility` at `src/types/coreRegistry.ts:151`, vice pole word **Proud** (`:153`), flavor *"Stands at the center of their own world, and corrects the horizon."* (`core-trait-content.ts:59`). Ids are built as `` `trait.core.${continuumId}.${pole}` `` (`:72`). It is a seeded definition, so `validateTraitRefs()` does not report it dead. The full node id is used, which is the form least likely to rot under the ANY-of match. **Pass 2 re-verified all four citations against source.**
...
**3 · Trait-only nudge?** **No.** The batch design's card-type budget for this hand is `cache` / `insurance` / `boost` (≤2) / `undertow` / `balm` (`border-perils-batch-design.md:150`), and `trait_card` is allocated to rows **2** and **5** (`:140`, `:171`). Adding one here would break the batch's anti-convergence allocation, which is a shared constraint rather than a local preference. The variant therefore carries no `addNudgeIds`.

**4 · Trait fragment?** **No, and the schema is why.** `TraitVariant` (`src/types/unifiedAction.ts:1630`) carries exactly five fields — `traitId`, `forecastDelta`, `difficultyDelta`, `factorLine`, `addNudgeIds` — and **no band-prose surface**. A trait-keyed band fragment is only reachable through `addNudgeIds` unlocking a card whose own `bandProse` then renders. With no trait card there is literally nothing for a trait fragment to key on. The variant's factor line is where the hook becomes visible to the player, and at one step that is the whole of the available surface. Recorded rather than left silent.
...
**Class-honesty across all four declared classes** — the rule this bundle has to satisfy, checked against `LOCATION_ROLE_ROSTERS` (`src/types/npc.ts`) and against the shipped per-class defaults in `DEFAULT_SETTING_SUPPORT_BUNDLES` (`src/data/default-support-bundles.ts`):

| Class | Rosters at its subtypes | Which reuse role can bind |
|---|---|---|
| `ruin` | none of the ruin subtypes carries a roster; the shipped `ruin` default reuses `scout` / `ranger` (`default-support-bundles.ts:518`) | `scout`, `ranger` |
| `wayside` | `wilderness` seeds `hermit`, `ranger`, `wanderer`, `hunter`, `hexer`, `pilgrim` (`npc.ts:318`) | `ranger` |
| `stronghold` | `castle` seeds `noble`, `marshal`, `guard_captain`, `guard`, `steward`, `herald`, `spy`, `attendant`; `fort` seeds `scout` and `mercenary` | `scout`, `mercenary` |
| `battlefield` | `battleground` carries no roster; the shipped `battlefield` default declares two specs, the second reusing **`mercenary` / `quartermaster`** (`default-support-bundles.ts:586`) | `mercenary` |

Every class has at least one binder, and no role in the list is placeless at any of the four — a scout, a ranger and a mercenary are all people who turn up at a ruin, a battlefield, a wayside hollow and a fort. **These are not authorial guesses: all three are the shipped defaults' own reuse roles at these classes**, re-verified against source at Pass 2. The named counter-example the spec teaches (the exemplar's rural-honest, wayside-placeless *miller's boy*) is exactly the shape avoided here.

**Delivery is `lazy-materialize-on-trigger`, not `pre-seeded`,** because four of the five band aftermaths write a `bond_change` against `$cast:claimant`. A `pre-seeded` spec binds an existing NPC **or stays unresolved**, and an unresolved sentinel silently no-ops the effect — which would leave a BOND chip claiming state nothing wrote, a Law 56 violation arriving through the cast door. The claimant must exist for the consequence to be honest, so the spec materializes when nobody is standing there. `persistence: must-persist` for the same reason: a person the encounter's own consequence points at cannot be collected at scene end.
...
**Gender.** The claimant is never gendered. Every sentence is written around the pronoun — *"chafing warmth back into both hands"*, *"and that was decent"*, *"got a hand in their collar"* — because reuse binds whoever is standing there. Pass 2 read all ten sentences that touch the claimant and found no gendered pronoun.
...
No `changes` at variant level — every chip is band-scoped, so every chip is backed by a write that fires on the band it renders on. **One reaction per band, carrying that band's writes, which is the only structure under which a band chip is guaranteed backed.** Pass 2 verified the mechanism rather than the argument: `AftermathOutcomeOverride` (`src/types/unifiedAction.ts:1903–1913`) carries `changes?` and `reactions?` as *siblings*, so a band renders its chips regardless of which reaction the player picks. Two rival reactions would therefore make every chip conditional on the pick — the `shell_state`-over-empty-`effects` pathology Law 56 exists to kill. The alternative (chip only the step-metadata-backed consequences and leave the bond write unchipped) buys a reaction fork by hiding a real write from the player, which is worse. One reaction per band stands.

**Overview rewrites, all five bands (trigger 22).** Every overview below previously opened by restating its own step afterimage — at `failure` the two were the same sentence with one word changed. Those two surfaces render seconds apart, in order: the step's afterimage lands, the encounter resolves, the overview is the landing. Hearing the same sentence twice is the flattest possible ending. Each overview now lands somewhere the afterimage did not.
...
**overview**
> *The frost holds the shape it sat in, a clean black circle the other claimant will not step into. {cast:claimant} has not moved from the ring, and asks, plainly, how it was done.*

> *Rewritten.* Previously opened *"They lifted it out on the first reach…"* against the afterimage's *"They lifted it clean…"* — same subject, same verb, same opening clause. The overview now opens on what the relic left behind, which is the image the concept art is built on and the only place in the packet it appears in prose.
...
**overview**
> *Their hands stayed shut for an hour afterwards, and the other claimant worked the fingers open one at a time. That is a story now, and it will be told at the next fire.*

> *Rewritten.* Previously opened *"It came up on the third try and they got clear of the ring…"* — three collisions with the afterimage in two sentences (`got it up`/`It came up`, `got clear`/`got clear`, `hands dead to the wrist`/`hands stayed shut`). The new overview also does work the old one did not: it puts the claimant's hands on the agent, which is the reciprocal of the spine's image, and it motivates the `under_watch` write instead of merely announcing it.
...
| `causeClause` | *It came out of the ring on the third try* | *It was taken in front of a witness who will not keep quiet* |
...
> *`causeClause` repaired.* Read *"a witness who will not keep it"* — which parses two ways (keep the relic? keep quiet?) and made the reader stop. Trigger 29.
...
1. **The referent is a graph object.** `location` is an anchorable member of `anchor-catalog.generated.md` — status 📍 *named*, declared by `entityId` = the location node id, with **no `visualKind` member**, so the chip names the place and does not promise a click. `named` satisfies Law 56 exactly as `linked` does, and the catalog is explicit: *"Do not fold a chip merely because its anchor cannot be clicked."*
2. **A write backs it.** `trait.condition.location.under_watch` is a live location condition (`src/data/condition-trait-content.ts:323`, THR-1143) — *"Someone is keeping eyes on this place. Quiet work here is harder and more likely to be seen"* — with a duration entry at `:414`, and `condition_attachment` accepts `targetLocationId` for exactly this (`src/types/unifiedAction.ts:650–654`). The condition has real readers, so this is a state change the simulation acts on and not a sentence. With one reaction on this band, the write is unconditional on it.
...
**overview**
> *The iron kept a strip off both palms, and it did not bleed until the warmth came back. The other claimant is already going through the dropped packs for a clean rag.*

> *Rewritten, for two reasons.* It previously opened *"They came out of the ring holding it and left skin behind on the iron"* against the afterimage's *"They came out with it and left skin on the iron"* — near-verbatim (trigger 22). And it ended on *"something clean to wrap a hand in"* — `something` is the evasive outcome-hider, banned in every field class, and this was the packet's one detector hit (trigger 15). Both repaired in one pass.
...
| `causeClause` | *It came out, and the iron kept what it touched* | *Both palms came off the iron raw* |
...
> *`causeClause` differentiated.* Both chips on this band carried *"the iron kept what it touched"* verbatim, rendering side by side. The scar chip now names its own cause.
...
**overview**
> *Their own pack is on the ground now, three paces out, in the ring with the others. The other claimant did not remark on it, and that was decent.*

> *Rewritten, and this was the worst echo in the packet.* It previously read *"Their grip opened before the count ran out and the relic went back into its own frost"* against the afterimage's *"Their grip opened before the count ran out, and the relic dropped back into its own frost"* — the same sentence, one word changed, on the band a rarity-1 open-draw `stone` test produces most often. The new overview pays off the promise the design block made at question 7 instead: the agent has *joined* the ring, and the evidence of everybody else's failure now includes theirs. That is the encounter closing its own loop, which the restatement was occupying the space for.
...
**overview**
> *{cast:claimant} swore about it the whole way out, and was still swearing when the arms came unlocked. Afterwards the ring got a stride wider, and the two of them kept opposite sides of it.*

> *Rewritten.* It previously opened *"They went down beside it with their arms locked…"* against the afterimage's *"They went down beside it, and the cold had them until the other one hauled them clear"* — the opening clause verbatim, plus `hauled` repeated. The overview now starts after the rescue rather than replaying it, which is also where the band's actual consequence lives.
...
| `causeClause` | *They had to be dragged clear of it* | *A stranger had to put hands on them to get them out* |
...
> *`causeClause` differentiated.* Both chips carried *"They had to be dragged clear of it"* verbatim, rendering side by side.
...
`spawn_artifact` mints a real graph node — `type: 'artifact'`, a real node id, `properties.category` / `tier` / `tags` / `sourceEncounterId`, and a `possesses` edge to the agent — plus a chronicle event. That node is the "real object with a real id" the batch design asks for, and it is clickable through the artifact route (`visualKind: 'artifact'`, 🔗 *linked* in the anchor catalog).

**Why no `templateId`, deliberately. Pass 2 opened both source files and confirms all three facts.**

1. `spawn_artifact`'s `templateId` is resolved against the **graph** (`state.graph.getNode(effect.templateId)`), and `validateNudgeGrantRefs` checks it against `ARTIFACT_TEMPLATES` (`src/engine/nudgeGrantLiveness.ts:84` — `artifact: new Set(ARTIFACT_TEMPLATES.map((t) => t.id))`). That catalog is `src/data/artifact-templates.ts:45` and holds **exactly three entries, all `tier: 4`** — `worldforge_anvil` (`:48`), `heartseed_first_garden` (`:98`), `voidgate_shard` (`:159`). Naming any reward-catalog possession here would **fail the liveness gate**; naming a legendary would hand a rarity-1 encounter a cosmic artifact.
2. The gate is explicit that a category-only spawn is the safe shape. `nudgeGrantLiveness.ts:116–119`, verbatim:
   ```ts
   case 'spawn_artifact':
   ...
   ```
   A category is a closed type union (`ArtifactCategory`), so there is no id to go stale — the THR-844 rot class is structurally absent.
3. It is the shipped precedent. The vertical slice mints The Causeway Lamp, The Loading Order and The Crossroads Gift exactly this way (`src/data/encounters/vertical-slice.ts:2394`, `:2485`, `:3914`), and THR-1164's comment at `:2376` recorded the reasoning: a category spawn has no template node to point at, so the chip anchors through its **carrier** (the agent), which is what the chips above do.

`category: 'relic'` is a live member of `ArtifactCategory` (`src/types/unifiedAction.ts:360` — `weapon | talisman | relic | tome | vessel | key | mundane`), and every field written above exists on the live effect type (`:717–734`).

**The `tier` omission is safe, and now for a recorded reason (Pass 2).** The type comment on `tier` reads *"Override template tier. **Determines node type and edge kind**"* — so leaving it unset is not cosmetic; it decides the reward economy this packet spent a paragraph controlling. Traced through `src/engine/encounterAftermath.ts`: `const saTier: ArtifactTier = effect.tier ?? 'common';` and then `const saActorEdgeType = saTier === 'legendary' ? 'bonded_to' : 'possesses';`. **With no `tier`, the spawn resolves `common` and takes a `possesses` edge** — exactly right for rarity 1, arrived at by default. Do not add a `tier` at implementation.
...
|---|---|---|
| A Little More | `generic.focus` | mind / focus — *a hand holding a needle still, the tremor going out of it* (`:628`) |
| Left Behind | `generic.matter` | matter / substance — *quarried stone opening cleanly under a chisel* (`:642`) |
| By The Book | `generic.ward` | order / ward — *a poured salt line the dark presses against and does not cross* (`:637`) |
| The Easier Way | `generic.dark` | darkness / concealment — *dark closing over an abandoned satchel like water* (`:630`) |
| It Passes | `generic.warmth` | life / warmth — *hearth-warmth creeping across cold flagstones* (`:639`) |
| A Sudden Surge | `generic.energy` | energy / energy — *a charge gathering at a weathervane's spike* (`:643`) |

All six resolve to real rows — **verified at Pass 2 against source, with line numbers** — so none falls back silently to the category generic. Every tag is sphere-matched to its card's sphere, and the common Boost takes the sphere-less `generic.focus`.
...
> **Batch-report observation (Pass 2, not a defect here).** `CARD_CONTENT`'s header comment in `src/data/nudge-card-library.ts` states that `imageTag` is *deliberately* absent throughout, because the image library has no card rows to bind to (THR-832 / THR-1170 own that). So per-hand tags like the six above are correct authoring under the spec's Images step, but they do not yet give a library **member** one shared image the way the communication pivot describes. Worth one line in the batch report.
...
|---|---|---|---|---|---|
| `claimant` (the other person at the ring) | `lazy-materialize-on-trigger` | reuse `scout` / `ranger` / `mercenary`; else spawn `scout` "Orin Vask" | **must-persist** | `bond_change` on four bands; `{cast:claimant}` in three overviews and two chip details | authored here |
| **The Cold Reliquary** (artifact node) | minted at step success | `spawn_artifact`, `category: 'relic'` — runtime-picked, no template id; `tier` unset ⇒ resolves `common`, `possesses` edge | permanent graph node | the BOON chip on three bands; the agent's possessions; `{artifact:any}` enrichment thereafter | authored here |
| `reward_tools_instruments_iron_tongs` | granted by the Cache card | `REWARD_POSSESSIONS` (live, `:870`) | bearer-held, `breakable` | Cache band fragments; the derived equipment factor line on the panel | **pre-existing — verified live ×2** |
| `trait.condition.terrified` | applied by `failureMetadata`; removed by the Balm grant | `CONDITION_TRAIT_DEFINITIONS` (live, `:175`) | duration-edged, 24 ticks | the SCAR chip on both failure bands | **pre-existing — verified live ×2** |
| `trait.condition.wounded` | applied by the `success_at_cost` reaction | `CONDITION_TRAIT_DEFINITIONS` (live, `:143`) | template-default duration | the SCAR chip on `success_at_cost` | **pre-existing — verified live ×2** |
| `trait.condition.location.under_watch` | applied by the `success` reaction, `targetLocationId` | `CONDITION_TRAIT_DEFINITIONS` (live, `:323`, THR-1143) | carried by the place | the location-anchored SCAR chip on `success` | **pre-existing — verified live ×2** |
| `trait.core.core_humility.vice` ("Proud") | read by `traitVariants` | `CORE_TRAIT_DEFINITIONS` (live, built from `coreRegistry.ts:151`) | n/a — read only | the authored factor line | **pre-existing — verified live ×2** |

**Nothing on this list is named-and-unbuilt.** Every id above was resolved against live source during drafting **and independently re-resolved by Pass 2**, and the one grant that could have rotted (`spawn_artifact.templateId`) was deliberately not authored.
...
| Block | Verdict |
|---|---|
| **Steps** | PASS — 1 plain step (inside 1–3), with `reach: 'stone'`, numeric `difficulty: 0.42`, and a `narrativeTemplate`. Nothing padded: no second step wearing a first step's job, no invented gate. |
| **Hand** | PASS — one nudge-bearing step; 6 cards; the full `checkNudgeHand` audit table is in § 8. |
| **Setting** | PASS — `settings` declared, four classes, four openings, `locationSubtypes` derived via `expandSettings`. `validateSettingEnvelope`'s four rules all satisfied. One class-scenery breach in a band fragment was found and repaired at Pass 2 (§ 8, card 6). |
| **Cast** | PASS — one explicit actor binding, class-honest across all four declared classes and verified against the shipped defaults (§ 11). Every `{cast:claimant}` token names a declared key. No gendered pronoun anywhere. |
...
| **Systems** | PASS — 3 connections from the authored manifest: `cast`, `rewards`, `conditions` (§ 1, question 8). At the floor, deliberately, so the batch's other rows carry `reputation`, `factions` and `seeds` — which is the brief's own allocation, not a shortcut. |
| **Images** | PASS — all six `imageTag`s resolve to real library rows (line-cited, § 13); no `illustrationUrl` declared. |
| **Law 56** | PASS — every one of the ten authored chips points at a write that fires on its own band; the pairing is tabulated per band in § 12. No `shell_state`-over-empty-`effects` chip. No `reputation_tally` chip (Law 13 parity). No variant-level `changes`, so nothing renders on a band whose reaction did not fire. |
| **Anchors** | PASS — **three** anchor kinds: agent (🔗 linked — reached by both `$actor` and `$cast:claimant`), attachment (🔗 linked — `trait.condition.*` by template id), location (📍 named — `$target`, no `visualKind`). **Two of the three are not agent**, which is what the brief asks for (*"avoid defaulting to `individual` agents as the only anchor kind"*). *Corrected at Pass 2: the draft claimed "four kinds, three of them not individual-only" by counting `$actor` and `$cast:claimant` separately; per `anchor-catalog.generated.md` both are actor/`individual` and therefore one kind. The brief's ask is still carried; the count was not.* |
| **Grant liveness** | PASS — `attachment_grant` → live `REWARD_POSSESSIONS` id; `remove_condition` / `apply_condition` / `condition_attachment` → live `CONDITION_TRAIT_DEFINITIONS` ids; `spawn_artifact` names no template id. All re-verified at Pass 2 with line citations. |
| **Trait refs** | PASS — one ref, `trait.core.core_humility.vice`, built by `CORE_TRAIT_DEFINITIONS` from `coreRegistry.ts:151`; not on the `validateTraitRefs` dead list. |
| **Enrichment dry-run** | PASS by inspection — tokens used are `{frag:opening}` (a declared slot), `{actor}` (resolved at `proseEnrichment.ts:590`), `{target}`, and `{cast:claimant}` (a declared bundle key). No other `{…}` appears in authored prose. No placeholder appears in any `stateNoun.text`, which is not enriched. |
| **Forecast arithmetic** | PASS — `0.42 + 0.49 = 0.91`, inside `[0, 1]`; with the trait variant's `+0.04` and `-0.02` it is `0.93`. No card buys past the ceiling. |
| **Reachability (THR-821)** | PASS — open-draw at `0.42`, under the 0.45 ceiling. |
| **Variance rule (THR-892)** | PASS — zero static `factorLines`; the one authored factor line is a `TraitVariant.factorLine`. One step, so no `carryoverFactorLines` exist to check. |
| **Detectors** | PASS **after repair**. One vagueness hit was found at Pass 2 and fixed (`something` in the `success_at_cost` overview, § 9) — the draft's sweep had asserted zero. Annotation clauses: effective count 0 against a budget of 1. Divine outcome-authorship: zero. |
| **Register** | PASS — baseline throughout; names, effect lines, factor line, purpose line and reaction labels all interactive-plain; the step's band prose is peak-*eligible* and deliberately declines it. Checked specifically for gothic drift given the cold-and-ruin material: none — the register is dry and physical, and the tone lands as the anecdote the batch design asked for, not as horror. |
| **Seams** | PASS **after repair**. Three seam classes the draft's table did not check were checked at Pass 2 and all three had hits: base-text→fragment (×1), afterimage→overview (×5), chip→chip within a band (×2). All repaired; full table in § 6. |
...
**FLAGS (for Pass 3, none blocking):**

1. **`$target` binding for `encounter.border.*`.** The `success` band's location write and its chip both assume `$target` resolves to a location. Precedent is live in the vertical slice, but confirm for this family before merge. Named in § 12; Pass 2 concurs with the flag and its stated remedy (bind through the support bundle, do not soften the chip).
...
3. **Wiring-guide drift, unrelated to this encounter but confirmed while checking it.** `Docs/plans/2026-04-16-systemic-wiring-guide.md` § *Spawn Artifact* documents fields that no longer exist — `artifactName`, `artifactSubtype`, `possessedByAgentId`, `chronicleEntry`. The live type (`src/types/unifiedAction.ts:717–734`) uses `nameOverride`, `category`, `targetAgentId`, `messageOverride`. This packet follows the type. Pass 2 verified the drift is real. Correct the guide in a separate pass.
...
2. **Does the prose have its own voice?** **YES.** Short declaratives with one image each, dry closers (*"the thorn wall has gone grey"*, *"there is one more pair of hands that could not hold it"*, *"and that was decent"*), and sentence lengths that vary between openings by design.
...
4b. **No seam echoes?** **YES — after repair, and this was the draft's one real failure.** The draft's seam table checked `opening → spine` and `spine → afterimages` only, and declared the encounter clean. Three further seam classes were checked at Pass 2 and all three had hits: base band text → card fragment (they render in the same paragraph), step afterimage → band overview (they render seconds apart, and at `failure` were the same sentence one word apart), and chip → chip within one band (two `causeClause` pairs verbatim, side by side). Eight repairs, all listed in § 6.

**Choices & Intervention**

5. **Does every card state its mechanism, with a generic 2–4 word title, a one-line quote, and zero scene-bespoke prose on the face?** **YES.** All six faces are the library's own authored faces, carried by `libraryCardId` and verified character-for-character against `nudge-card-library.ts`; no face names a relic, a ring, tongs, frost or a cold.
...
7. **Does every card pay off in failure?** **YES.** Six of six carry a failure-band fragment; the big-delta Undertow (0.16) carries both. The Insurance card's fragment sits on `critical_failure`, which is correct: `floor_at_cost` erases the other two failure bands while active.
...
9. **Do the cards answer different questions?** **YES.** Duration · a tool · the floor · refusing to let go at a cost to who they are · the fear · one burst of heat. The two Boosts are justified separately in § 8 and follow the exemplar's same-verb-different-physics pattern.
...
10. **Does the aftermath have its own prose?** **YES.** A variant `overview` plus five band `overview`s, each a written landing that appears before any mechanics — and, after the Pass-2 rewrites, each one landing somewhere its own afterimage did not.
...
12. **For medium+ scale, does the aftermath offer reaction choices?** **N/A — satisfied by scope.** This is short scale (a rarity-1 `Single Test`) and the requirement is explicitly conditioned on medium+.
13. **Do the reaction choices represent philosophical stances?** **N/A by structure, and stated honestly rather than claimed.** There is one reaction per band, so the player is never presented with a stance choice on any given run. That is deliberate and mechanically forced: `AftermathOutcomeOverride` carries `changes` and `reactions` as siblings, so a band's chips render regardless of which reaction is picked — two rival reactions would therefore make every chip on the band conditional on the pick, which is precisely the Law 56 violation rule 0 exists to kill. The god's posture does vary across the five bands (*let them be told how* · *let the word travel* · *let them feel it* · *let them share a fire* · *let the distance stand*), but that is authored variation the player cannot elect, and calling it a choice would be an overclaim. *The draft answered YES here on exactly that reasoning; corrected at Pass 2.*
...
**Twelve YES · two N/A-by-scope, both stated honestly.**
