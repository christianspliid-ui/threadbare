...
> Scale: short (Single Test) | Slug: the-unclaimed-relic | Pass: revised
> Revisions applied: all five band overviews rewritten to stop restating their own afterimages (trigger 22) · base-text↔fragment echo removed from the Cache `failure` fragment (trigger 22) · `something` removed from the `success_at_cost` overview (trigger 15) · class scenery ("in the room") removed from a band fragment (trigger 18) · the second colliding "ring" removed from the Cache `near_miss` fragment (trigger 29) · Balm `success` fragment made subject-first · two duplicated `causeClause` pairs differentiated · vagueness-sweep, anchor-count and gate-question-13 claims corrected to what is true · seam table extended with the two seam classes the draft did not check · `initiation` concreteness · rule-5b verb in § 5 · verified `spawn_artifact` tier default recorded
...
**This is a draft packet, not code.** No TypeScript was written and nothing under `src/` was touched. Field values below are given as plain field lists so Pass 3 can transcribe them; every id was resolved against live source and the resolution is cited inline. Pass 2 re-opened all nineteen cited source files independently and confirmed every resolution.
