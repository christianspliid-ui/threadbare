...
> Family: `encounter.border.*` · templateId: `encounter.border.the_sign_over_the_ruin` · Pass: **revised**
> Batch: border-perils (THR-1221), row **2 of 6** · Date: 2026-08-24 · Format: locked THR-883 nudge-native + THR-1045 Composition Contract
>
> **Revisions applied** (editorial pass, verdict PASS WITH REVISIONS — see [`the-sign-over-the-ruin-editorial.md`](the-sign-over-the-ruin-editorial.md)):
> the Investigation prize is now delivered in step 0's base afterimages (it was declared and never written); "the stone" is
> established at the wayside and stronghold classes; two overviews stop naming furniture two classes do not have; four seam
> echoes fixed (ruin→spine, wayside→spine, stronghold→spine, initiation→spine); four verbless opening fragments and three
> aphoristic inversions rewritten under plainness moves 1 and 3; one two-reading sentence split; one line borrowed verbatim
> from the golden exemplar replaced; the initiation rewritten off reporter register; the two Veil members swapped between the
> hands so hand B's ungated floor is four rather than three; A5/B5's effect-line verb harmonised; B4's effect line rewritten to
> retire the encounter's one annotation clause; the `under_watch` category justification, the `success_at_cost` no-chip
> rationale and § 9.5's claim about where the clue's words live all corrected against the code; step numbering normalised to
> 0-indexed throughout; the crux tightened to put the agent in it.
...
> **Systems-pass gate, flagged by the editorial pass.** `SCENE_SENTINEL_FIELDS` registers `targetLocationId` as `'location'`, and `$target` binds **only when the action's target node is actually a location** (`nodeMatchesSceneField`, `src/engine/encounterAftermath.ts`). Confirm that an ambient `encounter.border.*` draw targets the location and not the acting agent. If it does not, this write never fires and this chip claims state nothing wrote — a Law 56 breach in the field, on the batch's own location-anchor row. Confirm before implementation; do not assume from the sentinel table alone.
...
*Category check: `path` is "a way that has opened **and that the game will act on**". Under Watch is a real condition node attached to the location, with a duration edge the tick loop runs down (`CONDITION_UNDER_WATCH_DURATION`) and `visibility: 'discoverable'` — state moved, and the simulation carries it. Direction is `opens`; a way opening has no gain/loss axis.*

*Two claims the draft made here were checked against the code and dropped: **nothing reads `under_watch` today.** `LOCATION_CONDITION_MOVEMENT_TAX` (`src/data/condition-trait-content.ts`) deliberately excludes it — the comment in the table says so in terms, "being observed changes what you can do in a place, not how long it takes to walk in; its reader is the gate" — and no shipped template gates on it (`requiredTargetTraits`, swept corpus-wide). So there is no shadow tax on it and no location gating on it yet. The chip stays, because Law 56 rule 0 asks whether a **write** fired and one does; and the chip's `detail` says only what the condition's own description says, which is the honest bound on what the player is being told. The absent consumer is a corpus gap worth a note to the portfolio (§ 14 and the batch report), not a licence to overstate.*
