...
> Scale: short | Slug: road-ambush | Pass: draft
...
The compelling element is immediacy. This is not a negotiation or a slow political pressure. This is Iron: the reach of force, combat, physical will. The god can feel the crossbow quarrel that has not been fired yet, the sword-edge that is about to meet flesh, the bandit captain's grip tightening on his weapon because he knows he has lost control of the situation. The god chooses: does divine force flow into the defense of the caravan, or does divine force exploit this chaos for the god's own strategic purposes?
...
Dragan Halfmast's band was gone. Not scattered the way a broken ambush scatters -- there had been no rout, no panic, just a withdrawal as if they had all remembered somewhere else they needed to be. The god could feel the thread of attention still clinging to Dragan like a cobweb on a soldier's sleeve: invisible, negligible, and load-bearing. What that thread would carry -- obligation, resentment, opportunity, or nothing at all -- depended on what happened next, and the god had the patience to wait.

The road was neither safer nor more dangerous than it had been an hour ago. The waymarker stones stood in their places, moss-covered and indifferent. The sparrows that landed on the spilled grain did not care who had won.
...
| Lead driver (supporting NPC) | `lazy-materialize-on-trigger` | Encounter bundle -- created as Soraya's teamster | `scene-only` if unnamed; `must-persist` if the driver's survival/death becomes a narrative thread | Caravan encounters, road memory | `author-now` |
...
| Waymarker stones (flavor) | `scene-only` | Encounter prose -- atmospheric detail | `scene-only` | Referenced in future road encounters for continuity | N/A |
| Trade road safety (reputation channel) | `pre-seeded` | Existing reputation system -- route-level safety | `must-persist` | Merchant decision-making, travel encounters | `live` |
| Extracted goods (Branch B only) | `lazy-materialize-on-trigger` | Encounter bundle -- created from caravan cargo | `must-persist` if intelligence/letters; `scene-only` if generic trade goods | Depends on content: letters seed future information hooks, medicine is consumed | `author-now` |
...
- [x] Editorial review: this is a draft pass -- editorial review should be conducted as a separate pass
...
**Palette:** Dust brown, iron gray, the faded rust-red of old military dye. The warm amber of late-afternoon light on packed earth. The pale green of birch bark peeling. A single bright point: the crossbow bolt standing from the driver's shoulder, the fletching a scrap of color against the muted road.
