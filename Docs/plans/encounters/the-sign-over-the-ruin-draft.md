...
> Family: `encounter.border.*` · templateId: `encounter.border.the_sign_over_the_ruin` · Pass: **draft**
> Batch: border-perils (THR-1221), row **2 of 6** · Date: 2026-08-24 · Format: locked THR-883 nudge-native + THR-1045 Composition Contract
...
This packet is prose + data specification. **No TypeScript was written and nothing under `src/` was edited.**
...
*A sign is still hanging over this ruin, and the people who watched it arrive have already split into two readings of it.*

**0b. The title states the crux.** *The Sign Over the Ruin* — a player who reads only the title knows what the complication is and where it is.
...
- **Step 1 — `veil`.** Veil is Seer ↔ Manipulator: reading a thing that resists being read. The scene grew from that. It is *about* whether you can hold your eye on an omen long enough to see its actual shape, when everyone around you has already stopped looking and started deciding. Chosen before a word of prose.
- **Step 2 — `eye`.** Eye is Witness ↔ Judge: saying truly what you saw. Carryover step. It is *about* what a true statement is worth in a room that has already voted.
...
|---|---|---|
| Trait continuum — `trait.core.core_humility.virtue` (**Humble**) | `traitVariants` + the step-2 trait card | **state read** — the variant gate |
| Condition — `trait.condition.terrified` | step 2 `failureMetadata.effects`, narrated by the failure-side chips | **state write** |
| Location condition — `trait.condition.location.under_watch` | step 2 `successMetadata.effects` on `$target` | **state write** |
| Clue edge — `spawn_clue` | step 1 `successMetadata.effects` | **state write** (fail-soft; deliberately unchipped, § 9.5) |
| Relocation intent — `agent_relocation` | step 2 `successMetadata` / `failureMetadata` | **state write** |
| Reward pool draw | step 1 `successMetadata.rewardPool` | **state write** |
...
| Omen emission | the step-1 Omen card's `grants` | **state write** |
...
Base reward is `intel`: a reading the agent can state, and a lead the sign points at (`spawn_clue`). A clean step 1 also turns up an object off the stone (`rewardPool`, possession). The penalty is concrete and game-legible: **Terrified** (iron −0.06 for its duration) and a forced departure from this hex. Tension sits on step 2 — the reading is the easy half; saying it to people who have already decided is the half that costs. Quintessence stakes: **moderate.** Open-draw ambient content, so a critical failure is a rout and a fear, never a scripted death.
...
**7. Every promise pays off.** The opening makes the player lean forward at *what is over the stone*. That is the Investigation gate's prize and it is answered **behind step 1's outcome bands**, never in the opening prose (the shape's rule). The second promise — the pilgrim who stands there and says nothing — is answered at the `critical_failure` ending, where they are the one person on the ground who does not turn. No third mystery is opened.
...
2. **rewards** — `successMetadata.rewardPool` on step 1 (possession draw, no `tagFilters`).
...
- **A1 · Where are we?** Four grounds, each with a broken structure on it, each sketchable before anything happens: a split tower over a ruin field; a burnt stone barn on a churned battlefield; a roofless waystation beside a wayside camp; a burnt suburb outside a shut fort gate. The spine calls the broken structure "the ruin" and "the broken stone", which reads at all four.
- **A2 · How does it feel?** Two-plus senses beyond sight in every opening: wet ash and frost (ruin); crows and wet iron (battlefield); cold rain and smoke that will not lift (wayside); woodsmoke and the cold coming off the wall (stronghold).
...
- **A4 · What must we know?** That the sign came down here in front of everyone, that everyone here saw it, that the ground has stopped working because of it, and that nobody has actually read it. All stated before the first step is asked for. What the sign *is* is deliberately withheld — that is the Investigation gate's prize.
...
- **B8 · No contradictions.** One ground, one broken structure, one remnant over it, one argument. The hour is set per opening (first light / dawn / since noon) and the spine anchors on "since first light" without contradicting any of them — the pilgrim has stood there since first light in all four.
...
- **C10 · Do people react like people?** The two groups on the battlefield stand apart and stop speaking. The wayside camp has not eaten. The fort shouts across a shut gate. Nobody in the crowd is waiting to be persuaded, and step 2's prose says so plainly.
...
### `openings.ruin` (56 words)

> Half a tower still stands over the rest of it, split top to bottom, the stair inside open to the sky. Wet ash on the air, and frost on the fallen blocks. Thirty-odd people are camped in the lee of the wall. They have been arguing since first light about what they all watched happen here.

### `openings.battlefield` (53 words)

> The field is churned black and picked over. A stone barn on the ridge is burnt out, one gable standing. Crows, and the smell of wet iron. The carters and gleaners working the ground have stopped. They are standing in two groups on either side of the barn, not speaking to each other.

### `openings.wayside` (56 words)

> The camp sits against an old waystation, roofless, its doorframe still square. Cold rain since noon, and smoke that will not lift. Fifteen travelers have their fires going under the wall, pitched closer together than strangers usually pitch. Nobody has eaten. They have been at the same argument since the light over the waystation went out.

### `openings.stronghold` (56 words)

> Outside the fort's gate the old suburb is a burnt shell, roofbeams down, doorways standing. Woodsmoke, and the cold coming off the wall. The gate is shut. The garrison is up on the parapet and forty people are on the road below, and they have been shouting the same two answers at each other since dawn.
...
**No class scenery in the spine.** "The ruin" and "the broken stone" read as the split tower, the burnt barn, the roofless waystation and the burnt suburb without naming any of them.

### `narrativeTemplates.initiation` (scene class, 52 words)

> The sign came down over this ground at first light and it has not gone. Everyone standing here watched it arrive. By the ninth hour they had two answers between them and no patience left for a third. What is actually over the stone has been read by nobody who stayed looking.
...
| `failBehavior` | `continue_weakened` — a bad reading is still a reading, and step 2 has to carry it |
...
**Forecast arithmetic.** Step 0: `0.40 + 0.34` (full hand) = `0.74`. Step 1: `0.42 + 0.40` (full hand) `+ 0.05` (best carryover) `+ 0.03` (trait variant) = `0.90`. Both inside `[0, 1]`, so no card in either hand buys nothing.
...
**Type budget (binding, from the batch design):** `whisper`, `omen`, **`veil`**, `boost` (≤2 per hand), `trait_card`, `mercy`. Six types. Distinct spheres reachable inside that budget are exactly four — `light` (whisper), `time` (omen), `darkness` (veil), `energy` (boost) — so **both hands carry one of each**, which is what meets `HAND_SPHERE_COVERAGE_MIN` (4) without stepping outside the budget. Where a type has two library members I gave the two hands **different members** (`veil` signature/attunement, `whisper` signature/attunement); `omen` and `boost.signature.energy` have no second usable member, so those two faces deal at both steps. That is not a defect — it is what a library-driven hand does, and the two hands are separate decisions.
...
| A4 | Veil | `card.veil.signature.darkness` | No One Saw | `darkness` | 2 | +0.05 | — | `generic.dark` |
| A5 | Boost | `card.boost.signature.energy` | A Sudden Surge | `energy` | 2 | +0.10 | — | `generic.energy` |

Spheres: 4 ✓ · ungated sphere-less option: A1 ✓ · riders: **0** ✓ · boosts: 2 ✓ · distinct types: 4 ✓ · hand size 5 (4–8) ✓.
...
- `bandProse`:
  ...
  - `failure`: "The light came true and showed them the stone. What sat over it stayed unlit."
...
- `bandProse`:
  - `success_at_cost`: "The recognition steadied their eyes and left them sure they had lost an hour they could not account for."
  - `near_miss`: "It read as familiar from top to bottom. Familiar is what it was not."

**A4 · Veil — `card.veil.signature.darkness` "No One Saw"** · `darkness` · 2 essence · +0.05 · `costs: { detectionDelta: -0.12 }`

*The type's content debut — `veil` is one of the eight library types with zero authored users corpus-wide.*

- `fiction`: *"The kindest help leaves no fingerprints."*
- `effectLine`: **"You draw the dark in close while you work, so the reading comes easier and no rival god finds a fingerprint on it. A small help, and your hand goes unseen."**
- `bandProse`:
...
- `effectLine`: **"You flood them with a body's reserve, so they keep looking past the point where a person stops. A real help."**
- `bandProse`:
  - `critical_success`: "They outlasted it. The shape gave up first and showed them its whole edge."
  - `failure`: "They looked long past their limit, and long was not the same as clear."
...
| B4 | Veil | `card.veil.attunement.darkness` | Nothing To Find | `darkness` | 3 | +0.06 | — | `generic.dark` |
...
Spheres: 4 ✓ · ungated sphere-less option: B1 ✓ · riders: **1** ✓ · boosts: 1 ✓ · distinct types: 6 ✓ · hand size 6 (4–8) ✓. Two cards are gated (B6 on the trait, B4 on darkness attunement), so the **dealt** hand typically lands at 4–5, which is the dealt-size doctrine.
...
**B2 · Whisper — `card.whisper.attunement.light` "The Whole Shape"** · `light` · 2 essence · +0.09
...
- `bandProse`:
  ...
  - `near_miss`: "The ground half-remembered agreeing, and half is where it stopped."

**B4 · Veil — `card.veil.attunement.darkness` "Nothing To Find"** · `darkness` · 3 essence · +0.06 · `costs: { detectionDelta: -0.2 }`

- `fiction`: *"A practiced hand leaves less than a careful one."*
- `effectLine`: **"You leave less behind than a careful hand would, so the help lands and the ground's own readers find no god in it. A small help, at a steep price in essence."**
- `bandProse`:
...
| Field | Text |
|---|---|
| `criticalSuccessAfterimage` | "The shape over the stone resolved, edge to edge, and held still long enough to be read whole." |
| `successAfterimage` | "They read it steadily and came down with an answer they could stand behind." |
...
*Category check: `path` is "a way that has opened **and that the game will act on**". Under Watch is read by the shadow tax and by location gating, and it renders on the Location Profile, so the simulation acts on it. Direction is `opens` — a way opening has no gain/loss axis.*
...
> **overview:** "They got it said. It cost them the room's attention for the rest of the day, because from the second sentence on, the question stopped being what the sign was and became who this was who claimed to know. Both readings are still standing. So is a third, about them."

**changes:** *(none authored — this band's cost is attention and reputation among people who will scatter tomorrow, and there is no write behind either. The words go in the `overview`, where they claim nothing. Folding a chip rather than dressing fiction in a pointer is the rule; a `reputation_tally` chip here would be a released defect under Law 13 visibility parity, since per-Reach tallies render only in the debug designer tab.)*

---

#### `failure`

> **overview:** "The reading did not hold together on the stone and it did not hold together in front of the crowd. Both sides got to keep what they came with. The pilgrim stayed at the wall. The stone put a fear into them that will need walking off."
...
> **overview:** "They came down certain, and the shape they were certain of was not the one over the stone. The ground heard the difference before they had finished. By dusk both sides had agreed on one point, and the point was them. It is the pilgrim — {cast:witness} — who walks them out past the last fire and points them at a road, and does not say which reading was right."
...
| `condition` (second carrier) | `condition_attachment` → `trait.condition.location.under_watch` (**location**, `targetLocationId: '$target'`) | step 1 `successMetadata.effects` | A reading said out loud and taken is exactly what turns ignored ground into ground people keep eyes on. |
...
The family is still **wired**, which is what the gate asks. What the endings say about it — *"what the sign was aimed at, and it was not this ground"* — is a claim about what the mortal understood, which is scene-local and true regardless. The words live in the band `overview` and in step 0's `criticalSuccessAfterimage`; no chip claims the edge.
...
| A4 No One Saw | `generic.dark` | darkness / concealment — *dark closing over an abandoned satchel like water* | Yes. Any step where the help must go unseen: a theft, a bribe, a smuggled letter. |
...
| B4 Nothing To Find | `generic.dark` | darkness / concealment | Same row as A4 by design — the library has no per-member card art yet, so both Veil members resolve to the type's concealment plate. Documented rather than silently inherited. |
...
| `trait.condition.location.under_watch` | lazy — granted by step 1 `successMetadata` on `$target` | `src/data/condition-trait-content.ts` | must-persist | `critical_success` chip; the shadow tax and location gating read it | ✅ live |
...
**Opening → spine.** All four openings close on the *people*: "arguing since first light" / "not speaking to each other" / "at the same argument" / "shouting the same two answers". The spine opens on the *sign*: "What crossed the sky over the ruin has not finished." No shared image, no shared sentence shape, no near-identical phrasing across the boundary. The word "argument" appears once in the spine's descendants (step 1's prose says "argue"), and the spine itself avoids it entirely.

**Spine → step-1 prose.** The spine ends on "Everyone who tried to look steadily looked away first." Step 1 opens on "They come down off the stone with a reading." Different subject, different verb shape, no echo of "look".
...
| Shape obeyed — information behind the Investigation gate | **PASS** | Openings and initiation say only that nobody has read it. What it *is* lives in step 0's bands. |
...
| ≤1 annotation clause across the encounter | **PASS** | **Zero**. No `not … but` construction and no em-dash-then-negation anywhere. |
...
| Three plainness moves | **PASS** | Subject first in every sentence (no opening fragments); abstractions cashed in-sentence; one dry line per beat. |
...
6. **Every card's price real and legible?** **YES.** Essence on nine, essence plus a detection channel on the two Veils, and cost 0 on the trait card because the price was paid by being Humble. No zero-essence non-trait card.
...
## 16. What Pass 2 should look hardest at

Recorded so the critic's time goes where the risk is, rather than where the packet is thickest:

1. **The `veil` → `eye` reach honesty.** Step 0 must genuinely be a Veil test (reading a thing that resists reading) and not an Eye test wearing Veil's name. If the critic reads step 0 as "perceive accurately", that is Eye and the split collapses.
2. **The four-class envelope against a title with "Ruin" in it.** The design fixes both. My reading is that "the ruin" is *a broken structure on this ground*, which every one of the four classes carries — but the stronghold opening is the one where that reading is thinnest, and it is the opening to challenge.
3. **`success_at_cost` carrying no chip.** Deliberate (§ 9.3), and the alternative — a `reputation_tally` chip — is a released defect under Law 13. If the critic wants a chip on that band, the honest route is a new backing write, not a sentence.
4. **The repeated card faces across the two hands** (`card.omen.signature.time`, `card.boost.signature.energy`). Forced by the type budget's sphere reach; justified in § 5 as library behaviour. Worth a second opinion on whether it reads as repetition to a player.
5. **Grim, earned.** The batch allots this encounter one of its two grim resolutions. The claim is that the grimness comes from a true reading having no side to be on, and from being run off ground you were right about. If it reads as punishment rather than as plot, the failure bands need rewriting, not softening.
