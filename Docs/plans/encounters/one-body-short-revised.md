...
> Scale: short (1 step) | Slug: one-body-short | Pass: revised
> Revisions applied: consequence swap `thread` → `omen` (orchestrator ruling; `thread_weaken` removed, `emit_omen` authored in context and left unchipped) · `concepts` declared on all five chips, every one anchored by `entityId`, the placeholder tooltip id deleted · `success_at_cost` grief chip backed on its own band via band-level reactions · two detector hits removed (`way`, `thing`) · six seam echoes rewritten · Omen and Long Game effect lines rewritten to state mechanism · `wayside` opening rewritten so the ground testifies rather than the fight being narrated · four plainness-move rewrites · seam table widened from four seams to eighteen · detector self-scan and self-audit corrected
...
**This is a revised draft packet, not TypeScript.** Nothing under `src/` is touched.
Pass 3 (systems) reads **only this file** and compiles from it.
