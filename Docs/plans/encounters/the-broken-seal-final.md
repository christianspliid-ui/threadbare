...
> Scale: 2-step Test & Consequence (carryover) | Slug: `the-broken-seal` | Pass: final
...
| Pass | Verdict | Notes |
|------|---------|-------|
| Draft | Complete | Two-step delve, `star` descent → `stone` carry, hostile keepers, rival with a head start; design block written before prose. |
| Editorial | PASS WITH REVISIONS | 17 findings, 7 Must-fix. All 13 card faces adopted from the library verbatim; 5 afterimage→overview echoes rewritten; the "three tries" texture count removed; 4 effect lines stripped of odds-talk; `initiation`'s unmodelled decline cut; the fourth promise (the keeper who did not come back) closed in step 1's spine; 3 cross-encounter echoes with the calibration case rewritten. |
| Systems | READY FOR IMPLEMENTATION | Every id, effect kind, field name and constant in the packet checked against live source and resolves. The consequence draw was independently recomputed by running `npm run draw:consequences` and reproduced `['drive', 'movement']` exactly. All five of editorial's carried findings resolved to PASS. No missing primitives, no engine work needed. |
...
Editorial's pass (full detail: `the-broken-seal-editorial.md`) ruled on three flagged design
questions and found 17 prose/wiring issues, applying all of them to produce the revised packet
this document is built from:

- **Ruling A (counts):** *"Two of them went below yesterday and one came back"* survives — it is
  the doctrine's own prescribed P2 shape (a cost already paid), not clever specificity. *"It took
  three tries"* does not — that was texture and was cut.
- **Ruling B (tier-3 prize):** kept on `critical_success` alone. Gating the richest prize to the
  rarest band on a two-step carryover chain is the `byOutcome` floor working as designed, not a
  calibration error.
- **Ruling C (card faces):** all thirteen cards adopt their library member's authored title
  verbatim (the THR-1178/THR-1224 retrofit has landed; the sibling batch slot already does this;
  two of the draft's invented names collided with real library members the sibling slot deals).

Must-fix, Should-fix and Consider findings (F1–F18) are enumerated in full in
`the-broken-seal-editorial.md` §2; all are applied inline in the packet below.

### Implementation File Map

**Create:**
- `src/data/encounters/the-broken-seal.ts` — the template itself, built to §§ 4–18 below.
  `src/data/encounters/the-unclaimed-relic.ts` is the closest live reference for the template-level
  shape (`intrinsicTier: 'background'`, `crudType: 'read'`, `scale: 'local'`,
  `actorAffinities: ['individual']`) and for wiring `$target` on a self-targeting encounter.

**Modify:**
- `src/data/unified-action-templates.ts` — register the new template's import, following the
  existing pattern for other `encounter.delve.*` entries.
...
**Verify (run, no edit expected):**
- `npm run check:encounter -- encounter.delve.the_broken_seal` once the template is registered.
- The 30-tick CLI engine smoke (CLAUDE.md Pre-Commit Checklist item 7) since a new file under
  `src/data/encounters/` is content, not `src/engine/`, so the smoke is not strictly required by
  the letter of the rule — run it anyway if the PR also touches anything under `src/engine/`.

Full systems audit, every id checked and where: `the-broken-seal-systems.md`.
...
verbatim. Every prose string below is final. Every id below was resolved against the live catalogs
during drafting and independently re-verified against live source at Pass 3 (see
`the-broken-seal-systems.md`). Do not re-invent names, ids or prose; if something does not compile,
fix the wiring and keep the words.
...
| 5 — scale | **company** | The outcome touches the keepers (a body of people), the rival, and the place itself: `critical_failure` shuts the stair for everyone. `ActionScale: 'local'` is the schema value that carries company scale — the union has no `company` member (systems-confirmed: `src/types/unifiedAction.ts:20`). |

Consequence hand: `consequenceDraw: ['drive', 'movement']` — **binding, no swap taken, and
independently reproduced** by running `npm run draw:consequences -- encounter.delve.the_broken_seal
--reach star --rarity 2` at Pass 3 (see `the-broken-seal-systems.md` § 2). § 11 shows where each
family is wired in context.
...
**Systems-confirmed:** `supportRole` is `readonly supportRole: string`
(`src/types/encounter.ts:218`) — "Role label for the support cast member," a free string field with
no closed-set validator anywhere in the composition contract or the cast-target check.
`'rival_delver'` is accepted as authored.
...
**Systems-confirmed:** all thirteen titles and all thirteen quotes below match `CARD_CONTENT`
verbatim (`src/data/nudge-card-library.ts` lines 572–649), checked entry by entry at Pass 3.
...
2. **Variant?** — **Yes, one.**
   ...
   **Coverage note, measured and systems-confirmed.** `core_hope` is *not* an unspent cell: the
   ...
   something."), and the `vice` pole in `src/data/encounters/the-garrisons-price.ts:144`. Verified
   by direct grep at Pass 3, independent of the editorial pass's own correction. This encounter is
   the continuum's **fourth** encounter user, and it is picked for fit, not for coverage. The
   factor line is written clear of all three shipped lines and clear of the trait card's own
   `success` fragment, which renders beside it for the same holder.
...
**no swap taken, and independently reproduced at Pass 3** by running the live draw tool (see
`the-broken-seal-systems.md` § 2).
...
|---|---|---|
| `critical_success` | `assign_ambition` → `ambition_uncover_secrets` | They opened it and saw what was under a seal somebody meant to keep shut. The ambition's own selection prose is *"Something was buried here once, on purpose. She meant to know what."* — the encounter's premise, already written into the live template (verified at `src/data/ambition-templates.ts:541`). |
...
so the two cannot collide. (This card grant does not itself count toward `checkConsequenceDraw`'s
wiring check, which reads only aftermath-level effects — the required wiring for `drive` is
already satisfied by the two band reactions above.)
...
looking. **Systems-confirmed:** two attachment grants from one band is legal — nothing in the
Composition Contract bounds grant count per band, and each grant here is independently backed by
its own write (the fragment by step 1's `successMetadata`, the testament by this band's own
reaction).
...
**Systems-confirmed:** `seal.crit_fail.the_wanting`'s `concepts` text *"Uncover Ancient Secrets"*
matches `ambition_uncover_secrets`'s live `displayName` exactly (`src/data/ambition-templates.ts:491`).
The chip names the ambition the player's sheet will call it.

**Systems-confirmed:** `$target` resolves for this template shape. `classifyAnchorDeclaration`
accepts `'$target'` unconditionally at the gate; runtime resolution (`bindAftermathSceneTargets`
rebinding `$target` → `action.targetId` when the target node's kind is `'location'`) is proven by
the shipped, gate-green calibration case `the-unclaimed-relic.ts`, which has the identical
template-level shape (`intrinsicTier: 'background'`, `crudType: 'read'`, `scale: 'local'`,
`actorAffinities: ['individual']`) and does exactly this pairing (`targetLocationId: '$target'` +
`stateNoun.entityId: '$target'`, `visualKind: 'location'`).
...
|---|---|---|---|
| `seal.crit.prize` | attachment | `entityId: 'reward_tomes_scrolls_veilscript_fragment'`, `visualKind: 'attachment'` | 🔗 linked, systems-confirmed live |
| `seal.crit.testament` | attachment | `entityId: 'reward_tomes_scrolls_the_silent_testament'`, `visualKind: 'attachment'` | 🔗 linked, systems-confirmed live |
| `seal.success.prize` | attachment | as above (fragment) | 🔗 linked |
| `seal.cost.prize` | attachment | as above (fragment) | 🔗 linked |
| `seal.cost.wounded` | attachment | `entityId: 'trait.condition.wounded'`, `visualKind: 'attachment'` | 🔗 linked, systems-confirmed live |
| `seal.fail.worn_out` | attachment | `entityId: 'trait.condition.exhausted'`, `visualKind: 'attachment'` | 🔗 linked, systems-confirmed live |
| `seal.crit_fail.shut` | **location** | `entityId: '$target'`, `visualKind: 'location'` | 🔗 linked, systems-confirmed resolves (see § 12) |
...
the ambition in `concepts` as undecorated text — legal, because `chipAnchorViolations` only checks a
`concepts` entry when it declares an `entityId`, and this one deliberately does not. **Finding for
the batch report:** the anchor catalog and the anchor-declaration classifier disagree about
`ambition`; one of them should move. Upheld at Pass 2 and confirmed by direct code read at Pass 3.
...
**Card tags** — every one resolves to a row in `ENCOUNTER_IMAGE_LIBRARY`, systems-confirmed at
Pass 3 by direct read of `src/data/encounter-image-library.ts` (`NUDGE_CONCEPT_ART`, lines 628–643)
in addition to the draft-time check:
...
**Exact-symbol resolution: confirmed at Pass 3.** Every id above was checked against a `grep` hit
on the live source file, not merely against the directory it is expected to live in — see
`the-broken-seal-systems.md` § 1 finding 4 for the line numbers and the exact matched declarations.
No dead reference anywhere in this table.
...
| Card face uses its library member's authored title and quote | **PASS** — all thirteen, verbatim from `CARD_CONTENT`, re-verified line-for-line at Pass 3. |
...
| Hand total ≤ 0.70 · difficulty + hand ≤ 1.0 | 0.52 · 0.92 ✓ (re-derived independently at Pass 3) | 0.46 · 0.90 ✓ (re-derived independently at Pass 3) |
...
| ≥1 card with a real `grants` against built content | **4:** `Call Them Onward` (`emit_omen`) · `Ease The Suffering` (`remove_condition`) · `Set Aside For Them` (`attachment_grant`, live id) · `Kindle A Wanting` (`assign_ambition`, live id). All ids resolve for `validateNudgeGrantRefs`, systems-confirmed. ✓ |
...
| Systems | **4** — cast · rewards · conditions · seeds (floor 3, brief target 4), re-verified independent of card grants at Pass 3 ✓ |
| Images | 13 tags, all resolve; no `illustrationUrl` ✓ |
| Consequence draw | `['drive','movement']`, both wired by effects the gate's walk can see (band reactions + step metadata, **not** card grants) ✓ — independently reproduced by running the live tool at Pass 3 |

### Chips (Law 56)

| Clause | Verdict |
|---|---|
| 0 — every chip backed by a write that fires on that band | **PASS.** Each of the five faces performs at least one qualifying write reachable on it: `critical_success` (reaction `attachment_grant` + step 1 `successMetadata`), `success` (`encounter_seed` + step 1 `successMetadata`), `success_at_cost` (reaction `condition_attachment` + step 1 `successMetadata`), `failure` (step 1 `failureMetadata` ×2 + reaction `quintessence_shift`), `critical_failure` (reaction `condition_attachment` + `assign_ambition` + step 1 `failureMetadata`). Variant-level `changes: []`, so the fallback face claims nothing. Walked face-by-face against `chipBackingForFace` at Pass 3 — see `the-broken-seal-systems.md`. |
...
| Own `supportBundle`, class-honest at all three | ✓ § 5, checked against `LOCATION_ROLE_ROSTERS`, re-verified at Pass 3 |
| Rolled constraints honored | ✓ § 3, all five dice placed |
| Hook recorded | ✓ `hook.descent_into_darkness`, systems-confirmed live at `src/data/content-eval/plotHooks.ts:597`; `usedBy` stamp is a Pass-4 task |
| `consequenceDraw: ['drive','movement']`, both wired in context, no swap | ✓ § 11, independently reproduced at Pass 3 |
...
| Systems quota ≥4 from the authored manifest | ✓ 4, re-counted independently at Pass 2 and Pass 3 |
...
| No new engine primitives; mature systems only | ✓ movement, conditions, carryover, items, traits, seeds, cards — confirmed no missing primitive at Pass 3 |
...
### Findings resolved at Pass 3 (systems audit)

All five items editorial carried forward resolved to PASS by direct check against live source.
Full detail, file paths and line numbers: `the-broken-seal-systems.md` § 1–2.

1. `supportRole: 'rival_delver'` — confirmed a free string field (`src/types/encounter.ts:218`),
   not a closed enum. Accepted as authored.
2. `critical_success`'s two attachment grants — confirmed legal; nothing bounds grant count per
   band, and each is independently backed.
3. `seal.crit_fail.the_wanting`'s `concepts` text — confirmed matches `ambition_uncover_secrets`'s
   live `displayName` exactly.
4. All thirteen `imageTag`s and every id in § 15 — confirmed exact-symbol resolution against live
   source, not merely file-level presence.
5. `ActionScale` carries no `company` member — confirmed; `'local'` is the correct schema value.

**One informational note for the batch report and for future authors (non-blocking):** card-carried
`grants` (this template's `Set Aside For Them`, `Call Them Onward`, `Ease The Suffering`, `Kindle A
Wanting`) do not themselves count toward the systems quota or the consequence-draw wiring check —
`systemConnections()` and `checkConsequenceDraw()` both read only aftermath-level effects (band
reactions, step `successMetadata`/`failureMetadata`), not `step.nudges[].grants`. This encounter's
4-system count and both drawn families are wired independently of any card grant, so the verdict is
unaffected — flagged so a future template does not rely on a card grant to satisfy either gate.

### Findings carried to the batch report (not this pass's to resolve)
...
2. **Anchor-catalog contradiction.** The catalog lists `ambition` as an anchorable kind declared by
   ...
   shipped attachment template. One surface should move (§ 13). Confirmed by direct code read at
   Pass 3.
3. **`core_hope` is not a thin cell.** It carries three shipped encounter users already
   ...
   for fit. The draft's contrary claim was corrected at Pass 2 and independently re-confirmed by
   grep at Pass 3.
