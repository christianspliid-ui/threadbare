...
> Scale: medium | Slug: the-beast-in-the-granary | Pass: final
...
| Pass | Verdict | Notes |
|------|---------|-------|
| Draft | Complete | Full nudge-native Personality Fork built to the brief's rolled constraints — a bear denned on a settlement's winter store, a `sacrifice_survival` fork the mortal (not the player) decides from a composed beat-1 hand of two pole-leaning specials + dealt fill, nine systems findings filed honestly rather than worked around. |
| Editorial | PASS WITH REVISIONS | Design untouched (the fork and the composed hand are the strongest parts of the draft); fixed a missing prose surface (zero of fifteen afterimages authored), seven vagueness-lexicon hits, one second-person sentence, two objects (the bins, the watch) a card acted on before the spine introduced them, a possession beat 2 asserted that step 0 does not always mint, and an unbalanced branch (the `negative` pole had no floor to match the `positive` pole's peak). |
| Systems | READY FOR IMPLEMENTATION | Every id in the revised packet resolves against current code. `check:encounter -- encounter.hunt.the_beast_in_the_granary` will go green — traced block-by-block against the live gate implementation, not inferred. The core mechanism (an agent-decided branch reading a composed hand's net pole lean) is live, tick-loop-verified code, not a documented-but-unproven primitive. |

### Caveats / Blockers

None blocking. Two findings carried from the draft/editorial passes have no content-side fix and are correctly reported rather than worked around:

- **The bear has no cast primitive** (`NpcRole` is all-human) — the opposition of the game's first hunt cannot have a portrait, a click, or `StepNudge.opposes`. Every card acting on the animal attributes to the card, not the creature. This will recur in every `encounter.hunt.*` member; BACKLOG, not BUILD-NOW off one template.
- **`$faction:civic_guard` is a def-id anchor, not a node-id anchor** — the anchor catalog's own rule says chapters share a def id and a def id does not identify the body the player dealt with, and `resolveFactionNodeId` can, in a multi-chapter world, enroll the sworn-in agent in a `civic_guard` chapter that has nothing to do with this settlement. No content-side fix exists.

One structural gap governs how much of this packet the machine gate can independently verify, without threatening whether it passes: `check:encounter`'s structural blocks (hand composition, image resolution, enrichment tokens, forecast headroom, grant liveness) do not descend into `ActionStepBranch` variants, so beat 2's two composed hands are checked by nobody but this audit and the editorial critique pass. The **register detectors** (vagueness, second person, echo/seam checks) *do* correctly descend into both branches, so the editorial pass's prose fixes on beat 2 are gate-verified even though beat 2's mechanical structure is not. Full trace in the systems audit §3 and Finding #11.

### Editorial Notes Summary

The design survived the editorial pass unchanged — a genuine two-sided fork where beat 1's two specials argue *opposite* directions on the axis the branch reads (`Wake Their Fear` leans survival by making the door look like the only sane object in the room; `Weigh The Winter` leans sacrifice by showing the mortal how many mouths are in the bins), and neither pole reads as the "bad" one. What failed was transcription and word-level discipline, fully repaired in this file:

1. Authored the missing outcome surface — fifteen afterimages (five each on step 0, `positive`, `negative`) plus action-level `narrativeTemplates`, written from the draft's own outcome-ladder table, which had no field to live on.
2. Cleared seven vagueness-lexicon hits (`nothing` ×3, `thing`, `things`, `way` ×2) from `outcome`-class fields.
3. Removed the one second-person sentence (`positive`/`critical_failure`: "moves you" → "moves the one who tries").
4. Grounded the bins and the watch in the spine — `weigh_the_winter` no longer names an object the beat-1 prose never introduced; the drawn `membership` family's ending no longer arrives from a body ("the watch") the player has never read about.
5. Stopped beat 2 asserting a possession step 0's `continue_weakened` `failBehavior` does not always mint — both continuations now state *position*, never possession, true under every step-0 outcome. This also closed a verbatim twelve-word sentence the two continuations had shared.
6. Authored `negative`/`critical_failure`, giving the survivor pole a floor as deep as the martyr pole's peak, and paying off `granary.narrow_their_sight`'s obligatory big-delta failure fragment, which previously had no aftermath band to land in.
7. Corrected `granary.let_it_set` → `granary.let_it_teach`, reach `iron` → `star` — the reach now matches the reaction's own stated lesson (a lean toward *survival*, which is Star's axis), and the label no longer leaves "it" ambiguous between the wound and the character.
8. `granary.weigh_the_winter`'s effect line now states its odds mechanism (the floor, not only the count), earning its `forecastDelta`.
9. All four card faces made imperative-led (Fill · Show · Open · Dim); "the sleeping thing" → "the sleeping animal".
10. Trait factor line trimmed from 15 words to 11, inside budget.
11. Self-audit rows that had passed themselves on claims the text did not support (bins in the spine, no repeated name-word, no seam echo, vagueness at zero) rewritten to report the truth, with repairs recorded.

### Implementation File Map

Per THR-1246, registration and the structural test are compiler-owned by `compile:encounter` — do not hand-author them. Beyond the compiled set (`src/data/encounters/the-beast-in-the-granary.ts`, its generated `__tests__` file, and the two registrations), nothing new needs building:

- No new engine module, effect kind, node type, trait continuum, or roster role. Every mechanism this packet uses (`ActionStepBranch.decidedBy` in pole mode, `condition_attachment` with `targetLocationId`, `spawn_artifact`, `membership_change`, `hidden_mark`, `intelligence`, `axiological_mark_apply`) is live, exercised code — confirmed at the call-site level, not the type-declaration level.
- **Package-authoring note for Stage 4 (not a file to create):** `consequenceDraw` is compiler-derived from `(templateId, reach, rarityTier)` per `encounter-package-format.md` — do not author it in the `.package.json`'s `template` object even though it is documented on this packet's header line and in §16 below. Keep the template id `encounter.hunt.the_beast_in_the_granary` unchanged through implementation; the draw is id-keyed.
- Concept art per §15b, standard pipeline path.
...
**Systems verification (Stage 3):** both drawn families resolve to `wired` under `familiesWiredByEffects` because `aftermathVariants(template)` walks `config.variants`/`config.fallback` directly — a structure separate from `template.steps`, so the reaction-carried `membership_change` is visible to the gate regardless of the branch-blindness recorded in §7's Findings. `possession` clears via step 0's `spawn_artifact` (a plain step, always visible). See the systems audit §3 for the full trace.
...
| **the watch** (`civic_guard`) | `pre-seeded` | `FACTION_DEFINITIONS` → `civic_guard`; chapters seeded by `factionSeeding` | `must-persist` | The agent's `member_of` edge; faction roster; future watch work | `resolveFactionNodeId` resolves a def id to a chapter node | `live` — **with a locality caveat, Findings #2, systems-confirmed against the current implementation** |
...
| 30 | Every chip's referent is a real graph object and the sentence names it (Law 56 cl. 2) | **PASS with one flagged caveat** — anchors are `$target` (location, linked), `$faction:civic_guard` (faction, linked), `$actor` (agent, linked), attachment template ids (linked). Every one classifies `ok` under `classifyAnchorDeclaration`. The caveat is that a def id does not identify a *chapter*; no content-side fix exists. Findings #2, **re-confirmed against the current `resolveFactionNodeId` implementation at systems pass — the caveat is real and current, not stale.** |
...
| 35 | Consequence draw recorded and every drawn family wired | **PASS** — `['possession','membership']`; `possession` via `spawn_artifact` on a plain step *and* in a reaction, `membership` via `membership_change` in a reaction and promised in the spine. No swap. **Systems-verified: both families resolve `wired` under `familiesWiredByEffects`; `checkConsequenceDraw` passes.** |
...
Per the brief: *"a wanted-but-missing capability is a FINDING to report, never something you approximate or mint."* None of these was worked around in content. Verified current against `main` at systems pass (2026-08-26) — see `the-beast-in-the-granary-systems.md` for the code citations.
...
- **Confirmed at systems pass to extend to two more gate functions**: `check-encounter.ts`'s forecast-arithmetic sweep (`if (isActionStepBranch(step)) continue`) and `nudgeGrantLiveness.ts`'s `allTemplateEffects` (same guard) — both skip branch-variant steps identically, so beat 2's forecast headroom and its card grants (the `axiological_mark_apply` on `granary.narrow_their_sight`) are never validated either.
- **Not extending to the register detectors.** `collectClassedTemplateProse` (`nudgeAuditDetectors.ts`) explicitly walks both `variants` and `fallback` — the vagueness lexicon, second-person, and echo checks this editorial pass fixed on beat 2 *are* gate-verified, even though beat 2's structure is not. Worth stating precisely rather than folding into one "the gate is blind to branches" line, since the two halves have different implications for what a future author can trust.
...
*Editorial note:* this finding is also why the draft's own errors in the branch variants — a card face repeating a scene object the beat never introduced, a `reach` that contradicted its reaction's stated meaning, seven lexicon hits, and fifteen missing afterimages — would have shipped green on the structural side. It did not ship silently only because the register detectors and the editorial critique pass both correctly reach into the branch.
...
**11 · (Added at systems.) `axisRegistry.ts`'s code comment for the `star` reach conflicts with its own `REACH_VALUE_PAIR` data table — pre-existing, zero consequence for this packet, worth a maintenance note.**
`CANONICAL_AXES.star` carries a comment stating Star was re-scoped (THR-545) away from "inner outlook" toward "wayfinding/fate — guiding others' journeys vs leading them astray," with pole words `Beacon/Guiding` ↔ `Wrecker/Misleading`. But `REACH_VALUE_PAIR.star` (`src/types/agent.ts:46`) still hard-codes the `ValuePair` `'sacrifice_survival'` — the exact axis this encounter's fork runs on, and the exact one `granary.narrow_their_sight`'s reach-scoped mark targets. The packet's technical usage (`reach: 'star'` for a `sacrifice_survival` mark) is correct against the live lookup table and matches the one other shipped precedent that uses a reach-scoped mark. The packet's internal shorthand pole labels ("Martyr"/"Survivor", used only in design-table headers, never in player-facing prose) are the packet's own gloss, not a misreading of the registry — but a future reader of `axisRegistry.ts` alone could conclude `sacrifice_survival` no longer lives at Star, when the data table says otherwise. Not this packet's defect to fix; flagged for the retrospective.
