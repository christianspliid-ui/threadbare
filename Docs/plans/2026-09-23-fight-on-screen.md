...
**Files to touch:**
- F1: `src/components/Game/HexSidebar.tsx`, `src/components/Game/WorldPulse.tsx`, `src/components/shared/entityVisualResolver.ts`, `src/data/entity-visual-fallbacks.ts` (where `EntityVisualKind` is declared), the chip collaborator's render-time refinement (`worldRefAdapters.ts`), `src/components/StyleGuide/StyleGuide.tsx`, a new lair-card adapter, `src/debug-bridge.ts`/`.d.ts` (`getLairMonsterCard`)
