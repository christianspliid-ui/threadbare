...
**Where a query-resolved seed is judged (`resolutionLocationId`, engine-set, THR-1511):** a seed's `locationSubtypes` eligibility filter is judged at the *target's* current Location — unless the seed carries `resolutionLocationId`, in which case that Location is tried **first** and the target's feet **second**. Only the undertaking catalyst planter sets it today (`maybeSeedCatalyst` → `catalystAnchorLocationId`, the completed work's own settlement: a route's far end, an army's garrison, a founded place's parent), because a work completes wherever its actor stands while every catalyst family is settlement-gated. The `encounter_seed` aftermath effect does not expose it — an aftermath fires where the scene was, which is already the target's feet. The resolution trace names `judgedAtLocationId` / `judgedAt` so a wither can be read as "no place accepted it" rather than "the actor stood in the wrong place". **Content rule (THR-1515):** every family a cell names in `UNDERTAKING_CELL_CATALYSTS` must carry at least one individual-performable member for each of `hamlet`, `town`, `city` and `capital` — `cell.create.location` founds a hamlet, so a family with no hamlet member rejects the thing the work made. Widen an existing member whose fiction survives the place; do **not** ungate one, because `unifiedCandidates` offers any ungated `local` template at every location to every mortal.
...
**A tally key that is not `<reach>.positive|negative` is DISCARDED — and fails the gate (THR-1206).** `isValidReputationTallyKey` accepts only the sixteen reach-polarity keys; everything else is traced and dropped, so the encounter promises a consequence that never once occurs. Measured at survey: **171 of 518 authored tally writes (33%), across 78 distinct keys.** `check:encounter` and a corpus-wide test fail any off-axis key, and **the ratchet that froze the shipped backlog is now empty — THR-1207 drained all 78** (`src/data/content-eval/tallyKeys.ts`). There is no grandfathered set left, so an off-axis key is simply a build failure. If your consequence is not "what this mortal is becoming known for", it is not a tally — see Capability 22.

**Two authoring rules the sweep settled, worth copying rather than re-deriving.** *Polarity lives in the key, not in the delta's sign* — write `delta: 1` on `heart.negative`, never `delta: -1` on `heart.positive`; no shipped content does the latter and the surfaces read the key. And *`reputation_with` is not a free substitute for a tally*: `targetLocationId: '$target'` binds only when the target really is a place-tier location (`nodeMatchesSceneField`), so pointing a person-targeting scene at it swaps a discarded write for a refused one. Reach for it when the standing is genuinely **with** one named place, person or outside faction; reach for a tally when it is renown the mortal carries between scenes.
...
**Coverage is owed per path, not per template (THR-1509).** The corollary of that order: a band you author on `variants.paid` does nothing for a player who took `variants.refused`. Each path a player can land on — every `variants[choiceId]`, plus `fallback` only on a choice-less config (a fork's `fallback` is unreachable in play; the arm is decided before the deciding step's outcome lands) — **owes the losing bands it can reach**: `critical_failure` always (a critical step ends the action there whatever its `failBehavior`), and `failure` when any step on that path is `fail_action`. The reason is register: base endings are written as wins, so on a loss the base lies. Winning bands are optional per path. The `[?outcome]` verdict judges against the path the action resolved on — `unauthored_band` names the arm (`variantKey`) and what exists elsewhere (`templateBands`) — and the vertical-slice gate (`vertical-slice.test.ts` § THR-1509) reads through the same functions (`authoredOutcomeBandsOnVariant`, `reachableLosingBandsOnPath` in `src/engine/debugOutcomePin.ts`), so the gate and the console line cannot disagree. Worked example: `encounter.slice.swindler_found`, whose two arms author *different* winning bands on purpose and both losing bands by rule.
...
5. **A reaction that nests a modifier opens a window (THR-1568).** When a `reactive`'s nested effect is a reach modifier — `duration`, `passive`, `permanent` or `decay` — firing it does not "run" anything: it opens a **window** on the item's runtime state, and the bearer's rolls read the nested value while the window is open. Before THR-1568 every such burst was handed to the executor, which does nothing with a modifier, so *"when struck, Iron +0.03 for six ticks"* never landed on any item. How long the window lasts:
   ...
   The reaction's `cooldown` still gates re-firing: a second hit inside the cooldown opens no new window. If you author a cooldown shorter than the window, a re-fire **restarts** the window — a refresh, never a stacked second copy. Reactions nesting anything else (`spawn`, `dispel`, `cascade`, `resource_manipulate` `fight_clock`, `inflict_condition`, …) execute exactly as before. **`blessed` and `cursed` fire when a family condition lands (THR-1624).** Every condition-landing site — `applyConditionToActor`, the aftermath `condition_attachment` write, an item's `condition_grant` action trigger, and the bless/curse undertaking verb — calls `raiseConditionLanded` (`conditionProxyEvents.ts`), which raises `blessed` for a condition tagged `#blessing` and `cursed` for one tagged `#curse` (the two family tags in `content-tags.ts`), on people only. A curse is also `#negative`, so it raises `damaged` too — author *"answers when its bearer is cursed"* and *"answers when its bearer is hurt"* as the two different promises they are. Only landing raises them; a blessing or curse lifted or run out raises nothing new. A condition without the family tag never fires them, so tag new blessing/curse content with its family. Implementation: `src/engine/effects/reactiveWindow.ts`.
...
**Four primitives you may now rely on:**
...
4. **`prevent_loss` on the `condition` channel** (THR-1625, 2026-09-26) — had no reader; only `quintessence` was consumed. The condition writer `applyConditionToActor` (every fight, `inflict_condition`, aftermath `apply_condition`, Scarred) and the aftermath's `condition_attachment` path now ask it right after `tag_immunity`, and refuse with `reason: 'prevent_loss'`. Rules: it guards only **harmful** (`#negative`) conditions — a ward never stops a blessing; a guard with `tags` matches any one of them on the condition (`#`-normalised); a guard with **no** tags guards every harmful condition, permanently unless `consumeOnPrevent`. `amount` means nothing on this channel. A consumed **trait** guard removes only that bearer's `has_trait` edge (the definition is shared); a consumed artifact or agreement is removed outright. Aftermath refusals trace `failReason: 'prevent_loss'` naming the guard; a warded Scarred reads `scarSkipped: 'warded'` on the fight-ending trace. **Author an untagged guard with care** — it is blanket immunity to harm-conditions.
...
| The cells (slice 2) | `src/data/undertaking-cells.ts` — derived, never edited per cell | One template per declared (variant, type): `cell.undo.attachment`, `cell.control_seize.room`… resolvable by id at all times, walked only under the `cells` model |
| An ambition's cells | `strategicProfile.cells` on `src/data/ambition-templates.ts` | Which cells this ambition walks, ahead of its `templateIds` — the registration a cell needs to be reachable |
| A cell package (slice 3) | `{ slug, cell: { variant, objectTypeId }, override, profiles }` → `npm run compile:undertaking` | An authored override on a cell — display name, prose, cast, creation effects, mode, duration, catalysts, reach — compiled as `applyCellOverride(...)`, registered in the aggregate and the ambition's `cells`; at most `CELL_OVERRIDE_MAX_PER_CELL` per cell |
| The verb line-sets | `src/data/undertaking-verb-prose.ts` | Four activity lines, four completion lines and one GM narration per variant, with the four slots `{object}` `{owner}` `{actor}` `{place}` — filled by `resolveUndertakingProse` (`src/engine/undertakingProse.ts`) from the world, never from the template |
...
4. **Tier comes off the object, and a missing source is traced.** `tierOf` defaults to T2 and emits `undertaking_tier_defaulted`; the acceptance fails a type that defaults on more than `TIER_DEFAULT_SHARE_MAX` of its objects. The candidate carries `objectTier`, and the board and the checkpoints scale a cell's payoff and difficulty on it.
5. **A cell with nothing to act on says so, once.** Refused as `no_object_in_range:<type>` on the board trace's refusals and traced `undertaking_cell_unreachable` once per world per cell — `no_object_exists` (the world has none) or `no_owned_object` (it has some, none under the cell's ownership rule). The two are different worlds and want different fixes.
6. **Write prose with the four slots and nothing else.** `{object}` is the object's own name (a mark reads *the hold on <subject>*), `{owner}` its holder, `{actor}` the mortal, `{place}` the settlement the work stands in; `{Object}` etc. for sentence-initial. A slot the world cannot fill takes a neutral word; any other `{token}` is stripped (Law 43). The result names its concepts so a surface can chip them (Law 1).

**How to tell whether yours landed.** `npm run check:undertaking -- <cellId>` runs the ten blocks on a cell; `npm run check:undertaking-live -- <cellId> --seed 42` starts it on a seeded world under the cells model and reads `mutation_object` off the typed shape; the Package View (`?view=cms#undertaking-packages`) shows *The object*. With tracing on, `strategic_world_change` carries `undertakingVerb`, `objectTypeId`, `objectId` and `succeeded` for every cell completion; `undertaking_cell_unreachable` names every cell that could not. In the CLI: `traces 50` after a completion, or `eval` over `getTraces()`.
...
**Scene-targeting sentinels (THR-695, supersedes the THR-114 `role:` note):** `bindAftermathSceneTargets` runs at the top of aftermath dispatch (after the THR-555 reach-signature pass) and rebinds sentinel values on every field in `SCENE_SENTINEL_FIELDS` — **read that table in [`src/engine/sceneSentinels.ts`](../../src/engine/sceneSentinels.ts), not a list in prose.** It has grown six times (THR-1110, THR-1143, THR-1144, THR-1175, and twice under THR-1446) and the enumeration that used to sit here rotted every time; reading the stale copy instead of the table once manufactured a false critical finding (impediment #725). The sentinels themselves:
...
- **`'$actor'`** (THR-1025) → the action's `actorId`, kind-matched the same way.
- **`'$ascendant'`** (THR-1446) → **the player's god**, from `GameState.ascendantId`. The only sentinel that names nobody in the scene, which is exactly why it was missing: the others are all scene participants, and the ascendant is the player standing outside the scene. It binds `ascendantId` on the four `thread_*` kinds — the fields that made the whole `thread` consequence family unwirable, since the node id is minted per run as `asc.<archetypeId>` and no literal could ever name it.
- **`'$here'`** (THR-1446) → **the place the scene is happening at**, read from the actor's `located_at` and walked to the tier the field wants (a `location` field resolves up through `resolveToParentLocation`; a `sublocation` field takes the Place itself). Use this, not `$target`, whenever the consequence lands on a place: `$target` reaches a location only when the *card* targets one, so on a self-targeted encounter — most of them — it silently binds nothing. The resolver hops through `properties.avatarId` first, because an ascendant node carries no position of its own (measured seed 42/medium: `asc.archetype.chaos_0` has no `located_at`; `avatar.archetype.chaos_0` stands at `loc.start`).
- **`$realm`** (THR-1155) → **the Realm that holds the ground this scene is on**, and the sentinel realm content cannot be written without: a Realm's faction node is minted per world with a definition id of `realm.<cultureId>` over a *generated* culture, so neither a literal id nor `$faction:<defId>` can name one. It binds faction fields only (`factionId`, `targetFactionId`). **Resolved from the political map, not from the town:** `$here` gives the Location, the Location gives its hex, and the hex gives the nation out of the same `realmProjection` the border mesh draws — so an effect lands on the Realm the player can see holding that ground. Reading the Location's holder instead would be shorter and wrong: a holder may be a guild, an order or a monster faction, and the projection contains Realms only, so the map *cannot* return a non-Realm where the holder would need a remembered `factionClass` filter to be safe. **On ground no Realm claims it binds nothing** — *the realm that holds this town* must never quietly become *whoever holds this town*. Verified over 33 of 33 Realm-held towns on seed 42 / medium, sentinel and map agreeing, zero disagreements.
- **`$area`** (THR-1155) → registered, and **binds nothing on any effect field**, deliberately. No field in the aftermath effect union takes an Area: every node-id field on it names an agent, a faction, a location, a sublocation, an ascendant or a template. It is registered so the binder *consumes* it — an unregistered `$area` is not a sentinel, so the literal string passes through to be read as a node id, the silent death this vocabulary exists to end. Registered, it traces `UNRESOLVED` and the gate names the surface that does answer: **name an Area on a chip anchor**, whose `visualKind` carries `area`.

**The gate (THR-1446):** `sentinelBindabilityViolations` fails `check:encounter` on a sentinel pointed at a field it can never satisfy — `$ascendant` anywhere but the divine end of a thread, `$here` or `$actor` on a field that wants a place, `$realm` on any field that is not a faction, and `$area` **everywhere**, with the refusal naming the chip anchor as the surface that does answer. It deliberately says **nothing** about `$target`: the first cut ruled on it via the template's `targetCategories` and produced a false positive on `encounter.slice.the_table_that_holds`, which declares no categories yet resolves against a town. `targetCategories` filters what a *player* may aim a card at; it does not describe what an encounter resolves against.
...
| `trait.condition.location.under_watch` | 84 | *none* | Deliberate: being watched changes what you can *do* in a place, not how long it takes to walk in — so its reader is the **step modifier** below, not the tax |
| `trait.condition.location.harvest_blight` | 480 | ×1.6 | Pairs with the existing blight graph op |
| `trait.condition.location.tended_shrine` | 144 | *none* | Written by `encounter.shrine_offering` through `$here`; reader is the step modifier below |

**Three systems read it, which is the point** (UI Law 56 at world scope — a write nothing consumes is hollowness one level down). First, **template gating**: `requiredTargetTraits` already matched a location target's `has_trait` edges and now has content to match, so a festival-gated template becomes eligible while the festival runs and stops when it lifts. Second, **movement cost**: `LOCATION_CONDITION_MOVEMENT_TAX` (`src/data/condition-trait-content.ts`) maps a condition id to a multiplier on entering that place, multipliers compound, and the tax lifts by itself when the edge expires because the tax *reads the edge* — there is no second lifecycle to keep in step. A condition absent from that map costs nothing to travel through; that is the designed default, not an omission, because most of what can happen to a place is not about the road. Tuning travel feel is editing that one table (NFP #1).

Third, **step resolution** — `LOCATION_CONDITION_STEP_MODIFIER` (THR-1483), the same table shape answering the other half of the question: not what a place costs to *reach* but what it costs to *work in*. It maps a condition id to a per-reach additive term collected by `collectLocationConditionContributions` (`src/engine/resolutionModifiers.ts`) beside the terrain and faction terms that already read the same `locationId`. `under_watch` carries `{ shadow: -0.06 }` and `tended_shrine` `{ veil: +0.05 }`.

**What an author needs to know about it:**

- **It is reach-filtered, so it is a mechanism and not a mood.** A watched place costs Shadow and says *nothing* about carrying a beam. Write a condition whose penalty would apply to every reach and you have written weather, not a state.
- **It draws its own factor line.** A contribution surfaces on the test panel through `DERIVED_FACTOR_SENTENCES.condition`, so the player is told *which* condition tilted the step. A new `ModifierSourceKind` without a sentence pair is dropped silently by `deriveContributionLines` — the modifier would still move the roll while nothing explained it, which is exactly what the panel exists to prevent.
- **It resolves up one position tier, and no further.** The callers pass the actor's `located_at` target, which is the most specific node they occupy, so a Place also collects its enclosing Location's conditions — standing in the tavern of a watched town, you are in a watched town. It stops there on purpose: a hex is terrain, and a neighbouring settlement's watch is not your problem.
- **Terms add and then clamp** at `LOCATION_CONDITION_STEP_MODIFIER_CAP` (0.10), unlike movement taxes, which multiply. A step is one roll against one threshold; a place may lean on it hard without being allowed to decide it by accumulating bad seasons.
- **The effect line comes free.** `conditionEffectLine` reads this table as a third substrate, so a condition given a step modifier gets its player-facing sentence *derived from the term that moves the roll* — the sentence cannot drift from the mechanism. Conversely a condition with no substrate at all gets **no** sentence: `CONDITION_IDS_WITHOUT_EFFECT` is the (now empty) list of those, and a chip may not promise what the engine cannot enact.
...
**Verification:** `src/engine/__tests__/conditionOverflow.test.ts` (overflow pipeline), `src/engine/__tests__/conditionAttachment.test.ts` (aftermath effect), `src/engine/__tests__/conditionExpiry.test.ts` (expiry through the real decay loop — asserts the condition is *gone*, not merely that the edge was written), `src/engine/__tests__/locationConditions.test.ts` (places: write, sentinel binding, expiry, and all three readers falsified in each direction — including the tier walk, whose arm fails if the reader stops at the exact `located_at` node, and the sideways arm proving a sibling Place's condition stays its own).
...
> **Author these with sentinels, never with literals (THR-1446).** `ascendantId` takes **`$ascendant`** and `mortalId` takes **`$actor`** / `$cast:<key>`. Until 2026-09-10 neither field was a scene sentinel and the ascendant id is minted per run, so **every `thread_*` effect in the corpus was unwirable** — the handler found no edge and skipped with `thread_mutation_skipped · edge_missing`. The examples below were rewritten to the working shape; the skip-when-no-edge behaviour stays as the fail-soft for a mortal the god holds no thread to yet.

```typescript
// Strengthen after trust-building reaction
{ kind: 'thread_strengthen', ascendantId: '$ascendant', mortalId: '$actor', delta: 0.15 }

// Break the thread if the bond shatters
{ kind: 'thread_break', ascendantId: '$ascendant', mortalId: '$actor' }

// Branch a new thread from an existing one
{ kind: 'thread_branch', ascendantId: '$ascendant', sourceMortalId: '$actor', newMortalId: '$cast:protege', initialStrength: 0.4 }
```
...
**A dead route is gone, and its towns feel it (THR-1615).** When `phaseTradeRouteDecay` dissolves a lane it also removes the Route identity node (the `trade_route`-subtype location carrying `routeEdgeId`), so any holding on the road goes with it, and it pushes `SHOCK_TRADE_ROUTE_LOST` to both endpoint settlements (cause `trade_route_lost`). A Route object is only an object while its `routeEdgeId` edge stands, so content that claims, uses or changes a route never lands on a dead one. Content that wants to react to a road dying can read the `trade_route_dissolved` trace (`identityNodeIds` names the removed road) or the prosperity shock.
...
**Seven anchor forms are sentinels rather than literal ids**, because the object they name is minted per world: `$actor`, `$target`, `$artifact` (THR-1275), `$cast:<key>`, `$faction:<defId>`, `$here` (THR-1462 — the place the scene happens at, Location tier) and `$realm` (THR-1499 — the Realm whose political map claims the scene's hex; `$faction:<defId>` cannot reach a Realm because its definition id is minted per world as `realm.<cultureId>`). `$here` and `$realm` resolve through the same lookups the effect binder uses (`sceneHere.ts`, `sceneRealm.ts`), so a chip and the effect it reports cannot name different nodes; `$realm` is refused on a template whose effects never bind it, and resolves to nothing on unclaimed ground so the chip stays `named`. A realm-court standing chip anchors its `stateNoun` to `$realm` with `visualKind: 'faction'`, exactly as a guild chip anchors `$faction:<defId>`. Reach for one whenever the referent did not exist before this playthrough — a literal id that works in the authoring session and nowhere else is the exact dead link Law 56 clause 2 exists to remove. `$artifact` is the newest and closes the `possession` family's structural gap: `spawn_artifact` keys its node with the tick and effect index in the id, so before it the only anchor a possession chip could reach for was the *holder*, and every chip about a thing was a chip about a person. `check:encounter` refuses `$artifact` on a template that mints nothing.
