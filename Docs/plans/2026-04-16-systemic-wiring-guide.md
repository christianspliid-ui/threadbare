...
> **⚠️ The `revealFamilies` in this example are dead — do not copy them (measured 2026-07-29, THR-661; tracked as THR-844).** A family is matched by **`templateId.startsWith(family)`**, so each string must be a *prefix of a real template id*. Measured against the live 672-template pool: `investigation` → **0** matches, `mercenary` → **0**, `brinewall` → **0**. `social` (14) and `faction` (6) are live. A mark whose families are all dead is placed, decays, and can never be surfaced by anything.
>
> **Before you write a `revealFamilies` array, confirm each entry prefixes a real template id.** Live examples to copy instead: `encounter.anomaly.` (10 mortal-drawable templates — the "something here is wrong" family), `action.veil.` (4 — detect-magic, dispel, modify-enchantment, cast-spell), `social`, `faction`. `src/engine/__tests__/curseMark.test.ts` carries a scoped guard test asserting exactly this property; copy its shape.
...
- The mark goes on a **mortal**, so its `revealFamilies` must be prefixes the *mortal* can draw. This is where the dead-family trap bites hardest: `DIVINE_WORKING_REVEAL_FAMILIES` (`hex.` / `loc.` / `artifact.`) are the *ascendant's* casting families and match **zero** mortal-drawable templates. `CURSE_MARK_REVEAL_FAMILIES` in `src/data/ascendant-expression-constants.ts` is the correct-by-measurement example.
