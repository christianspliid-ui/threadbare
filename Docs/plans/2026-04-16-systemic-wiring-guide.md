...
**Four more graph-only ward ops, and the badge that proves they're wired (THR-605).** `attune_artifact`, `curse_artifact`, `nullify_artifact`, and `fortify_location` (`src/engine/ascendantWards.ts`, dispatched as `graphOpExecutor` cases) are the second reference for the essence-source pattern above — pure `(graph, op, ctx)`, no `GameState`/`rng`, fail-soft, each emitting an `ascendant_expression` trace. They reuse *already-consumed* substrate rather than inventing anything: attune/curse append an `AttachmentEffect` to the artifact's `properties.effects` (the same array the effect walker reads off `possesses`-edge artifacts — attune a deterministic sphere-flavored passive via `pickSphereFlavoredEffect(sphere, () => 0)`, curse a per-tick quintessence drain, the negative twin of the bestow regen); nullify clears that array plus the attune/curse/enchant property state (**set cleared keys to `undefined`, not omit — `graph.updateNode` shallow-*merges* properties, so an omitted key survives**); fortify raises `properties.fortificationMultiplier` toward a cap, read by `siegeResolution`. **The wiring is proven by the catalog badge, and the badge comes from the template's own step ops — not the engine registry.** `effectSourceFor` (`src/data/actionEffectSource.ts`) checks `stepHasOps` *first*: putting `onSuccess: [{ op: 'fortify_location', nodeId: '$target' }]` on the template flips its badge `not wired → wired · template`. So a composed op in `onSuccess` needs **no** `ENGINE_EFFECT_TEMPLATE_IDS` entry — that set is only for id-keyed bridges whose template carries *no* step ops (perceive/relay, hex-bridge, enchant). Don't add op-carrying templates there; it would be dead precedence. Attune deliberately skips the `aligned_with` edge the plan floated (sphere-node ids aren't a stable convention across worldgen) — the `attunedSphere` property stamp is the queryable alignment.
