...
**Axis B — breadth beats (THR-613 Slice 2b).** Where a Deepening marks a reach growing *deeper*, a **breadth** beat marks the god's dominion growing *wider*. The first breadth beat is the **essence-source milestone** (`beat.milestone.sources`): `phaseAscendantProgression` reads the ascendant's controlled-source portfolio via `readSourceMilestone` (`essenceSources.ts`) and, the first tick the god controls `MILESTONE_SOURCES_FOR_BEAT` sources (or holds a *flowering* one), enqueues a one-shot recognition beat (latched by `ascendant.properties.sourceMilestoneFired`) + a chronicle line. **For content authors:** the beat + its presentation + chronicle prose live in `src/data/ascendant-milestone-beats.ts` (`ASCENDANT_MILESTONE_BEATS`, `SOURCE_MILESTONE_PRESENTATION`, `sourceMilestoneChronicleProse`), catalogued in `ascendantBeat.ts`. Like the Deepening beats it is **prose-first, granting no card** — the entire source loop (find→claim→consecrate→sanctify→defend) is already granted by `beat.pool.invest.the_wellspring`, which is a hard prerequisite for controlling any source, so any source-verb grant would be a fake reveal of a card the god already holds (a genuinely-new *portfolio-tier* card is TODO THR-647). **The third breadth path is the shipped Ruins/Delve discovery pattern** (`src/data/ruins-delve-content.ts` — Clue → Delve → grants a Foundation-sphere card): it is not re-implemented here, only *named* as Axis B's discovery path so player progression reads as one spine. Force-fire the milestone for verification with `__DEBUG.fireBeat('beat.milestone.sources')`; read portfolio state with `__DEBUG.getAscendantProgression()` (`controlledSources`, `sourceMilestoneFired`).
