...
**`[consecrate] <location>`** — only on `temple` / `shrine` subtypes (subtype enum exists). Converts the location into a faith/domain-spreading site via a **sustained ControlEffect** (`perTickCost`, `src/types/controlEffect.ts`). Alternative: pay a high one-time cost to mint a **relic artifact** (`lossCondition: 'permanent'`) that sustains the consecration with **zero ongoing upkeep**. *Reuse:* subtype gating, `ControlSpec` sustained perTick cost/income, permanent-loss artifacts. *Decided (2026-06-26) — faith-spread:* each tick, every **threaded agent located at the consecrated site** accrues a small passive contribution toward their thread's **tier promotion** — deepening their divine connection (thread `tier`, 0–4) over time without the player spending essence (`CONSECRATE_DEVOTION_PER_TICK`). Reuses the existing tier-promotion machinery (`tierPromotion.ts`, `TIER_PROMOTION_THRESHOLDS`). *New (engine):* the co-located passive-devotion aura + the relic-as-upkeep-substitute wiring — **both reusable primitives, see below.** *Cost:* sustained per-tick essence, or high upfront for the relic.

**`[bestow power] <threaded agent>`** — requires the thread's `awareness` ≥ `faith` (awareness tiers exist: unaware/intuition/faith/communion). Grants the agent castable `ActivatedAbility` spells — start with two: (1) a **reach bonus** in the ascendant's domain, (2) a **quintessence-over-time** boon. *Reuse:* `ActivatedAbility` + `spellActivation.ts`, awareness gating, passive reach-bonus effect. *Decided (2026-06-26):* the quintessence spell raises the agent's **passive regen by +0.01/tick, capped at their max** (`BESTOW_QUINTESSENCE_REGEN = 0.01`), via the existing `resource_manipulate` per-tick path bounded by the cap. The reach-bonus spell is a passive reach modifier in the ascendant's domain. *New:* just the two spell definitions. *Cost:* one-time domain-relevant essence.
...
**Reusable primitives surfaced.** Design these as engine building blocks, not one-offs — we will author many more actions on top of them, so the investment compounds:

- **`relic_upkeep_substitute`** — a relic artifact whose existence pays a sustained ControlEffect's per-tick cost (high upfront, zero ongoing). Any future sustained control can be "bought out" by minting such a relic.
- **`co_located_thread_aura`** — a location effect applying a small per-tick mutation to every threaded agent currently at that location (here: devotion/tier progress; reusable for any per-tick boon or affliction to co-located threads).
- **`chosen_status_grant`** (anoint) — a typed status flag on a node + a power payload selected by node type × ascendant domain. Reusable for any "elevate this entity into a domain-aligned role" verb.
- **`sphere_flavored_effect`** (imbue) — pick a concrete effect from a sphere→effect table so one verb yields domain-appropriate magic. Reusable across every create/grant verb.

Consecrate's faith-spread and anoint's chosen-power are the two genuinely new engine effects; they may split into their own issues at implementation planning. New constants: `CONSECRATE_PERTICK`, `CONSECRATE_RELIC_UPFRONT`, `CONSECRATE_DEVOTION_PER_TICK`, `BESTOW_COST`, `BESTOW_QUINTESSENCE_REGEN` (0.01), `ANOINT_COST`, `BESTOW_MIN_AWARENESS` (`'faith'`).
