...
**Out of scope, flagged:** mastery traits (`subcategory: 'mastery'`, granted today only by mentorship graduation) lose a level every `MASTERY_DECAY_PERIOD` (48) ticks, and since THR-1503 deleted their success counter nothing reinforces them (`traits.ts:161-199`). That is time-based loss, not failure-based, so it is not one of the two traps. It is reported to Christian separately and not changed here.
