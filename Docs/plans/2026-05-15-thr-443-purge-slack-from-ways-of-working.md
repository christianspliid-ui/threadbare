...
> **Superseded (THR-823, 2026-08-03) — the two struck rows above.** `Docs/drafts/weekly-project-hygiene-SKILL.md` was a stale pre-installation draft and has been deleted; the task is installed. Its live pair is `Docs/ops/scheduled-task-prompts/weekly-project-hygiene.md` plus `C:\Users\chris\.claude\scheduled-tasks\weekly-project-hygiene\SKILL.md`, edited together per CLAUDE.md § Scheduled Tasks. Both rewrites were already applied there, so nothing is outstanding — the rows are struck because they name a dead edit target, not because the work is unfinished.
