...
Nine live undertaking cells wrote into systems nothing read. This wires their readers, opening three subsystems mortal work had never reached (Intelligence & familiarity, Ruins & delves, Spheres & quintessence). **Step 0 finding:** `spawnClue` typed its `precision` parameter as `number` while every consumer (`delveVariant`'s admission scan, `CLUE_MAX_AGE`) reads the `CluePrecision` string union — so the wanderer pack's live "Walk the Mark Down" template wrote clues no layer could ever act on. Narrowed to `CluePrecision`; the one authored call site now reads `'located'`, which is what its own completion prose already claimed.

| Wiring point | Where | Notes |
|---|---|---|
| Observe readers (R1) | `src/data/undertaking-objects.ts` | The shared `observe` semantic keeps `recordIntelligence` (additive) and gains per-kind familiarity via `seedKnowsOf`; a band-graded clue on ruin/wonder classes; a chart (`mintTreasureMap`) and a mark (`mintLeverageMark`) on `critical_success` only. Area resolves through `contains` edges (an Area is a `region` node; a Hex is not a node at all). Every reader traces, refusals included. |
| Outcome band plumbing | `strategicActionLifecycle.ts` → `undertakingResolver.ts` → `ObjectVerbContext.outcome` | Read off `checked.lastCheckpoint.band` at the multi-tick completion call — already resolved, so no second resolution. Optional: the instant path has no checkpoint and every reader takes the plain-success row. |
| Ruin → delve (R2) | `undertaking-objects.ts` `destroy` + `src/engine/ruins/delveVariant.ts` | The cell stamps `ruinMagnitude` (banded by pre-ruin subtype); the admission scan widens from `locationType === 'elder_ruin'` to *also* a `ruins` subtype whose `ruinedTick + RUINED_SETTLEMENT_DELVE_DECAY_TICKS <= tick`. Located-clue requirement unchanged. **`sphereAlignment` deliberately untouched** — the plan named `ruinSphereAlignment`, but the delve layer reads `sphereAlignment`, so a new name would have been another write nobody reads. |
| Holding income (R3) | `src/engine/holdingIncome.ts` (new) | `payHoldingIncome` / `phaseHoldingIncome` / `collectHoldingPayments` / `titheMultiplier`. Pays mortals only — a faction is an `actor` wearing `actorType: 'faction'`, filtered explicitly. `WEALTH_PROSPEROUS_HOME_INCOME` left unpaid on purpose (residence is observed, not stamped — THR-822). |
| Orchestrator phase | `holding_income` (`src/engine/orchestrator.ts`, after `trade_route_decay`, before `prosperity`) | Registered via `runInlinePhase` so its chronicle patch merges. Wrapped fail-soft. |
| Spell price (R4) | `src/engine/spellActivation.ts` + `undertaking-objects.ts` `use` | `payCosts` returns the capped `doom_increase` total as `soulPrice` instead of writing `doom`; the `use × Power` cell pushes a `QuintessenceEvent` (`source: 'spell_price'`) onto `pendingQuintessenceEvents` and refuses `spell_exhausted` while `exhaustedUntilTick > tick`. `health_sacrifice` still writes `doom`. **Also fixed:** `tick_exhaust` read the agent's `currentTick` property, which no writer sets, so every deadline landed in the past; `payCosts` now takes the caller's tick. |
| Army supply (R5) | `src/engine/armySupply.ts` | `hasReliefLine` filtered its `controls` sources to `actorType === 'faction'` — a faction is an `actor` node, so filtering on `node.type` would have matched nothing. |
| Traces | `src/types/trace.ts` | `undertaking_reader` registered at all four sites (`TraceCategory`, `TRACE_CATEGORIES`, `UndertakingReaderTrace`, `TraceEntry` union); `WealthDeltaTrace.reason` gains `'location_tithe'`. |
| Prose | `src/data/undertaking-verb-prose.ts` + `src/engine/undertakingProse.ts` | `{learned}` token on the observe completion lines, resolved by `learnedClause` from the knowledge edges themselves — the sentence and the graph cannot disagree (Law 56). Trailing clause, so an empty resolution still reads whole. `UNDERTAKING_PROSE_TOKENS` updated. |
| Chronicle | `src/data/economic-chronicle-content.ts` | `holding_income` trigger + significance 0.3 + five templates; rate-limited per holder by `HOLDING_INCOME_CHRONICLE_INTERVAL_TICKS`, tracked on the holder's own node. |
| Read model | `src/engine/agentDetail.ts` | `AgentDetail`/`AgentInfoCardData` gain `wealth`, `wealthSource`, `knownPlaces`; `describeWealthSource` turns the machine reason into a phrase, `collectKnownPlaces` reads `knows_of` + live `knows_clue_of`. Ungated with `holdings`. |
| UI — the live sheet | `src/components/Game/tabs/OverviewTab.tsx` | **Means** (wealth tier word + tooltip) and **Knows the way to** sections. **`AgentDetailPanel` is not mounted** — its own header says so and only its test imports it — so the rows went to `OverviewTab`, which `AgentProfileModal` actually renders; the same rows were added to `AgentDetailPanel` to keep it in sync as its comment asks. |
| UI — faction | `src/components/Game/FactionSheet.tsx` | **Means** section beside *What They Control*, reading `properties.wealth` off the node. |
| Debug bridge | `src/debug-bridge.ts` + `.d.ts` | `getHoldingIncome(actorQuery?)` and `getUndertakingReaders(actorQuery?)`, both trace-backed (require `enableTracing()`). |
| Grid + canon (R6) | `scripts/undertaking-grid-dispositions.ts`, `scripts/generate-undertaking-grid.ts`, `Docs/canon/undertakings.md`, `Docs/canon/rulebook.md` | `reader` on every `wanted` disposition (all 20 authored) and `readBy` replacing the nine paid-off `owes` notes; the generator fails by name on an empty `reader`. Canon carries the same-commit reader rule; the rulebook gains four `[IMPL]` rules of play. |
| Tests | `src/data/__tests__/undertaking-readers.test.ts`, `src/engine/__tests__/holdingIncome.test.ts`, `…/owedReaders-spell-and-supply.test.ts` (all new) + `ruins/__tests__/delveVariant.test.ts` | Each guard falsified at its owning layer: both band arms, the decay window's admit *and* refuse arms, a faction and a mortal controlling settlements in one world. |
| Known gap | `UNDERTAKING_MODEL === 'templates'` | R3's live population is **zero until the cells model flips** (THR-1403): measured on seed 42 medium at tick 150 — 0 `owns`, 0 strategic `controls`, 0 seized routes. The pass is correct and unit-falsified; it has nothing to pay yet. R1's clue fix *is* live on the same run (8 `undertaking_survey` clues, all `located`). |
...
**During a design session:** For each new engine module or UI component in your plan, list which integration surfaces it must connect to. Reference the specific checklist items below. If a surface doesn't exist yet, note that the surface itself must be created.

**During an execution session:** Before marking a feature complete, verify every connection listed in the plan's wiring section. Use this checklist as the canonical list of integration surfaces. If you discover a new surface, add it here.
...
Phases below ship as descriptors and are picked up automatically by `runRegisteredPhases` from the slot anchor in `runTick`. **The source of truth is `ENGINE_PHASES` in `src/engine/phases/index.ts` (26 descriptors as of 2026-08-29, THR-1365)** — verify there before trusting this table; inline legacy phases migrate opportunistically when touched.
...
| `post-doom` | `planted_compulsion_decay` (THR-886) | `src/engine/phases/plantedCompulsionDecay.ts` | `src/engine/plantedCompulsion.ts` |
| `post-resolution` | `autonomous_aftermath` (THR-530) | `src/engine/phases/phaseAutonomousAftermath.ts` | (same file) |
| `post-resolution` | `player_receipts` (THR-727) | `src/engine/playerReceipts.ts` | (same file) |
| `pre-economy` | `resource_stock_tiers` (THR-615) | `src/engine/phases/resourceStockTiers.ts` | `src/engine/resourceEconomy.ts` |
| `post-economy` | `route_events` | `src/engine/phases/routeEvents.ts` | (same file) |
| `post-economy` | `army_supply` (THR-626) | `src/engine/phases/armySupply.ts` | `src/engine/armySupply.ts` |
| `post-economy` | `economic_power` | `src/engine/phases/economicPower.ts` | (same file) |
| `post-economy` | `schism_resolution` (THR-430) | `src/engine/phases/schismResolution.ts` | `src/engine/factionTopology.ts` |
| `post-economy` | `personality_origin_seed` (THR-539) | `src/engine/phases/personalityOriginSeed.ts` | `src/engine/personality/originBaseline.ts` |
| `post-economy` | `core_personality` (THR-542) | `src/engine/phases/corePersonality.ts` | `src/engine/core/coreMechanics.ts` |
| `post-narrative` | `faction_succession` (THR-432) | `src/engine/phases/factionSuccession.ts` | `src/engine/anointSuccessor.ts` |
...
| 9.25 (orchestrator `6.6385`) | `phaseLocationTraits` | Location traits minted from the world's own scalars — *Welcoming* (prosperity), *Lawless* (unrest), *Veil-thin* / *Haunted* (saturation, the latter co-conditioned on `deathCount`) — with the promotion phase's hysteresis (enter / release / persisted sustain counter); released when the scalar recovers; `haunted` supersedes `veil_thin`; calls `touchWorld`; `location_trait` aggregate trace; read by `scoreAndSelect`'s `locationTraitBonus`, the movement tax, the step modifier and the location page (THR-790) |
| — (no phase; called from writers) | `artifactTraits` | Artifact traits (THR-1521) — the one writer of `has_trait` edges whose source is an `artifact` / `artifact_legendary` (`assignArtifactTrait` refuses a holding face, a non-artifact and a definition outside `trait.artifact.*`; seeds the definition on demand). Producers: `mintMasterwork` (`strategicGraphOps`) stamps *Storied* at level 1; `curse_artifact` / `nullify_artifact` (`graphOpExecutor`) write / remove *Cursed* beside THR-661's flag; the monster return's `cursed: true` op (`returnEngine`) gets the same edge; `recordArtifactEncounterPresence` from both growth roads (`encounter.ts`, `unifiedActionResolution.ts`) lifts *Storied* a level every `ARTIFACT_STORIED_ENCOUNTERS_PER_LEVEL`. Readers: `resolveTraitPredicate` (artifact as bearer), `ArtifactSheet` Traits chips, `__DEBUG.getArtifactTraits` / `stampArtifactTrait`, CLI `traits`; `artifact_trait` trace per change. The `condition_template` carve excludes `trait.artifact.*` unless `classes: ['artifact']` |
...
| `encounterSoundDesign` (THR-346) | **RETIRED IN FULL 2026-09-04 (THR-1168)** — no rows remain. **Moment 2** (registration cue) retired 2026-08-18 on mechanical grounds: its pitch map was keyed by the engine effect vocabulary (§4.1 `intelligence` / `hidden_mark` / `spawn_artifact`), which THR-1082 had already replaced on every player surface with six story-first consequence kinds sharing zero keys — the only available wiring would have played one fallback pitch forever. **Moment 1** (tension reveal) retired on the director's ruling in chat, *"no audio please"*: whether committing nudge cards should carry ~1.6s of held breath was a feel question, and the answer was no. | Module, hook (`useThreadReveal`), both test files and every `ENCOUNTER_*` constant are deleted. Moment 1 had had no consuming surface since THR-1049 removed the D1/D2 encounter screen and THR-1167 removed `ThreadOverlay`, leaving the hook as the sole caller of all three cues, reachable from its own tests alone. **The cue design of record survives as spec, not code** — canonical UI spec §3.3 keeps the full three-cue table with every timing and level; the implementation is in git history. Revival needs the ruling revisited first. |
...
> **Wiring checks only.** The pre-commit *gate law* — diff classification, test/ratchet/build, tree-diffing freshness gates run last, the 30-tick engine smoke, browser-verify — is `Docs/canon/verification-gates.md`; passing this wiring list does not discharge it.
...
Process steps that spawn subagents as part of the design-session workflow (the Cowork lane these rows predate was retired 2026-07-21, THR-654 — the steps live on in Claude Code design sessions). When adding a new agent-invocation step: (a) document it in CLAUDE.md § Design Governance, (b) document it in `game-design-direction/SKILL.md` § Finalization or the relevant skill, (c) add a row here.
...
## The `followOnTags` retirement (THR-1212 slice 6 — the last slice)

| Surface | Wiring |
|---|---|
| What was removed | The whole convention, not just the field the plan named: four declarations (`ClearanceGateConfig.followOnTags`, `ClearanceGateTransitionRule.addFollowOnTags`, `ClearanceGateRuntimeState.followOnTags`, `ClearanceGateTransitionRecord.addedFollowOnTags`), the four write paths that filled them (config seed, transition rules, the `clearance_gate_tag` aftermath effect, gate-duty branch `successTags`/`failureTags`), the single reader (`gateFollowOnSentence` + its `future_hook` chip), and the authored content that wrote it. |
| Evidence cited | Slice 4's ledger row — `clearance_gate_tag`, `write-without-consumer`, surfaced unprompted. **The row is gone from the ledger as a consequence of this deletion**, because membership is derived from the union; the tombstone now lives in this row, the changelog, and the sources file's header in the past tense. A deletion that removes its own evidence is the expected shape here, not a loss. |
| Predicate, not a line list | Derived by grep at execution time (THR-688 rule A). The plan's pinned lines (`contentShells.ts:44,70`, `unifiedAction.ts:415`, `clearanceGate.test.ts:248-249,337-340`) named **4 of the 14 files** that actually carried it — the transition-rule arm, the branch-tag path, the four authored `addFollowOnTags` in civic-guard content and the three `clearance_gate_tag` effects in guild content were all outside the list. |
| `future_hook` survives | The chip *kind* is untouched and still has live producers (`encounter_seed`). Only the gate-tag producer is retired — the veil's `future_hook` rendering and its category sort are unchanged. |
| Effect-kind allowlists | `src/data/content-eval/compositionContract.ts` and `src/testing/contentInvariants.ts` both enumerate the effect vocabulary; both updated in this PR. A retired kind left in either would keep authoring it legal. |
| Orphans swept | `touchedClearanceGateStates` / `nextClearanceGateStates` in `encounterAftermath.ts` (the effect was their only writer — the flag would have been permanently `false`), `gateRuntimeId` in `buildEncounterAftermathReactions` (whose `action` parameter became unused and was dropped), and `EffectShellGateTransitionTrace.followOnTagsAdded`. |
| Traces | No category added or removed. The `encounter_aftermath_effect` `effectKind` union loses one member. `EffectShellGateTransitionTrace` remains declared with **no emitter anywhere** — pre-existing and deliberately left alone, since pruning a never-emitted trace is dead-code work outside this predicate. |
| Interface map | **No row** — `clearance_gate_tag`, `followOnTags` and `clearanceGate` appear in neither `scripts/interface-contracts.ts` nor `Docs/canon/interface-map.md`, checked by grep. Nothing to retire or repoint. |
| Ledger guard, post-retirement | The old assertion `toEqual(['clearance_gate_tag'])` becomes `toEqual([])`, which is **vacuous on its own** — it also passes on an empty corpus or a broken classifier. It ships beside a population guard (both unions non-empty), a membership guard (`effectKinds` must not contain the retired id, so reintroduction fails by name), and the pre-existing synthetic `classifyRow` tests, which are uncoupled from any live member and so survive every future retirement. |
| Deliberately NOT wired | `followOnTags: ['proven_master']` on an effect-shell **result band** was deleted as one of the three conventions the ruling named, but the ledger row's claim that all three "rot in one field" is **wrong**: that one sits on `ResultBandConfig`, a type `src/types/contentShells.ts` does not declare at all (`effectShellRuntime.ts` imports it from there and gets nothing). That module's broken import surface is pre-existing red-baseline breakage and is not this slice's to fix. |
...
## Attention follows ambition — the spotlight pull (THR-1348)

Plan: `Docs/plans/2026-09-21-thr-1348-attention-follows-ambition.md`. A strategic-profiled ambition pulls its holder into the spotlight at assignment and swaps out the least-recently-witnessed spotlight mortal with no strategic ambition (never the avatar, a threaded mortal, an army commander or seated faction leader/commander, a mortal mid-act, or one already pulled). No new phase, no new node or edge type, no component edit. Every individual-actor `pursues` writer now routes through `assignAmbitionToActor`: `worldSeed` (protagonists), `gameInit` (the dev First), `agentLifecycle` (births), `mintInhabitant` (the binder — routes, but opts out of the pull with `skipSpotlightPull`: a support mint is an ambient face by contract), `ambitionTick` (mint and re-eval), `encounterAftermath` (the card).

| Module | Orchestrator phase | UI component | GameState field | Trace emitted | Debug visibility |
|--------|-------------------|-------------|-----------------|---------------|------------------|
| `engine/spotlightPull.ts` (new: `pullHolderIntoSpotlight`, `demotionCandidates`, `compareDemotionCandidates`, `countOverflowPulls`, `markWitnessed`, `collectBusyActorIds`, `readSpotlightLedger`) | called from `assignAmbitionToActor` — inside whichever phase assigns (worldgen · `2.4` births · the ambition phase's mint and re-eval · aftermath `assign_ambition`; the binder mint opts out) | — (the tier split already surfaces a pulled mortal: `hexMapAgentVisibility.ts`, `LocationView`) | actor properties `spotlightTier`, `spotlightPulledTick`, `spotlightPullDemotedId`, `spotlightPullTemplateId`, `spotlightPullRefusedReason/Tick`, `lastWitnessedTick` | `spotlight_pull` (one aggregate per tick, flushed by `phaseAmbitionProgress`) | `__DEBUG.getSpotlightLedger()`, CLI `spotlight`, `census:reachability`, `census:undertakings` |
| `engine/ambitionAssignment.ts` (extended: `findAmbitionTemplateById` lookup, `rng` / `seed` / `busyActorIds` / `extraProperties` options, returns `pull`) · `engine/ambitionTick.ts` (the two inline `pursues` writers routed through the helper) | ambition phase | — | `pursues` edges (byte-identical shape — `ambitionAssignment-routing.test.ts`) | existing `ambition_*` | — |
| `engine/npcGraduation.ts` (`demoteToTier`) | — | — | `spotlightTier` only | — | — |
| `engine/unifiedActionResolution.ts` (`markWitnessed` at the Event-node site) | encounter resolution | — | `lastWitnessedTick` on actor, actor target, bound cast | — | — |
| `engine/strategicKindReachability.ts` (`ALL_AMBITION_TEMPLATE_POOLS` default) · `scripts/kind-reachability.ts` (avatar excluded, summary per family, pulls named) · `scripts/undertaking-census.ts` (`spotlight` pulls / demotions / refusals) | instrument | — | — | — | census output |
| `data/ambition-templates.ts` (`forge_legend` gate → boost) · `engine/worldSeed.ts` (commanders `'spotlight'`) · `engine/gameInit.ts` (dev counterparties `'ambient'`) | worldgen | — | — | — | — |
| `data/agent-behavior-constants.ts` (`SPOTLIGHT_AMBITION_PULL_ENABLED`, `SPOTLIGHT_AMBITION_PULL_MAX`, `SPOTLIGHT_AMBITION_PULL_OVERFLOW_SHARE`, `SPOTLIGHT_WITNESS_WINDOW_TICKS`, `SPOTLIGHT_PULL_EVENT_SIGNIFICANCE`) | — | — | — | — | — |

Player controls: none — the pull is the world's; the god's lever stays the Kindled Ambition card, which pulls through the same helper. Prose pipeline: one chronicle line (`narrative`, `SPOTLIGHT_PULL_EVENT_SIGNIFICANCE`).
...
**A declared-but-unreachable half, recorded rather than implied — then closed.** `catalystQuery` is read by `maybeSeedCatalyst`, gated by `check:undertaking`, and was carried by 35 pack templates — all on the legacy arm under `UNDERTAKING_MODEL: 'cells'`, where a profile's `templateIds` are not walked (THR-1403). [THR-1497](https://linear.app/threadbare/issue/THR-1497) (2026-09-16) put it on the cell: `UNDERTAKING_CELL_CATALYSTS` in `src/data/undertaking-cells.ts` is read into every synthesised cell, and `UndertakingCellOverride.catalystQuery` lets a package override it. The same pass found the seeding site tracing every seed at `encounter_seed` — `undertaking_catalyst` was in `CONTENT_QUERY_SITES` and emitted by nothing — and reading a Place's (absent) subtype instead of its Location's; both fixed in `encounterSeeding.ts` (`seedQuerySite`, `seedTargetSubtype`). Reachability under the live model is a wiring-checklist question from here on: **a field is not wired until a template the live board walks carries it.**
...
## Fight steps read their opponent — fight block FB1 (THR-1537)

Plan: `Docs/plans/2026-09-23-fight-block.md` §1–3c. A step carrying `ActionStep.fightRole` is rated against its opponent's card, resolves at `FIGHT_STEP_SCALE`, may have its reach overridden by the card, and reads the fighter's standing modifiers in a combat context. No new phase, no new node or edge type, no component edit. No shipped template carries `fightRole` until FB7, so every live step is byte-identical (golden test untouched and green).

| Module | Orchestrator phase | UI component | GameState field | Trace emitted | Debug visibility |
|--------|-------------------|-------------|-----------------|---------------|------------------|
| `engine/fights/opponentCard.ts` (new: `readOpponentCard`, `defaultOpponentCard`, `deriveMightWord`, `readTemper`, `recoveredSegments`) | read at every fight step (pure) | — (plan doc 4) | reads `monsterState` / raw Iron / `reputationScore` / `trait.temper.*` | inside `fight.step` (`card`) | `fight.step` trace; `inspectOpponentCard` lands in FB7 |
| `engine/fights/fightStepInputs.ts` (new: `resolveFightStepInputs`, `fightRoleOf`, `stepScaleFor`, `isActionOnFightStep`, `resolveFightOpponent`) | unified-action progress, inside `resolveUncontestedStep` (the roll); the attended forecast joins in FB7 | — (forecast in FB7) | — (pure) | `fight.step` | `fight.step` trace |
| `engine/unifiedActionResolution.ts` (`resolveUncontestedStep` fight branch + `emitFightStepTrace`; `StepResolutionResult.reach/difficulty`; `executeStepResult`'s post-roll readers on `resolutionStats?.reach/difficulty`; crit severity via `stepScaleFor`; phase loop passes the resolved inputs and the fight-step predicate to band opposition) | unified-action progress | existing encounter veil | `unifiedActions[]` (unchanged shape) | `fight.step`; `resolution.input` reports the fight scale | trace viewer |
| `engine/resolutionModifiers.ts` (`computeResolutionModifiers(..., encounterType?)`) | — | — | — | — | — |
| `engine/groups/bandOpposition.ts` (`collectBandOppositions(..., isFightStep?)`) | unified-action progress | — | — | — (no `group_contested` for a fight step) | — |
| `data/fight-constants.ts` (new) · `types/fight.ts` (new) · `types/traces/fight-traces.ts` (new, `FightStepTrace`) · `types/trace.ts` (`fight.step` in the THR-928 trio) · `types/unifiedAction.ts` (`fightRole`, `opponentRef`, `difficultyContext: 'opponent_rated'`) | — | — | — | — | — |

**Declared, not yet reached:** an unbound `opponentRef`, a self-target or a deceased opponent currently rolls against `FIGHT_DEFAULT_CARD` with `opponentStatus` recorded on the trace. FB2 (THR-1538) routes all three through the no-roll end. *(Closed by FB2, below.)*

## fightState, the clock, early end — fight block FB2 (THR-1538)

Plan: `Docs/plans/2026-09-23-fight-block.md` §4–6. A fight carries `UnifiedAction.fightState`; clashes land on the opponent's clock; a set result ends the action early with `FIGHT_RESULT_ACTION_OUTCOME`; an opponent that never bound, died or left ends the fight through the no-roll route. No new phase, node type, edge type or component. No shipped template carries `fightRole` until FB7, so every live step is byte-identical.

| Module | Orchestrator phase | UI component | GameState field | Trace emitted | Debug visibility |
|--------|-------------------|-------------|-----------------|---------------|------------------|
| `engine/fights/fightClock.ts` (new: `advanceFightClock` — the one writer; `applyPerFightClockDelta`, `drainFightClockMailbox`, `clearStaleFightClockMailbox`) | inside the fight handler (unified-action progress); FB6 adds the effect-path callers | — (plan doc 4) | opponent node `monsterState.clockFilled` / `clockUpdatedTick`; the `pendingFightClockDelta` mailbox | `fight.clock` | `fight.clock` trace; `inspectOpponentCard` in FB7 |
| `engine/fights/fightState.ts` (new: `applyFightStepResult` — the handler; `checkFightContinuation`, `fightStateForNoRollEnd`, `withFightResultMemory`, `fightResultIndex`, `isLastFightStep`) | unified-action progress: `executeStepResult` (after the consequence, before growth) and `resolveUncontestedStep` (before the roll) | existing encounter veil | `unifiedActions[].fightState`; one `choiceHistory` entry at `fightResultIndex` | via `fight.clock` / `fight.end` | `getFightState` in FB7 |
| `engine/fights/fightOutcome.ts` (new: `onFightEnded` dispatcher, `FIGHT_END_BRANCHES` (empty), `finalizeFightEnd` — the call site both routes share) | after `advanceStep` on a rolled step; inside the no-roll branch | aftermath chips (plan doc 4) | `fightState.ending` / `opponentEnding` / `lairOutcome` (declared) | `fight.end` (once per fight) | trace viewer |
| `engine/unifiedActionLifecycle.ts` (`advanceStep` early-end branch) | unified-action progress | — | `resolved`, `outcome` | — | action list |
| `engine/unifiedActionResolution.ts` (`StepResolutionResult.fightEnd`; the loop pass-through; `executeFightNoRollEnd`; the shared tail extracted as `recordResolvedActionTelemetry` / `fireResolvedActionTriggers` / `withResolvedAftermathSummary` / `recordStepEventNode`; the trigger ladder fed `FIGHT_RESULT_ACTION_OUTCOME`; `storiedClimbs` recorded) | unified-action progress | existing aftermath | `unifiedActions[]` | `effect.event_raised` (`combat_ended`, site `fight_end`) on a mid-fight no-roll end | trace viewer |
| `engine/agentLifecycle.ts` (`MortalDeathCause` `'fight'`) · `engine/agentDetail.ts` (`DEATH_CAUSE_WORDS.fight`; `killerIsKnown` reads a fight death as seen) | — (callers land in plan docs 1 and 3) | agent sheet death line | — | — | sheet |
...
| Module | Orchestrator phase | UI component | GameState field | Trace emitted | Debug visibility |
|--------|-------------------|-------------|-----------------|---------------|------------------|
| `data/fights/fightBlock.ts` (new: `fightBlock`, `fightResultIndex`, `assertFightBlockTerminal`, `expandFightBlockSteps`, `FIGHT_DEFAULT_AFTERIMAGES`) · `scripts/compile-encounter.ts` (package pre-expansion) | — (authoring) | — | — | — | — |
| `data/encounters/fight-lair-confront.ts` (new: `FIGHT_LAIR_CONFRONT`) · `data/monster-encounter-content.ts` (registration) · `data/content-objects.ts` (`fight.` prefix, encounter kind) | spawn-only (plan docs 3 and 6; the debug lever) | existing encounter veil | `unifiedActions[]` | the fight traces | `spawnFight`; CLI `spawn fight` |
| `engine/fights/fightAdvantages.ts` (new: `readFightAdvantages` (pure), `fightAdvantageModifiers`, `spendFavourAtFightStart`, `spendSecretAfterClash`) · `engine/fights/fightAllies.ts` (new: `getCompanyMembersAtHex`, `agentHexOf`) · `engine/undertakingMotive.ts` (`isInjuryProvenance` exported) | fight start inside the handler; the forecast (pure) | forecast factor lines | `fightState.advantages` (with `spent`); `owes_favor.redeemed`, `knows_secret_of.revealed` | `favor_redeemed`, `secret_revealed`; the named terms ride `fight.step` | `getFightState(...).advantages`; `getFightForecastCheck(...).factors` |
| `engine/fights/fightState.ts` (advantages at fight start; the secret after its clash; complication `fight_clock` at step (4), `fight_momentum`, `fight_condition`, `fight_offer_quarter`) · `engine/fights/fightStepInputs.ts` (advantage terms; `fightComplicationScope`) | unified-action progress (`executeStepResult`) | — | `fightState` | `fight.clock` (cause `complication`), `fight.fork` (trigger `quarter`) | `getFightState` |
| `engine/complicationSelection.ts` (`inFight` scoping; the authored pool; `{opponent}`) · `engine/complicationEffects.ts` (the four fight effects are no-ops there) · `types/complication.ts` · `data/complication-templates.ts` (12 `complication.fight.*`) | the existing per-step complication pipeline | existing complication prose | `stepComplications` | `complication_selection` | trace viewer |
| `engine/scaledForecast.ts` (new: `forecastActionAtScale`) · `components/Game/encounter-stage/adapters/buildNudgePhaseModel.ts` (fight inputs, `forecastScale`, fight factor lines, the Whisper's next demand) · `components/Game/encounter-stage/useNudgeHand.ts` · `components/Game/encounter-stage/adapters/buildUnifiedEncounterStageModel.ts` (header threat/reach; Scene So Far reach from `StepProseRecord.reach`) · `engine/chapterArchive.ts` (the archived step label) | UI (pure) | encounter veil forecast, header, Scene So Far; the chapter ledger | reads `fightState`, the card, standing modifiers | — | `getFightForecastCheck(actionId, cardIds?)` |
| `engine/unifiedActionResolution.ts` (`deriveStepRollInputs` / `buildStepCoreInput` extracted from `resolveUncontestedStep`; `previewStepProbability`; the complication context's `fight`; the handler receives the complication's effects) | unified-action progress | — | — | unchanged | `getFightForecastCheck` |
| `debug-bridge.ts` / `.d.ts` (`getFightState`, `inspectOpponentCard`, `spawnFight`, `getFightForecastCheck`) · `engine/debugEncounterTools.ts` (`targetId` spawn option) · `scripts/cli.ts` (`spawn fight`) · `scripts/calibrate-fights.ts` + `testing/fightCalibration.ts` (`npm run calibrate:fights`) | — | DebugPanel / console | — | — | as named |

## The monster card — Monsters M1 (THR-1544)

Plan: `Docs/plans/2026-09-23-monsters-as-opponents.md` §1–3. A lair's named elite is minted with a `monsterState` card and a temper trait edge; legendary escalation hardens the card; one `isMonster` predicate keeps monsters out of systems meant for mortals. No new phase, no new node or edge type, no component edit (the monster's surface is plan doc 4).

| Module | Orchestrator phase | UI component | GameState field | Trace emitted | Debug visibility |
|--------|-------------------|-------------|-----------------|---------------|------------------|
| `engine/monsters/monsterCard.ts` (new: `mintMonsterCard`, `hardenMonsterCard`) · `engine/lairEscalation.ts` (`createNamedElite` calls the mint; the major → legendary tier calls the hardening) | `phaseLairEscalation` (existing) | — (plan doc 4) | elite node `monsterState`; `has_trait` → `trait.temper.*` | `monster.minted`, `monster.hardened` | `__DEBUG.listMonsters`, `__DEBUG.inspectOpponentCard`, CLI `monsters` |
| `data/monster-families.ts` (new: `MONSTER_FAMILIES`, `MONSTER_CLOCK_BY_TIER`, `MONSTER_LEGENDARY_DREAD_STEP`, `MONSTER_FAMILY_FALLBACK`) · `data/temper-trait-content.ts` (new: `TEMPER_TRAIT_DEFINITIONS`) · `types/monster.ts` (`MonsterState`, `MonsterFamilyId`) · `types/traits.ts` (`TraitCategory` `'temper'`) | seeded at world init (`traitDefinitionSeeding.ts`) | — | — | — | `check:attachment`, `contentTags.test.ts` |
| registrations: `data/content-objects.ts` (Trait kind `trait.temper.` + catalog) · `data/contentCatalogs.ts` · `data/content-tags.ts` (`#temper`) · `engine/attachmentTemplateIndex.ts` (hover) · `data/world-objects.ts` (`TRAIT_SUBCATEGORIES` `'temper'`) | — | trait tooltip (existing) | — | — | `worldObjects.test.ts` |
...
| Module | Orchestrator phase | UI component | GameState field | Trace emitted | Debug visibility |
|--------|-------------------|-------------|-----------------|---------------|------------------|
| `engine/fights/opposedRoll.ts` (new: `rollOpponentSide`, `duelOpponentId`, `opponentStream`, `mirrorFightState`, `opponentSideAction`) · `engine/unifiedActionResolution.ts` (`StepResolutionResult.opponentRoll`; `executeStepResult`'s `resolutionStats.opponentRoll` / `noComplications`; the phase loop carries it) | unified-action progress, inside `resolveUncontestedStep` after the fighter's core | veil header (plan doc 4 F2) | `fightState.opponentBands` | `fight.step` (`opponentBand`, `opponentProbability`, `fighterClockNow`) | `getFightState` |
| `engine/fights/fightState.ts` (`applyDuelStepResult`: the matrix, both clocks, both sides' harm / conditions / momentum / events / mailbox drains) · `engine/fights/fightStepInputs.ts` (no `opponentModifierDelta` in a duel) | `executeStepResult` | — | `fightState` (`fightMode`, `fighterClockSize`, `fighterClockNow`, `opponentLoss`, `opponentMomentum`, `opponentAdvantages`, `opponentWounds`, `opponentBlowsLanded`, `opponentHarmTaken`) | `fight.clock`, `fight.end` (`opponentLoss`) | `getFightState` |
