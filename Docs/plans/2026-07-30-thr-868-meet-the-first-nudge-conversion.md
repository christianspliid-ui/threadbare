...
- **Browser verification runs `?view=game&firstunmet&size=medium` (THR-874).** An earlier revision of this line named bare `?view=game` as the route; that was wrong and blocked every browser Done-when on this surface. `?view=game&seeded` pre-bonds The First (`isMeetTheFirstAvailable` → false), and bare `?view=game` supplies **no ascendant identity**, while `GameView` mounts the flow only on `meetingState && ascendantIdentity` — so neither URL can ever render the beat. THR-874 split the dev seeding flags so `firstunmet` gives identity without the bond. Drive with `window.__DEBUG.tick(n)` (never the Play button — `document.hidden` throttles an automated tab to ~1 tick/click, THR-689) and assert via `window.__DEBUG.getMeetingState()`.
