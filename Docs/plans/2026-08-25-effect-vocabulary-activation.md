...
> **Shipped 2026-08-26 — three corrections this plan's text needed, recorded so the next reader does not re-derive them.**
> (a) **The named subfamily does not exist.** There is no wound/disease/curse field on a condition; every entry in `CONDITION_TRAIT_DEFINITIONS` carries `#negative` or `#positive`, and that tag is the live predicate (`isHarmfulCondition`, `src/engine/effects/conditionProxyEvents.ts`). Writing to the named-but-absent field would have produced one more phantom property with no writer — the class this whole program exists to retire.
> (b) **`conditionDecay.ts` is where the raise must *not* go.** The plan lists it among the raise sites, but it is the **natural**-expiry path, and `healed` is defined here as removal *before* natural expiry — so wiring it would have contradicted the stage's own definition and fired every ward in the world on every decay sweep. The property is enforced by the module simply not being called from there, and pinned by a test that expires a condition naturally and asserts silence.
> (c) **Three aftermath writers, not one.** `apply_condition`, `remove_condition` **and** `condition_attachment` all write the same `has_trait` edge; the third is the kind every shipped `trait.condition.wounded` in the tavern package actually authors, so wiring only the first would have left the busiest path silent while the stage read as done.
>
> **Scope left open:** a fourth writer (`actionTriggerPayloads.ts`) is live and still raises nothing, and the conditions it grants sit in catalogs tagged topically rather than by polarity — the two must be fixed together or the raise is live and silently misclassifying. Ticketed as **THR-1257**.
