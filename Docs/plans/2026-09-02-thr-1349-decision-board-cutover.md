...
## Executor amendments (slice 3, 2026-09-02)

Two numbers in the Constants table above were corrected at execution, with the reasoning here rather than silently in the diff; the design is unchanged.

- **`CENSUS_VARIETY_SAMPLE_STARTS` 300 → 150, `CENSUS_DISTINCT_AT_SAMPLE_FLOOR` 30 → 25.** The sample must be one every arm passing the throughput floor beside it can fill, or the variety gate is a second throughput gate wearing the wrong name — the vacuity the constants file has shipped twice. The throughput floor (4 per mortal per 100 ticks, ~27 mortals, 150 ticks) underwrites ~162 starts, so 150 is the largest sample it guarantees. 300 was inconsistent with the plan's own floor: the live board at its calibrated weight starts 297–324 on seed 42, and the first census run under `'live'` reported *"sample of 300 never filled"* while the throughput gate beside it passed. The floor at the 150-sample: contest B 36 / 41, the calibrated board 27 / 27, the uncalibrated (0.18) board 23 / 24 — 25 separates them on both seeds with a two-template margin.
- **`BOARD_VARIETY_PENALTY_WEIGHT` 0.5 → 0.7.** The plan chose 0.5 on the 300-sample column, where 0.5 and 0.7 tie (36 / 36 vs 35–36 / 34–36). At the 150-sample the census gates on, 0.7 is the plateau on both seeds (27 / 27) while 0.5 sits on the floor on seed 99 (27 / 25). Starts, envelope shares and throughput stay inside every gate at 0.7 (303 / 324 starts; undertaking share 18–25%).
- **`'shadow'` is no longer a playable mode.** With contest B deleted nothing else selects undertakings, so under `'shadow'` the board records and zero undertakings start (measured on both seeds). The mode constant's docblock says so; a future scorer needs its own shadow channel.

The kill criteria were not tripped: the one red gate on the first live run was the sample-size inconsistency above, a defect in the gate's derivation rather than in the board, and the corrected gates pass on both seeds under `'live'` (the closing PR carries the run).
