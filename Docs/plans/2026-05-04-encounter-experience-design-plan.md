...
- **Chronicle prose** for tier-3 beats (doom escalations, mandate milestones) — *superseded: runtime LLM generation is a rejected approach (Docs/canon/prose.md), and "literary quality" as a tier target predates the register model; Chronicle prose is authored/template prose at baseline register with doom transitions as the peak surface.*
