# Attachment Upgrade Pipeline: Anomaly Rewards
> Slug: upgrade-anomaly-rewards | Pass: draft | Mode: upgrade
> Items: 24 items (10 artifacts, 8 bestowed powers, 6 conditions) | Date: 2026-04-06

## INVALID REACH REMAPPING

Two items use reach domains that do not exist in `ReachDomain`:
- `anomaly_moonpearl_strand` uses `spirit` -> remapped to `heart` (devotion, soothing dead)
- `anomaly_crystal_headache` uses `mind` -> remapped to `eye` (cognitive/perceptual domain)

---

## Signature Artifacts (10)

### 1. Uncut Ruby (`anomaly_uncut_ruby`)

**Niche:** Wealth beacon — raw precious stone that draws trade and attention.
...
### 2. Resonance Shard (`anomaly_resonance_shard`)

**Niche:** Arcane tuning fork — amplifies magic at a cost. High-tier crystal with unstable resonance.
...
### 3. Amber Phial (`anomaly_amber_phial`)

**Niche:** Healing draught — one-shot curative with lingering warmth.
...
### 4. Herb Bundle (`anomaly_herb_bundle`)

**Niche:** Simple field medicine — limited charges, wilderness bonus.
...
### 5. Sealed Codex (`anomaly_sealed_codex`)

**Niche:** Ancient knowledge repository — deep lore with stacking mastery.
...
```typescript
{
  ...
  properties: {
    ...
    mechanicalSummary: '+0.10 Eye, +0.05 Star, +0.01 Eye per exploration success (max +0.05)',
...
### 6. Corroded Crown (`anomaly_corroded_crown`)

**Niche:** Cursed wealth amplifier — powerful gold bonus with social tradeoff.
...
### 7. Fossilized Eye (`anomaly_fossilized_eye`)

**Niche:** Ancient watcher — passive perception with reactive awareness burst.
...
### 8. Star Metal Shard (`anomaly_star_metal_shard`)

**Niche:** Cosmic weapon fragment — fate-touched iron with cycling power.
...
### 9. Moonpearl Strand (`anomaly_moonpearl_strand`)

**Niche:** Devotional vestment — calming presence with social resonance.

NOTE: Original `reachBonus: { spirit: 0.15 }` uses invalid reach `spirit`. Remapped to `heart` (devotion, soothing, calming the restless dead aligns with Heart domain).
...
### 10. Spore Lantern (`anomaly_spore_lantern`)

**Niche:** Hallucinogenic vision tool — reveals hidden things but unreliably.
...
```typescript
{
  ...
  properties: {
    ...
    mechanicalSummary: '+0.05 Eye, +0.05 Eye during exploration, +0.02 Veil / -0.02 Heart (spore haze)',
    lossCondition: 'permanent',
...
### 11. Prospector's Eye (`anomaly_prospectors_eye`)

**Niche:** Resource-spotting talent — exploration advantage with mineral awareness.
...
### 12. Crystal Attunement (`anomaly_crystal_attunement`)

**Niche:** Magical sensitivity — heightened arcane perception, mystical context expert.
...
### 13. Sap-Blessed (`anomaly_sap_blessed`)

**Niche:** Living healer — plants respond, wounds close easier. Social and healing affinity.
...
### 14. Herbalist's Knowledge (`anomaly_herbalists_knowledge`)

**Niche:** Wilderness medic — simple knowledge with practical application.
...
### 15. Vault Scholar (`anomaly_vault_scholar`)

**Niche:** Ancient knowledge savant — lore and construction expertise from pre-collapse study.
...
### 16. Tide Reader (`anomaly_tide_reader`)

**Niche:** Navigation sense — fate-touched wayfinding, water-affinity.
...
### 17. Spore-Touched (`anomaly_spore_touched`)

**Niche:** Altered perception — sees beyond the veil, but withdrawn from social world.
...
### 18. Ironblood (`anomaly_ironblood`)

**Niche:** Star-forged constitution — combat resilience with fate alignment.
...
### 19. Crystal Headache (`anomaly_crystal_headache`)

**Niche:** Cognitive impairment — disorientation from resonance overload.

NOTE: Original `domainContributions: { mind: -0.10 }` uses invalid reach `mind`. Remapped to `eye` (perceptual/cognitive domain).
...
### 20. Golden Euphoria (`anomaly_golden_euphoria`)

**Niche:** Blissful impairment — warm social boost, combat dulled.
...
### 21. Vault Curse (`anomaly_vault_curse`)

**Niche:** Fate penalty — misfortune that worsens over time.
...
### 22. Brine Lungs (`anomaly_brine_lungs`)

**Niche:** Physical debilitation — weakened endurance with combat penalty.
...
### 23. Spore Visions (`anomaly_spore_visions`)

**Niche:** Perception overdrive — extraordinary sight at cost of social withdrawal.
...
### 24. Fossil Whispers (`anomaly_fossil_whispers`)

**Niche:** Ancient murmurs — fragments of lost knowledge seeping in.
...
| # | Name | Type | Tier | Niche | Primitives Added | Total Value | Changes |
|---|------|------|------|-------|-----------------|-------------|---------|
| 1 | Uncut Ruby | artifact | T2 | Wealth beacon | passive + conditional + behavior_weight | 0.15 | +effects[], -reachBonus, updated summary |
| 2 | Resonance Shard | artifact | T3 | Arcane tuning fork | passive x2 + conditional + test_shaper | 0.20 | +effects[], -reachBonus, kept onUseTriggers |
| 3 | Amber Phial | artifact | T2 | Healing draught | passive + consumable_charge | 0.11 | +effects[], -reachBonus, kept onUseTriggers |
| 4 | Herb Bundle | artifact | T1 | Field medicine | conditional + consumable_charge | 0.06 | +effects[], -reachBonus, kept onUseTriggers |
| 5 | Sealed Codex | artifact | T3 | Ancient knowledge | passive x2 + stacking | 0.15+ | +effects[], -reachBonus, kept onUseTriggers |
| 6 | Corroded Crown | artifact | T2 | Cursed wealth | passive + tradeoff | 0.15 | +effects[], -reachBonus, kept onUseTriggers |
| 7 | Fossilized Eye | artifact | T2 | Ancient watcher | passive x2 + range_modifier | 0.08 | +effects[], -reachBonus, updated summary |
| 8 | Star Metal Shard | artifact | T3 | Cosmic weapon | passive x2 + cooldown | 0.13+ | +effects[], -reachBonus, updated summary |
| 9 | Moonpearl Strand | artifact | T2 | Devotional vestment | passive + conditional + social_modifier | 0.13 | +effects[], -reachBonus, spirit->heart remap |
| 10 | Spore Lantern | artifact | T2 | Hallucinogenic tool | passive + conditional + tradeoff | 0.12 | +effects[], -reachBonus, kept onUseTriggers |
| 11 | Prospector's Eye | bestowed | T2 | Resource spotter | passive + conditional + behavior_weight | 0.13 | +effects[], -domainContributions |
| 12 | Crystal Attunement | bestowed | T3 | Magical sensitivity | passive x2 + conditional + tag_immunity | 0.18 | +effects[], -domainContributions |
| 13 | Sap-Blessed | bestowed | T2 | Living healer | passive + conditional + prevent_loss | 0.09 | +effects[], -domainContributions |
| 14 | Herbalist's Knowledge | bestowed | T1 | Wilderness medic | passive + conditional | 0.09 | +effects[], -domainContributions |
| 15 | Vault Scholar | bestowed | T3 | Ancient savant | passive x2 + conditional + trait_grant | 0.17 | +effects[], -domainContributions |
| 16 | Tide Reader | bestowed | T2 | Navigation sense | passive + conditional + range_modifier | 0.14 | +effects[], -domainContributions |
| 17 | Spore-Touched | bestowed | T2 | Altered perception | passive x2 + tradeoff | 0.14 | +effects[], -domainContributions |
| 18 | Ironblood | bestowed | T2 | Star-forged fighter | passive x2 + conditional | 0.15 | +effects[], -domainContributions |
| 19 | Crystal Headache | condition | T1 | Cognitive impairment | passive + conditional | -0.09 | +effects[], -domainContributions, mind->eye remap |
| 20 | Golden Euphoria | condition | T1 | Blissful impairment | passive x2 + social_modifier | net +0.02 | +effects[], -domainContributions |
| 21 | Vault Curse | condition | T2 | Escalating misfortune | passive + stacking | -0.08 to -0.13 | +effects[], -domainContributions |
| 22 | Brine Lungs | condition | T1 | Physical debilitation | passive + range_modifier | -0.06 | +effects[], -domainContributions |
| 23 | Spore Visions | condition | T2 | Perception overdrive | passive x2 + range_modifier | net +0.02 | +effects[], -domainContributions |
| 24 | Fossil Whispers | condition | T1 | Ancient murmurs | passive x2 + axiological_drift | 0.08 | +effects[], -domainContributions |

### Primitive Variety

| Primitive | Count |
|-----------|-------|
| passive | 24 (base layer for all) |
| conditional | 12 |
| behavior_weight | 2 |
| test_shaper | 1 |
| consumable_charge | 2 |
