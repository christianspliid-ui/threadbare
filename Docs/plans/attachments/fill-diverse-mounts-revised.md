...
> Category: mounts_beasts | Slug: fill-diverse-mounts | Pass: revised
...
| # | Item | Change | Reason |
|---|------|--------|--------|
| 2 | Cindermaw | Renamed to "Smoke-Tooth" | "Cindermaw" is a generic fantasy compound ([dark syllable]+[body part]); flavor text describes smoke leaking from teeth -- name pulls from that specific detail |
| 3 | Veilstag | Renamed to "Shimmer Hart" | "Veilstag" is literal [ReachName][Animal]; the creature's most distinctive feature is the heat-shimmer trail and its hart form |
...
**Niche:** Arcane tracker. The Shimmer Hart is drawn to places where the boundary between worlds is thin -- hence the awareness range bonus (it senses disturbances the rider cannot) and the mystical conditional. It moves gently through terrain that confounds normal beasts, always finding the path between. More explorer than warhorse.
...
| 2 | Smoke-Tooth | Shadow+Iron | T3 | passive x2, range_modifier, trait_grant, aura | 0.10 Shadow + 0.03 Iron | permanent | 5 |
| 3 | Shimmer Hart | Veil | T2 | passive, range_modifier, conditional | 0.07 Veil | stealable | 3 |
...
| Veil | 2 | T2, T3 | Phase-hart tracker + ley-line familiar with immunity |
...
| trait_grant | 3 | Smoke-Tooth, Sorrowheart Mare, Pale Pilgrim |
| aura | 3 | Smoke-Tooth (enemy debuff), Sorrowheart Mare (ally buff), Pale Pilgrim (ally buff) |
| conditional | 2 | Dustwalker, Shimmer Hart |
...
| T2 | Shimmer Hart | 0.04 + 0.03 cond = 0.07 | 3 | Yes (T2: 0.05-0.08) |
| T2 | Hearthbound Hound | 0.06 passive + 0.04 reactive burst | 4 | Yes (T2: 0.05-0.08 base, reactive is conditional) |
| T3 | Smoke-Tooth | 0.10 + 0.03 = 0.13 passive, -0.03 aura enemy | 5 | Yes (T3: 0.08-0.12, slight over but aura is utility) |
...
**Aura design:** Three mounts use auras, each differently. Smoke-Tooth debuffs enemy Eye (shroud). Sorrowheart Mare buffs ally Heart (calm). Pale Pilgrim buffs ally Star (fate). All at radius 1, keeping the effect local and requiring positioning decisions.
