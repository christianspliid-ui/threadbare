...
> Slug: upgrade-wounds-t1t2 | Pass: draft | Mode: upgrade
...
**Niche:** Movement impairment. A deep leg wound slows travel and makes running impossible. The flesh penalty is secondary to the real problem: getting anywhere takes longer. Heals over time but slower than surface wounds. Movement cost penalty makes this wound feel distinct from arm/rib injuries -- it changes how the agent navigates the map, not just how they fight.
...
```typescript
{
  ...
  properties: {
    ...
    maxLevel: 1,
    visibility: 'public',
    mechanicalSummary: '-0.04 Flesh (decays over 30 ticks), +30% movement cost while wounded',
    flavorText: 'The bandage is soaked through again. Walking is a negotiation with pain.',
    effects: [
      { type: 'decay', reach: 'flesh', startValue: -0.04, changePerTick: 0.0013, limitValue: 0, destroyAtLimit: true },
...
**Total reach value:** -0.04 Flesh initial, decays over ~30 ticks (~2.5 game days). +30% movement cost for the wound's entire duration.
...
```typescript
{
  ...
  properties: {
    ...
    maxLevel: 1,
    visibility: 'public',
    mechanicalSummary: '-0.07 Iron, -0.05 Flesh; worsens when damaged (-0.03 Iron for 6 ticks)',
...
**Total reach value:** -0.07 Iron / -0.05 Flesh sustained; spikes to -0.10 Iron for 6 ticks after taking damage (12-tick cooldown prevents permanent stacking).
...
| 2 | Gashed Leg | T1 | Movement impairment, slow heal | decay + range_modifier | -0.04 Flesh + 30% move cost | +effects[], -domainContributions, updated summary |
...
| 5 | Deep Stab Wound | T2 | Serious, worsens under stress | 2x passive + reactive(duration) | -0.07 Iron / -0.05 Flesh + spike | +effects[], -domainContributions, updated summary |
...
All items are within the 0.15 per-item cap (EFFECT_PER_ITEM_CAP). These are debuffs (negative values), so the cap applies in reverse -- no single wound imposes more than -0.15 total reach penalty. The highest total is Deep Stab Wound at -0.12 base (-0.07 Iron + -0.05 Flesh) with a temporary -0.03 spike on damage.
