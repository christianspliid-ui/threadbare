...
> **lint_plan_doc:** exempt — program plan in the pre-template shape; edited in place only for dated refinement markers (THR-1372).
...
3. **Nudge options are per-encounter authored content.** Quite-a-few options per encounter; mostly not reused across encounters; only generics (blessing, steady-breath class) are shared. Sphere tags + unlock gates + trait gates filter what a given god-and-agent pair can play.
   ...
   > *Reversed 2026-07-30, marked 2026-08-29 (THR-1324).* **The THR-883 communication pivot inverted this ruling's default:** cards are now **library-generic and reusable** — "prose does the scene, cards do the rules" — drawn from the 21-type card library, with **zero scene-bespoke prose** on a card face. Per-encounter bespoke cards are the exception (authored specials), not the rule. Prose Doctrine v2 then retired the card flavor quote outright. What survives verbatim: the gating model — sphere tags, unlock gates and trait gates filter the playable hand. See [`2026-07-30-nudge-card-repertoire.md`](2026-07-30-nudge-card-repertoire.md) § Decision 4 and [`nudge-authoring-spec.md`](../../.claude/skills/encounter-pipeline/reference/nudge-authoring-spec.md).
4. **Unavailable options are hidden entirely** (not dimmed) *(refined in the shipped hand: the Repertoire partitions playable / dimmed / hidden — dim when the god could pay the price, hide when they could not; gated cards — trait, group, favor — stay hidden when unmet, as ruled here; marked 2026-08-29, THR-1372)*. Many options per encounter × different gods × different agents × different traits = the replayability engine.
