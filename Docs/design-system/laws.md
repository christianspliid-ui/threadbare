...
13. **[E] No raw magnitudes on any mortal-facing surface.** Difficulty is `gentle/fair/steep/severe`; forecast is `doomed/perilous/uncertain/favorable/fated`; derived change sentences band their deltas the same way (`engine/aftermathWords.ts`). Numbers live in traces and the designer view only. No percentages anywhere player-facing. One exception, ratified 2026-08-06: **resource-pool balances in persistent chrome** (the essence counter) may render as whole-number numerals — a pool balance is not a card face, and forty pips would be unreadable (THR-890). Fractional float noise is still a defect. (Ruling 6, THR-772/868; THR-1004.)
    - **Amended 2026-08-12 (THR-1082), ratified by Christian in chat the same day.** **Consequence chips render magnitude *visually* — the delta cluster, three rough steps banded from the same data ladders — with the banded word in the tooltip.** A visual magnitude is not a raw numeral; the numeral ban stands unchanged. The amendment exists because obeying this law as originally written produced *"Vara's Stone grew steadily"*, and Christian's verdict on that line was: **"what does steadily even mean? how can a player use that word to gage anything"**. The word ladder is a correct answer to "don't print `0.05`" and a wrong answer to "how much?" — an adverb carries no scale a reader can place against the adverb above it. So the ladders survive as *data* (other systems band against them, and the word is still the hover-tier detail) and stop being the headline reading. **An executor who reads this law without the amendment will re-derive `steadily` and believe it correct** — which is the failure this bullet is placed here to prevent.
    - **Visibility parity, added 2026-08-16 (THR-1136 §5), director ruling in chat.** **A quantity the aftermath reports must be inspectable on some player surface — the character sheet or equivalent. A system-only quantity is invisible in both places.** Reporting a change the player can then never look up is the asymmetry this closes: per-Reach reputation tallies were absent from the character sheet and reported on every ending, so each aftermath carried two or three chips naming a quantity with no home. Christian's verdict: *"if we make it invisible it has to be invisible in the aftermath also … they are small and more systemic than telling the player anything. they are noise."* The tallies keep running — they steer scoring and gating and mint the Whispered/Known/Legendary traits at their thresholds — and a minted trait *is* sheet-visible, so it still reports normally. World standing and faction standing both pass the test and keep their chips. The rule binds authors both ways: surfacing a new quantity on an ending means giving it a player surface, or not surfacing it.
...
15. **[E] Pips are the sanctioned magnitude glyph language *for odds and price*** — twenty ~5% steps in four tiers of five, shape changing per tier — and they annotate words, never replace them. (THR-890.) **Rescoped 2026-08-12 (THR-1082), ratified by Christian in chat:** the **delta cluster** (the triangle family, `DeltaCluster`) is the sanctioned glyph language for **realised state change** on consequence chips. Two languages, two meanings, one glyph family — which is Law 10 applied rather than bent: pips still mean *effect on the odds* everywhere, and reusing that row for a change that already happened would put one glyph in two jobs, the exact confusion THR-972 §5 found at 13px. Christian's approval was conditional on the family being shared with the screens that precede the ending (*"chevrons are fine as long as we use the same iconography as the encounter screens"*), which `▲`/`▼` satisfies — `▼` is already the hand's setback marker. A third magnitude language needs a ruling, not an implementation choice.
...
31. **[E] Polarity coloring is consistent game-wide:** green = works for the mortal / gain; red = against / loss; gold = the god's attention, seeds, and the blessed; neutral warm text = context without pull. Factor lines, chips, and outcome bands all follow it — and polarity is never carried by hue alone: a word, glyph, or kind-label always accompanies the colour (amended 2026-08-06). (`NudgePhaseShell` polarity map; mockup chip tones.) **Clarified 2026-08-12 (THR-1082):** a *category* tint is not a polarity signal and must never be read as one. A consequence chip's category (SCAR / BOND / BOON / PATH) says what kind of thing changed; whether it helped or hurt is carried separately by the delta cluster's direction and by the words of its label. A BOND can be a friend made or an enemy earned, and colouring the category by polarity would make the taxonomy lie about half its members.
