...
2. Commit that file. It is the whole write — `Docs/project-status.md` is generated by `prebuild` and untracked, so no PR carries it.
...
**Note:** CLAUDE.md no longer has a `## Project Status` section — project status lives entirely in `Docs/status/`. Do not add status content to CLAUDE.md.
...
```bash
git add CLAUDE.md Docs/changelog.md Docs/status/ Docs/project-history.md
...
| `Write` | `Docs/status/YYYY-MM-DD-thr-XXXX.md` | One new file per closeout; `Docs/project-status.md` is generated from these and untracked — never edit or stage it (THR-1016) |
