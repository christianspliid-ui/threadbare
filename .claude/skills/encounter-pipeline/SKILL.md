...
model: opus
last_validated_against: 2026-09-22
...
0e. [`reference/content-tag-catalog.generated.md`](reference/content-tag-catalog.generated.md) — the closed content-tag vocabulary (THR-1486): one table per axis, what each tag means, and how many entries of each kind wear it. **DEAD** badges name tags a query site asks for that nothing wears. Read it before writing a `tags` array or a tag filter; the vocabulary is closed, and the nearest-sounding word is usually one of the sixty-three spellings the migration retired. Inject it into the draft prompt as well as the critics' — since THR-1488 an *encounter* carries tags too (the `family` word a sequel can find it by), so the choice is made while the prose is being written, not audited afterwards.

0f. **Sequels are named by family, not by id (THR-1488).** A `encounter_seed` takes `templateId` for one specific authored scene, or `query` — `{ kind: 'encounter_template', tags: ['#circle_errand'] }` — for "one of these". Reach for `query` whenever the fiction is a *kind* of follow-up; the reason is measured, not stylistic: the operand it replaces named an id **prefix**, and 41 of the corpus's 51 families matched no template at all, so those sequels had been withering on arrival since they were written. `encounterFamily` is deprecated — do not author it. Both operands are fatal in `check:encounter`, so "the sequel exists" is proved before the PR does. Full rule: `reference/nudge-authoring-spec.md` § *Seeded Sequel — the two operands*.
...
3. Obsidian vault pages, read from the filesystem via `OBSIDIAN_VAULT_PATH` (no Obsidian MCP for vault work — THR-654):
...
If `OBSIDIAN_VAULT_PATH` is unset, note it and proceed. The draft agent should not need to read these files itself.
